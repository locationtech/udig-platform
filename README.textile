...
#. Here is the branch, check it out, or fork as you see fit:
...
#. Download stuff (using maven and wget):
   <pre><code>
...
#. Build native fragments used for target platform:
   <pre><code>
     cd libs
     cd jai-imageio
     mvn clean install
   </pre></code>
#. Build Online Help (requires and sphinx-build to be installed):
   <pre><code>
     cd docs
     ant help-deploy
...
#. Packaging, branding and installers::
   <pre><code>
      cd deploy
      ./all.sh
...
#. Import the targets/indigo/eu.udig.targets.indgo project.
#. Open up udig-indigo-target.target and click on Set as Target Platform

   !target.jpg!

   This will reset the target platform which will take some time.
#. Once that is done you can import the remaining uDig projects
#. And open up *net.refractions.udig/udig.product and click *Run*
