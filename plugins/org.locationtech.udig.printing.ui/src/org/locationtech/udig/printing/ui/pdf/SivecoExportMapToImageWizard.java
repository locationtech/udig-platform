...
public class SivecoExportMapToImageWizard extends Wizard implements IExportWizard {
    ...
    private static final String FEATURE_ID_KEY = "FEATURE_ID"; //$NON-NLS-1$

    private ImageExportPage imageSettingsPage = new ImageExportPage();

    private MapSelectorPageWithScaleColumn mapSelectorPage = new MapSelectorPageWithScaleColumn(
            "Map Select", null, null); //$NON-NLS-1$
    ...
    /**
     * Utility method to export a given map to a PDF according to the passed parameters.
     */
    public static void exportSivecoMap(IMap map, int dpi, Paper paper, File dest,
            SelectionStyle selectionStyle, IProgressMonitor monitor) throws RenderException,
        ...
        try {
            ...
            DrawMapParameter drawMapParameter = new DrawMapParameter(g,
                    new java.awt.Dimension(contentDim.width, contentDim.height), mapCopy,
                    boundsStrategy, dpi, selectionStyle, monitor);
        ...
        Image2Pdf.write(image, dest.getAbsolutePath(), paper,
                /* top, left, bottom and right margin */
                new Insets(10, 10, 10, 10), isLandscape);
    ...
    private void exportMap(IMap map, IProgressMonitor monitor) throws RenderException, IOException,
        ...
        if (imageSettingsPage.isPDF()) {
            exportSivecoMap(map, imageSettingsPage.getFormat().getDPI(),
                    ((PDFImageExportFormat) imageSettingsPage.getFormat()).paper(),
                    determineDestinationFile(map), imageSettingsPage.getSelectionHandling(),
                    monitor);
            return;
        }
        ...
        try {
            ...
            DrawMapParameter drawMapParameter = new DrawMapParameter(g,
                    new java.awt.Dimension(width, height), mapCopy, boundsStrategy,
        ...
        String baseFile = destination.getPath().substring(0,
                destination.getPath().lastIndexOf(".")); //$NON-NLS-1$
        createWorldFile(renderedMap.getViewportModel().worldToScreenTransform().createInverse(),
                baseFile);
    ...
    private static List<Layer> prepareMapGraphics(Map map, Dimension contentDim,
            IProgressMonitor monitor) {

        List<Layer> mapGraphics = new ArrayList<>();

        // whitebox
        Rectangle whiteboxLoc = new Rectangle((int) (contentDim.width - (contentDim.width * 0.25)),
                0, contentDim.width, contentDim.height);
        Layer whiteboxLayer = createWhitebox(map, whiteboxLoc);
        mapGraphics.add(whiteboxLayer);

        // title area
        Rectangle titleAreaBox = new Rectangle(whiteboxLoc.x, whiteboxLoc.y, whiteboxLoc.width,
                (int) (whiteboxLoc.height * 0.25));
    ...
    /**
     * Adds a whitebox MapGraphic as the top-most layer on the map. On failure nothing is added to
     * the map.
     *
     * @param map
    ...
    private static Layer createWhitebox(Map map, Rectangle location) {
        try {
            ...
            URL resourceID = new URL(null,
                    "mapgraphic://localhost/mapgraphic#org.locationtech.udig.tutorial.mapgraphic.Whitebox", //$NON-NLS-1$
                    CorePlugin.RELAXED_HANDLER);
            ID id = new ID(resourceID);
            IGeoResource resource = localCatalog.getById(IGeoResource.class, id,
    ...
    /**
     * Adds a MapGraphic as the top-most layer on the map with details about the selected feature.
    ...
    protected static Layer createTitleArea(Map map, Rectangle location, IProgressMonitor monitor) {
        try {
            ...
            while (layerIterator.hasNext()) {
                ...
                if (layer.getName().equals("SDE.DATE_PRIMARE")) { //$NON-NLS-1$
                    farmParcelLayer = layer;
                }
            }

            if (farmParcelLayer == null) {
                throw new IllegalStateException("cannot find farm layer"); //$NON-NLS-1$
            }

            // get title from selected feature of farm layer
            Filter farmParcelFilter = farmParcelLayer.getFilter();
            Query query = new Query(farmParcelLayer.getSchema().getTypeName(), farmParcelFilter,
                    new String[0]); // {"nume_com", "fbid"}

            FeatureSource<SimpleFeatureType, SimpleFeature> featureSource = farmParcelLayer
            ...
            if (!featureIterator.hasNext()) {
                throw new IllegalStateException(
                        "At least one feature must be selected in the farm layer. " //$NON-NLS-1$
                                + features.size() + " found."); //$NON-NLS-1$
            }
            ...
            URL detailsResId = new URL(null,
                    "mapgraphic://localhost/mapgraphic#org.locationtech.udig.tutorial.mapgraphic.TitleArea", //$NON-NLS-1$
                    CorePlugin.RELAXED_HANDLER);
            IGeoResource detailsRes = localCatalog.getById(IGeoResource.class, new ID(detailsResId),
                    new NullProgressMonitor());
            ...
            XMLMemento selectedFeatureMemento = XMLMemento.createWriteRoot("style"); //$NON-NLS-1$
            selectedFeatureMemento.putString(FEATURE_ID_KEY, id);
            titleAreaLayer.getStyleBlackboard().put(DialogSettingsStyleContent.EXTENSION_ID,
                    selectedFeatureMemento);
            return titleAreaLayer;
        } catch (Exception e) {
            throw new IllegalStateException("failed to create title area", e); //$NON-NLS-1$
        }
    ...
    /**
     * This method is responsible from creating a world file to GeoReference an image given the image
     ...
     *
     * @param gridToWorld the transformation from the image to the world coordinates.
     * @param baseFile Basename and path for this image.
     * @throws IOException In case we cannot create the world file.
     * @throws TransformException
     * @throws TransformException
     */
    private void createWorldFile(final AffineTransform gridToWorld, final String baseFile)
