...
/**
 * Display a color in a table, when used for editing a palette will be presented to the user.
 *
 * @since 1.0
 */
        ...
        if (property.equals(PROPERTY_NAME_STYLE_EXPRESSION)) {
            if (element instanceof TreeItem) {
                ...
                for (int i = 0; i < rules.size(); i++) {
                    ...
                    if (thisExpr == null) {
                        ...
                        min.add(Double.valueOf(minMax[0]));
                        max.add(Double.valueOf(minMax[1]));
                    } else {
                ...
                if (min.size() > 0) {
                    ...
                    this.styleThemePage.customBreak = new RangedClassifier(
                            ...
                            max.toArray(new Double[max.size()]));
                } else {
            ...
            if (element instanceof TreeItem) {
                ...
                if (data instanceof Rule) {
                    Rule rule = (Rule) data;
                    if (rule.getDescription().getTitle().toString().equals(value))
                        return; // don't bother -- value is the same
    ...
    private void modifyColourColumn(Object element, Object value) {
        ...
        if (data instanceof Rule) {
            ...
            if (symb.length == 1) { // we're only expecting 1
                ...
                if (this.styleThemePage.customPalette == null) {
                    ...
                    for (int i = 0; i < maxColors; i++) {
                        if (i > 0) {
                            ...
                            try {
                                suitability.setSuitability(i + 1, new String[] { unknown, unknown,
                    ...
                    this.styleThemePage.customPalette
                            .setDescription(Messages.StyleEditor_theme_custom_desc);
                    this.styleThemePage.customPalette.setType(pal.getType());
                    if (!this.styleThemePage.getBrewer()
                            .hasPalette(Messages.StyleEditor_theme_custom)) {
                        this.styleThemePage.getBrewer()
                                .registerPalette(this.styleThemePage.customPalette);
                    }
                }
                ...
                if (actualIndex == -1) {
                    SLDPlugin.log(
                            "color match unsuccessful... " + oldColorExpr + " vs " + newColorExpr, //$NON-NLS-1$ //$NON-NLS-2$
                            null);
                    // TODO: use different color matching technique
                    return;
                }
                colors[actualIndex] = newColor;
                this.styleThemePage.customPalette.setColors(colors);
                this.styleThemePage.paletteTable.setInput(this.styleThemePage.getBrewer());
                this.styleThemePage.paletteTable
                        .setSelection(new StructuredSelection(this.styleThemePage.customPalette));
            }
        }
        this.styleThemePage.treeViewer.refresh();
    }
