...
public class SLDTextEditorPart extends SLDEditorPart {
    ...
    private void setStylingElements(TextSymbolizer symbolizer) {
        ...
        labelFont
                .setColorValue(new RGB(fontFill.getRed(), fontFill.getGreen(), fontFill.getBlue()));
        if (tempFD == null) {
            tempFD = new FontData[1];
            tempFD[0] = new FontData(SymbolizerContent.DEFAULT_FONT_FACE,
                    SymbolizerContent.DEFAULT_FONT_SIZE, SymbolizerContent.DEFAULT_FONT_STYLE);
        }
        ...
        if (attributeList != null) {
            for (int i = 0; i < attributeList.size(); i++) {
                ...
                if (label != null && attributeDescriptor != null
                        && attributeDescriptor.getName().toString().equals(label.toString())) {
        ...
        labelHaloColorEditor
                .setColorValue(new RGB(haloFill.getRed(), haloFill.getGreen(), haloFill.getBlue()));
    ...
    private void applyLabel(TextSymbolizer textSymbolizer, StyleBuilder styleBuilder) {
        ...
        if (labelCombo.getSelectionIndex() != -1) {
            ...
            textSymbolizer.setFill(styleBuilder
                    .createFill(new Color(xfontColor.red, xfontColor.green, xfontColor.blue)));
            ...
            for (org.geotools.styling.Font f : font)
                textSymbolizer.fonts().add(f);
            ...
            if (labelHaloEnabled.getSelection()) {
                ...
                if (textSymbolizer.getHalo() != null) {
                    if (textSymbolizer.getHalo().getFill() != null) {
                        textSymbolizer.getHalo().getFill().setColor(styleBuilder.colorExpression(
                                new Color(haloColor.red, haloColor.green, haloColor.blue)));
                    } else {
                        textSymbolizer.getHalo().setFill(styleBuilder.createFill(
                                new Color(haloColor.red, haloColor.green, haloColor.blue)));
                    }
                    textSymbolizer.getHalo().setRadius(
                            styleBuilder.literalExpression(haloWidthScale.getSelection()));
                    textSymbolizer.getHalo().getFill().setOpacity(styleBuilder.literalExpression(
                            haloOpacityScale.getSelection() / opacityMaxValueFloat));
                } else {
                    HaloImpl halo = new HaloImpl();
                    textSymbolizer.setHalo(halo);
                    textSymbolizer.getHalo().setFill(styleBuilder
                            .createFill(new Color(haloColor.red, haloColor.green, haloColor.blue)));
                    textSymbolizer.getHalo().setRadius(
                            styleBuilder.literalExpression(haloWidthScale.getSelection()));
                    textSymbolizer.getHalo().getFill().setOpacity(styleBuilder.literalExpression(
                            haloOpacityScale.getSelection() / opacityMaxValueFloat));
            ...
            currLabel = CommonFactoryFinder.getFilterFactory(GeoTools.getDefaultHints())
