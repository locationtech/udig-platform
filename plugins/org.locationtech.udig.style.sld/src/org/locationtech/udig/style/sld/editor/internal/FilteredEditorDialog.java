...
/**
 * Baseclass for Editor dialogs that will show two tabs of Editors - filtered and unfiltered.
 *
 * @since 3.0
 */
public abstract class FilteredEditorDialog extends EditorDialog
    ...
    /**
     * Creates a new Editor dialog under the control of the given Editor manager.
     *
    ...
    /**
     * Differs from super implementation in that if the node is found but should be filtered based
     * on a call to <code>WorkbenchActivityHelper.filterItem()</code> then <code>null</code> is
     * returned.
     *
    ...
    /**
     * Selects the current page based on the given Editor page identifier. If no node can be found,
     * then nothing will change.
     *
     * @param EditorPageId The Editor page identifier to select; should not be <code>null</code>.
     */
    ...
    protected void okPressed() {
        ...
        if (workingCopyManager != null)
            try {
                ...
                IStatus errorStatus = new Status(IStatus.ERROR, StylePlugin.ID, IStatus.ERROR, msg,
                        e);
                ErrorDialog.openError(getShell(),
                        Messages.FilteredEditorDialog_very_informative_error,
                        Messages.FilteredEditorDialog_save_failed, errorStatus);
            }
    ...
    protected boolean showPage(IEditorNode node) {
        ...
        if (success) {
            history.addHistoryEntry(new PageHistoryEntry(node.getId(), node.getLabelText(), null));
        }
        return success;
    }
    ...
    protected void selectSavedItem() {
        ...
        if (getTreeViewer().getTree().getItemCount() > 1) {
            // unfortunately super will force focus to the list but we want the type ahead combo to
            // get it.
