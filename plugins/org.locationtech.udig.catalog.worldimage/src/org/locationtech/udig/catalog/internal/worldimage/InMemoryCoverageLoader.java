...
public class InMemoryCoverageLoader extends GridCoverageLoader {
    ...
    static {
        ...
        Envelope envelope = new ReferencedEnvelope(0, .00001, 0, 0.00001,
    ...
    private volatile SoftReference<GridCoverage> coverage = new SoftReference<>(null);
    ...
    public synchronized GridCoverage load(GeneralGridGeometry geom, IProgressMonitor monitor)
        ...
        if (coverage.get() == null) {
            try {
                AbstractGridCoverage2DReader reader = resource
                        .resolve(AbstractGridCoverage2DReader.class, monitor);
                ...
                RasteringsPlugin.log("WARNING.  Loading image fully into memory.  It is about " //$NON-NLS-1$
                        + size(image) + " MB in size decompressed", null); //$NON-NLS-1$

                @SuppressWarnings("rawtypes")
                BufferedImage bi = new BufferedImage(image.getColorModel(),
                        (WritableRaster) image.getData(), false, new Hashtable());
                GridCoverageFactory fac = new GridCoverageFactory();

                GridCoverage2D c = fac.create(fileName, bi, env);
                coverage = new SoftReference<>(c);
    ...
    private void updateMemoryLevel() throws IOException {
        ...
        coverage = new SoftReference<>(EMPTY_COVERAGE);
        ...
        Display.getDefault().asyncExec(new Runnable() {
            ...
            public void run() {
                ...
                String desc = MessageFormat.format(InMemoryCoverageLoader_message,
                        resource.getIdentifier(), finalHeap);
