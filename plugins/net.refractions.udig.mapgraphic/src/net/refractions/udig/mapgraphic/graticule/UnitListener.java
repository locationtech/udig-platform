 ...
 * 
 * @see GraticuleStyleConfigurator
 * @author Jesse
 * @since 1.1.0
 */
public class UnitListener implements ModifyListener {
    ...
    private void converSpinnerValueToNewUnit( ) {
        ...
        if( GraticuleStyleConfigurator.selectedString(thisSpacer.getUnit()).equals(SpacerController.PIXELS)){
            convertSpinner(oldValue, 1, 1, 1, 0);
        }else if( GraticuleStyleConfigurator.selectedString(thisSpacer.getUnit()).equals(SpacerController.MINUTES)){
            convertSpinner(oldValue, 60, 1, 1/(double)60, 4);
        }else if( GraticuleStyleConfigurator.selectedString(thisSpacer.getUnit()).equals(SpacerController.SECONDS)){
            convertSpinner(oldValue, 3600, 60, 1, 2);
        }else if( GraticuleStyleConfigurator.selectedString(thisSpacer.getUnit()).equals(SpacerController.DEGREES)){
            convertSpinner(oldValue, 1, 1/(double)60, 1/(double)3600, 6);
        }
        
        thisSpacer.getUnit().setData(GraticuleStyleConfigurator.selectedString(thisSpacer.getUnit()));
    }
