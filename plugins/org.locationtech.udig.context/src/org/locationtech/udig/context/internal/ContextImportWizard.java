...
public class ContextImportWizard extends Wizard implements IImportWizard {
    ...
    public static void process(final URL url, IProgressMonitor monitor) throws Exception {
        ...
        try {
            ...
            monitor.subTask(
                    MessageFormat.format(Messages.ContextImportWizard_task_connecting, url));
            monitor.worked(5);
            InputStream input = null;
            try {
                input = url.openStream();
                monitor.worked(15);
                SubMonitor subMonitor = SubMonitor.convert(monitor, 80);
                try {
    ...
    static protected void process(InputStream input, IProgressMonitor monitor) throws Exception {
        ...
        monitor.worked(55);
        process(document, SubMonitor.convert(monitor, 40));
    }
    ...
    static public void process(Document document, IProgressMonitor monitor) {
        ...
        monitor.beginTask(MessageFormat.format(Messages.ContextImportWizard_importing, title),
                resourceList.getContentSize() + 2);
    ...
    static private void processBoundingBox(Element boundingBox, Map map) {
        ...
        try {
            ...
            env = new Envelope(Double.parseDouble(lowerCorner[0]),
                    Double.parseDouble(upperCorner[0]), Double.parseDouble(lowerCorner[1]),
                    Double.parseDouble(upperCorner[1]));
    ...
    static private void processFeatureType(Element resource, Map map) {
        ...
        try {
            URL url = new URL(
                    href + "?SERVICE=WFS&VERSION=" + serverversion + "&REQUEST=GetCapabilities"); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    static private void processLayer(Element resource, Map map) {
        ...
        if (queryable != null) {
            layer.setInteraction(Interaction.SELECT, !"0".equals(queryable)); //$NON-NLS-1$
        }
        ...
        }
        {
            ...
            try {
                URL url = new URL(
                        href + "?SERVICE=WMS&VERSION=" + version + "&REQUEST=GetCapabilities"); //$NON-NLS-1$ //$NON-NLS-2$
     ...
     * to create the map based on the collected parameters. Builders are parameterized by a factory -
    ...
    public void init(IWorkbench workbench, IStructuredSelection selection) {
        ...
        // selection can be used to figure out project ...
    }

    @Override
    public void addPages() {
        super.addPages();
        page = new URLWizardPage(Messages.ContextImportWizard_wizard_name,
                Messages.ContextImportWizard_wizard_title,
                ContextPlugin.getImageDescriptor("icons/wizban/import_owscontext_wiz_gif")) { //$NON-NLS-1$

            /** Override to check extension */
            @Override
            protected boolean fileCheck(File file) {
                ...
                // return true;
                // }
                // setErrorMessage("File required to have extension XML" );
                // return false;
                return true;
            }
        };
