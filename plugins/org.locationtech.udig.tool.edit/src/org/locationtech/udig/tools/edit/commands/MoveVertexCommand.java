...
public class MoveVertexCommand extends AbstractCommand implements UndoableMapCommand {
    ...
    public MoveVertexCommand(Point lastPoint, Selection toMove, EditToolHandler handler,
            Point start, EditState stateAfterSnap, boolean doSnap) {
    ...
    public void run(IProgressMonitor monitor) throws Exception {
        if (command == null) {
            ...
            int deltaY = dest.getY() - lastPoint.getY();
            ...
            command.run(SubMonitor.convert(monitor, 1));
        }
    }

    private Coordinate calculateDestinationPoint(EditBlackboard editBlackboard,
            Coordinate destinationCoord) {
        if (doSnap) {
            destinationCoord = EditUtils.instance.getClosestSnapPoint(handler, editBlackboard,
                    lastPoint, false, snapBehaviour, stateAfterSnap);
        }
        Point dest;
        if (destinationCoord == null) {
            dest = editBlackboard.overVertex(lastPoint, PreferenceUtil.instance().getVertexRadius(),
                    true);
            if (dest == null)
                dest = lastPoint;
    ...
    public void rollback(IProgressMonitor monitor) throws Exception {
        command.rollback(SubMonitor.convert(monitor, 1));
    }
