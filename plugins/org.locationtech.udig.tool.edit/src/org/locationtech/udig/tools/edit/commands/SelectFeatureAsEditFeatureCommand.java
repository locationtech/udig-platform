 ...
 * @since 1.1.0
 */
public class SelectFeatureAsEditFeatureCommand extends AbstractCommand
        implements Command, UndoableMapCommand {
    ...
    public SelectFeatureAsEditFeatureCommand(EditToolHandler handler, SimpleFeature feature,
            ILayer selectedLayer, Point mouse) {
        if (mouse == null)
    ...
    public void run(IProgressMonitor monitor) throws Exception {
        ...
        command = context.getEditFactory().createSetEditFeatureCommand(feature, selectedLayer);

        unselectcommand.setMap(getMap());
        command.setMap(getMap());

        unselectcommand.run(SubMonitor.convert(monitor, 10));
        command.run(SubMonitor.convert(monitor, 10));
        ...
        Collection<EditGeom> geoms = handler.getEditBlackboard(selectedLayer)
                .setGeometries((Geometry) feature.getDefaultGeometry(), feature.getID()).values();
        Class<?> type = selectedLayer.getSchema().getGeometryDescriptor().getType().getBinding();
        boolean polygonLayer = Polygon.class.isAssignableFrom(type)
    ...
    public void rollback(IProgressMonitor monitor) throws Exception {
        ...
        command.rollback(SubMonitor.convert(monitor, 10));
        unselectcommand.rollback(SubMonitor.convert(monitor, 10));
    }
