...
public class FeatureUtils {
    ...
    public static <T extends Geometry> SimpleFeature createFeature(
            ...
            SimpleFeatureType type, Coordinate[] coordinates, Class<T> geomType) throws Exception {
    ...
    public static Map<String, String> createAttributeMapping(SimpleFeatureType sourceSchema,
     ...
     */
    private static void mapGeometryAttributes(SimpleFeatureType sourceSchema,
            SimpleFeatureType targetSchema, Map<String, String> queryAttributes) {
        ...
        if (defaultGeometry == null) {
            ...
            if (defaultGeometry.getType().getBinding().isAssignableFrom(binding)) {
                queryAttributes.put(defaultGeometry.getName().getLocalPart(),
                ...
                for (int i = 0; i < sourceSchema.getAttributeCount(); i++) {
                    ...
                    if (defaultGeometry.getType().getBinding()
     ...
     */
    private static void performDirectMapping(SimpleFeatureType sourceSchema,
            SimpleFeatureType targetSchema, Map<String, String> queryAttributes) {
        for (int i = 0; i < sourceSchema.getAttributeCount(); i++) {
            AttributeDescriptor source = sourceSchema.getDescriptor(i);
            for (int j = 0; j < targetSchema.getAttributeCount(); j++) {
                ...
                if (target.getName().getLocalPart()
                        .equalsIgnoreCase(source.getName().getLocalPart())) {
                    queryAttributes.put(target.getName().getLocalPart(),
                            source.getName().getLocalPart());
                }
            }
        }
    ...
    public static Collection<SimpleFeature> copyFeature(final SimpleFeature source,
            final SimpleFeatureType destinationSchema, final Map<String, String> attributeMap,
            final MathTransform mt) {

        return new AbstractCollection<SimpleFeature>() {
            @Override
            public Iterator<SimpleFeature> iterator() {
                final Map<String, Iterator<? extends Geometry>> geometries = new HashMap<>();
                final Object[] attributes = copyAttributes(destinationSchema, source, geometries,
                        attributeMap, mt);
                ...
                return new Iterator<SimpleFeature>() {
                    ...
                    public boolean hasNext() {
                        if (template == null) {
                            try {
                                template = next = SimpleFeatureBuilder.build(destinationSchema,
                                        attributes, null);
            ...
            public int size() {
                int size = 0;
                for (Iterator<SimpleFeature> iter = iterator(); iter.hasNext(); iter.next())
                    size++;
                return size;
            }
        };
    ...
    private static Object[] copyAttributes(SimpleFeatureType destSchema, SimpleFeature source,
            Map<String, Iterator<? extends Geometry>> geometries, Map<String, String> attributeMap,
            MathTransform mt) {
        ...
        for (int i = 0; i < attributes.length; i++) {
            ...
            if (name != null) {
                if (destSchema.getDescriptor(sourceAttributeName).getType().getBinding()
                        .isAssignableFrom(source.getFeatureType().getDescriptor(name).getType()
                                .getBinding())) {
                    attributes[i] = source.getAttribute(name);
                } else {
                    // if not directly assignable then check for an adapter
                    if (AdapterUtil.instance.canAdaptTo(source.getAttribute(name),
    ...
    private static Collection<? extends Geometry> createCompatibleGeometry(Geometry geom,
        ...
        if (nonCollectionToPoint(geom, targetType, result))
            ...
            return result;
        throw new IllegalArgumentException("do not know how transform from " //$NON-NLS-1$
                + geom.getClass().getName() + " to " + targetType.getName()); //$NON-NLS-1$

    }

    /**
     * return true if Geometry is a point and targetType is a point. Result will be populated with
     * the new point
     */
    ...
    private static boolean collectionToMultiPoint(Geometry geom,
            Class<? extends Geometry> targetType, Collection<Geometry> result) {
    ...
    private static boolean nonCollectionToMultiPoint(Geometry geom,
            Class<? extends Geometry> targetType, Collection<Geometry> result) {
        if (!(geom instanceof GeometryCollection) && MultiPoint.class == targetType) {
    ...
    private static boolean collectionToLine(Geometry geom, Class<? extends Geometry> targetType,
        ...
        if (!(geom instanceof Polygon) && !(geom instanceof MultiPolygon)
            ...
            for (int i = 0; i < geom.getNumGeometries(); i++) {
                result.add(geomBuilder.safeCreateGeometry(targetType,
    ...
    private static boolean polygonToLine(Geometry geom, Class<? extends Geometry> targetType,
        ...
        if (geom instanceof Polygon && LineString.class == targetType) {
            ...
            result.add(geomBuilder.safeCreateGeometry(LineString.class,
    ...
    private static boolean polygonToMultiLine(Geometry geom, Class<? extends Geometry> targetType,
        ...
        if (geom instanceof Polygon && MultiLineString.class == targetType) {
            ArrayList<Geometry> tmp = new ArrayList<>();
    ...
    private static boolean multiPolygonToMultiLine(Geometry geom,
            Class<? extends Geometry> targetType, Collection<Geometry> result) {
        if (geom instanceof MultiPolygon && MultiLineString.class == targetType) {
            ...
            for (int i = 0; i < geom.getNumGeometries(); i++) {
                if (!polygonToMultiLine(geom.getGeometryN(i), targetType, result))
                    throw new RuntimeException(
                            "Huh?  multi polygons should only have polygons in them, found a " //$NON-NLS-1$
                                    + geom.getGeometryN(i));
            }
    ...
    private static boolean multiPolygonToPolygon(Geometry geom,
            Class<? extends Geometry> targetType, Collection<Geometry> result) {
    ...
    private static boolean collectionToMultiPolygon(Geometry geom,
            Class<? extends Geometry> targetType, Collection<Geometry> result) {
        if (!(geom instanceof Polygon) && geom instanceof GeometryCollection
                && MultiPolygon.class == targetType) {
            for (int i = 0; i < geom.getNumGeometries(); i++) {
    ...
    private static boolean polygonToMultiPolygon(Geometry geom,
            Class<? extends Geometry> targetType, Collection<Geometry> result) {
