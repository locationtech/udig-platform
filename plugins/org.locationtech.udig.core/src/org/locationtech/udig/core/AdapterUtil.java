...
public class AdapterUtil {
    ...
    public IAdaptable getAdaptable(Object obj) {
        ...
        if (Platform.getAdapterManager().hasAdapter(obj, IAdaptable.class.getSimpleName()))
            return Platform.getAdapterManager().getAdapter(obj, IAdaptable.class);
        return null;
    }
    ...
    public IBlockingAdaptable getBlockingAdapter(Object obj) {
        ...
        if (Platform.getAdapterManager().hasAdapter(obj, IBlockingAdaptable.class.getName()))
            return Platform.getAdapterManager().getAdapter(obj, IBlockingAdaptable.class);
        if (obj instanceof IAdaptable) {
            IAdaptable adapter = (IAdaptable) obj;
            return adapter.getAdapter(IBlockingAdaptable.class);
        }
        return null;
    }
     ...
     * <p>
     * Example: A SimpleFeatureType readonly operation is called on a IResolve. The
     * getOperationTarget would resolve(FeatureSource.class) and return the SimpleFeature Source for
     * the operation.
     * </p>
     *
    ...
    public <T> T adapt(String targetClass, Object obj, IProgressMonitor monitor)
            throws IOException {
        Class<T> target = null;
        ...
        try {
            ...
            // do nothing.
            CorePlugin.log(
                    "This exception is bad.  The framework should not allow this to be reached.", //$NON-NLS-1$
                    e);
            return null;
        }
