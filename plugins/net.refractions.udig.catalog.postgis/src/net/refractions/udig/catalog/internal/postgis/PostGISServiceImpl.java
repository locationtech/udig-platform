 ...
 * @since 0.6
 */
public class PostGISServiceImpl extends IService {
     ...
     * @param arg2
     */
    public PostGISServiceImpl( URL arg1, Map<String, Serializable> arg2 ) {
        url = arg1;
        params = arg2;
    }
    ...
    public <T> T resolve( Class<T> adaptee, IProgressMonitor monitor ) throws IOException {
        ...
        if (adaptee.isAssignableFrom(PostgisDataStore.class))
    ...
    public <T> boolean canResolve( Class<T> adaptee ) {
        ...
        return adaptee.isAssignableFrom(PostgisDataStore.class) ||
     ...
     */
    public List<PostGISGeoResource> resources( IProgressMonitor monitor ) throws IOException {

        PostgisDataStore ds=getDS();
        rLock.lock();
        try{
            if (members == null) {
                members = new LinkedList<PostGISGeoResource>();
                String[] typenames = ds.getTypeNames();
                if (typenames != null)
                    for( int i = 0; i < typenames.length; i++ ) {
                        members.add(new PostGISGeoResource(this, typenames[i]));
                    }
            }
        }finally{
            rLock.unlock();
    ...
    private volatile List<PostGISGeoResource> members = null;
    ...
    public IServiceInfo getInfo( IProgressMonitor monitor ) throws IOException {
        ...
        if (info == null && ds != null) {
            ...
            try{
                if (info == null) {
                    info = new IServicePostgisInfo(ds);
                }
            }finally{
                rLock.unlock();
            }
    ...
    private volatile PostgisDataStore ds = null;
    private Lock dsInstantiationLock=new UDIGDisplaySafeLock();
    PostgisDataStore getDS() throws IOException {
    ...
    private class IServicePostgisInfo extends IServiceInfo {

        IServicePostgisInfo( PostgisDataStore resource ) {
            super();
            String[] tns = null;
            try {
                tns = resource.getTypeNames();
            } catch (IOException e) {
                PostgisPlugin.log("Unable to read typenames", e); //$NON-NLS-1$
                tns = new String[0];
            }
        ...
        public String getTitle() {
            Serializable user = params.get(PostgisDataStoreFactory.USER.key);
            Serializable schema = params.get(PostgisDataStoreFactory.SCHEMA.key);
			return "PostGIS: "+ user + "@"+getIdentifier().getHost()+"/"+schema; //$NON-NLS-1$
        }
        public ImageDescriptor getIcon() {
            return AbstractUIPlugin.imageDescriptorFromPlugin(PostgisPlugin.ID,
