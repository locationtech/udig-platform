...
/**
 * Composite responsible of managing the mark model coordinates edit operations with the aid of
 * tools that interact with the map and a table to directly edit them.
 *
 * It's also an observer of the {@link GeoReferencingCommand} and {@link MarkModel}.
 *
 ...
 *
 */
public final class CoordinateTableComposite extends Composite
        implements Observer, GeoReferencingComposite {
    ...
    private List<MouseSelectionListener> listeners = new LinkedList<>();
    ...
    private void createContent() {
        ...
        Composite compositeGridCoord = new Composite(this, SWT.BORDER);
    ...
    private void createListeners() {
        ...
        this.capturedListener = new CapturedCoordinateListener() {
            ...
            public void capturedCoordinate(Coordinate newCoord) {
                ...
                MainComposite mainComposite = getMainComposite();
                assert mainComposite != null;
                assert mainComposite.getMapMarkGraphic() != null;
                mainComposite.refreshMapGraphicLayer();
            }
    ...
    /**
     * Creates the {@link ImageRegistry} that stores the image shown by the tools.
     *
     * @return The imageRegistry.
     */
    private ImageRegistry createImageRegistry() {

        ImageRegistry imageRegistry = new ImageRegistry(this.getDisplay());

        String opId = "Delete"; //$NON-NLS-1$
        String imgFile = "image/delete.gif"; //$NON-NLS-1$
        imageRegistry.put(opId,
                ImageDescriptor.createFromFile(CoordinateTableComposite.class, imgFile));
    ...
    private void createToolbar(Composite parent) {
        ...
        ToolBar mapToolBar = new ToolBar(parent, SWT.LEFT_TO_RIGHT);
    ...
    private void createGrid(final Composite parent) {
        ...
        coordinatesTable = new Table(parent, SWT.BORDER | SWT.SINGLE | SWT.H_SCROLL | SWT.V_SCROLL
        ...
        TableColumn tableColumnID = new TableColumn(coordinatesTable, SWT.NONE);
        ...
        parent.addControlListener(new ControlAdapter() {
            ...
            public void controlResized(ControlEvent e) {
                ...
                int width = area.width - coordinatesTable.computeTrim(0, 0, 0, 0).width
                        - vBar.getSize().x;
     ...
     *
     * @param mark Associated mark model
    ...
    /**
     * Find the row that contains a reference to this mark model and update the values of that row.
     *
     * @param mark The mark model.
     */
     ...
     *
     * @param mark MarkModel model.
     * @param item Table item.
     * @param newCoord Coordinate with the values.
     */
     ...
     *
     * Creates a {@link MarkMapPresenterImp} and associates it with the {@link MapMarksGraphics}.
     * </p>
     *
     * @param mark Mark model.
     */
    private void addMarkOnTable(MarkModel mark) {
        ...
        mainComposite.getMapMarkGraphic().addMarkMapPresenter(markPresenter);
     ...
     *
     * There must be 1 empty row. If there is a selected row and also is empty, then the coordinate
     * will be added there. If there isn't any selected row, the coordinate will be added in the
     * first empty row. If there isn't any empty rows, the coordinate won't be added.
     *
     *
     * @param newCoord Coordinate to be added.
     */
    private void addCoordinateFromMapToTable(Coordinate newCoord) {
        ...
        if (selection.length > 0) {
            ...
            MainComposite mainComposite = getMainComposite();
            if (mainComposite != null) {
                mainComposite.setMessage(new InfoMessage(
                        Messages.CoordinateTableComposite_cant_add_gcp, Type.WARNING));
            }
     ...
     *
     * @param e
     * @throws NumberFormatException When the given character isn't an Integer.
     */
    private void validateCharacter(KeyEvent e) throws NumberFormatException {

        if (e.character == '.' || e.keyCode == SWT.DEL || e.keyCode == SWT.BS
                || e.keyCode == SWT.END || e.keyCode == SWT.HOME || e.keyCode == 16777219
                || e.keyCode == 16777220 || e.character == '-' || e.character == '+') {
            return;
        }
    ...
    /**
     * Delete this mark on the table and refresh the layer that contains the mapGraphic so the
     * changed are shown.
     *
     * @param mark Mark model to be deleted.
     */
    ...
    private void deleteAllMarksOnTable() {
        ...
        MainComposite mainComposite = getMainComposite();
        if (mainComposite != null) {
            mainComposite.getMapMarkGraphic().clear();
        }
    }

    /**
     * Add a coordinate into table when mark associated to this table item doesn't have any
     * coordinate value stored.
     *
     * @param item Table item
     * @param newCoord Coordinate to be added.
     * @return True when the coordinate was added into the table.
     */
    private boolean addCoordinateOnTable(TableItem item, Coordinate newCoord,
            boolean existSelection) {
     ...
     * @param item Table item.
     * @param mark MarkModel model.
     * @param newCoord The coordinate containing the data.
     */
    ...
    public void setContext(IToolContext newContext) {
        ...
        if (this.toolContext == null) {
            ...
            MainComposite mainComposite = getMainComposite();
            if (mainComposite != null) {
                mainComposite.getMapMarkGraphic().addMouseSelectionListener(mapSelectionListener);
                mainComposite.addMouseSelectionListenerToImgComposite(imageSelectionListener);
            }
        }
    ...
    private void hideCoordinateTools() {
        ...
        // this is a workaround to provoke an event in the layer view that could be catch by the
        // CoordToolPropertyValue
        CoordToolPropertyValue.setVisible(false);

        EditManagerImpl editManager = (EditManagerImpl) ApplicationGIS.getActiveMap()
                .getEditManager();
    ...
    /**
     * <p>
     * When a mouse_down event occur, gets all the map presenters and search if any of these
     * presenters are affected by the event. If any of the presenter returns true, this mark
     * coordinate data will be deleted from the table and from the mark model.
     *
     * If the event is not a mouse_down, delegates the event to the {@link MapMarksGraphics}.
     * </p>
     *
     * @param point Map point where it was clicked.
    ...
    private void deleteFromTable(java.awt.Point point, InputEvent event) {
        ...
        MapMarksGraphics mapMarkGraphics = mainComposite.getMapMarkGraphic();

        if (InputEvent.MOUSE_DOWN.equals(event)) {
            // get the presenters
            Map<String, MarkMapPresenter> presenters = mapMarkGraphics.getPresenters();
            ...
            mapMarkGraphics.eventhandler(event, point);
        }
    }
    ...
    private void moveFromTable(java.awt.Point point, InputEvent event) {
        ...
        // the mapMarkGraphics will be the responsible to handle these behaviors
        MainComposite mainComposite = getMainComposite();
        if (mainComposite != null) {
            mainComposite.getMapMarkGraphic().eventhandler(event, point);
        }
    }
    ...
    /**
     * Broadcast to all its listeners that the given mark has been selected in the coordinate table.
     *
     * @param mark The mark model
     */
