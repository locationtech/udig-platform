...
/**
 * Preview created to show the {@link MarkMapPresenterImp} moving or D&D through the map in real
 * time.
 *
...
class MarkMapPreview extends AbstractMarkPresenter implements MarkMapPresenter {
    ...
    /**
     * Constructor. Like all the previews, they have a parent presenter. With this constructor, it
     * knows which one is his father.
     *
     * @param parent The parent presenter.
     */
    ...
    public boolean eventHandler(InputEvent event, int x, int y) {
        ...
        if (event == InputEvent.MOUSE_DRAG) {
    ...
    public void draw() {
        ...
        if (point != null) {
            ...
            this.extent = new Rectangle((int) (point.getX() - halfsize),
                    (int) (point.getY() - halfsize), EXTENT_SIZE, EXTENT_SIZE);

            graphics.fillOval(extent.x, extent.y, extent.width, extent.height);
            Rectangle2D stringBounds = graphics
                    .getStringBounds(String.valueOf(getMarkModel().getID()));
