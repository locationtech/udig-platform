...
public class ContextExportWizard extends Wizard implements IExportWizard {
    ...
    private static void writeFeatureType( ILayer layer, BufferedWriter out ) throws IOException {
        UDIGWFSDataStore wfs = (UDIGWFSDataStore) layer.getResource( WFSDataStore.class, null );
        WFSCapabilities caps = wfs.getCapabilities();
        FeatureType type = layer.getSchema();
        FeatureSetDescription description = find( caps, type.getTypeName() );

        int hidden = layer.isVisible() ? 1 : 0;
        String title = caps.getService().getTitle();
        String version = "1.0.0"; // only known entry at this time //$NON-NLS-1$
        String get = caps.getGetCapabilities().getGet().toExternalForm();

        append( 4, out, "<FeatureType hidden=\""+ hidden +"\">" ); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        append( 6, out,   "<Name>"+description.getName()+"</Name>"); //$NON-NLS-1$ //$NON-NLS-2$
        append( 6, out,   "<Title>"+description.getTitle()+"</Title>");     //$NON-NLS-1$ //$NON-NLS-2$
...
//        append( 6, out,   "<sld:MinScaleDenominator>"+layer.getMaxScaleDenominator()+"</sld:MinScaleDenominator>");
        append( 6, out,   "<SRS>"+description.getSRS()+"</SRS>"); //$NON-NLS-1$ //$NON-NLS-2$
        append( 4, out, "</FeatureType>" ); //$NON-NLS-1$
    }
...
for( String format : caps.getRequest().getGetMap().getFormatStrings() ){
    if( first ){
