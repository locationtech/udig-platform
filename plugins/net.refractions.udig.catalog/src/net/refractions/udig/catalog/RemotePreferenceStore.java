...
/**
 * An abstract preference store implementation for remote preference stores.
 * This class is quite similar to
 * <code>org.eclipse.jface.preference.PreferenceStore</code>, with the
 * exception that it loads and saves its values from a remote source on startup
 * and shutdown, respectively.
 *
 * @author chorner
 */
public abstract class RemotePreferenceStore implements
	...
	/**
	 * The locally stored copy of the map from preference name to preference
	 * value (represented as strings).
	 */
	...
	/**
	 * Obtains the remote value for the specified preference. Subclasses must
	 * implement.
	 *
	 * @param name
	 *            key
	...
	/**
	 * This is the initial hit of the remote store. All methods that read or
	 * write will check to ensure we've interacted with the store prior to doing
	 * anything.
	 */
	protected void load() {
		// grab all the preferences (initial load so we can detect local
		// modifications)
	...
	/**
	 * Had to add this method as the Property object was being unreliable in
	 * returning its contents using the getProperty method. We also want null to
	 * be null, rather than not "null".
	 */
	...
	public boolean contains(String name) {
		...
		if (localProperties.containsKey(name)
				|| defaultProperties.containsKey(name))
			return true;
	...
	public boolean isDefault(String name) {
		...
		return (!localProperties.containsKey(name) && defaultProperties
				.containsKey(name));
	}
	...
	public void setValue(String name, int value) {
		...
		if (oldValue != value) {
			setValue(localProperties, name, value);
			dirty = true;
			firePropertyChangeEvent(name, Integer.valueOf(oldValue), Integer
					.valueOf(value));
		}
	}
	...
	public void setValue(String name, long value) {
		...
		if (oldValue != value) {
			setValue(localProperties, name, value);
			dirty = true;
			firePropertyChangeEvent(name, Long.valueOf(oldValue), Long
					.valueOf(value));
		}
	}
	...
	public void setValue(String name, boolean value) {
		...
		if (oldValue != value) {
			setValue(localProperties, name, value);
			dirty = true;
			firePropertyChangeEvent(name, Boolean.valueOf(oldValue), Boolean
					.valueOf(value));
		}
	}
