...
import static org.geotools.data.mysql.MySQLDataStoreFactory.DBTYPE;
import static org.geotools.data.mysql.MySQLDataStoreFactory.PORT;
...
public class MySQLuDigConnectionFactory extends UDIGConnectionFactory {
    ...
    protected URL toCapabilitiesURL(String string) {
        ...
        if (!string.contains("mysql.jdbc") && !string.contains("jdbc.mysql") //$NON-NLS-1$ //$NON-NLS-2$
                && !string.contains("mysql")) { //$NON-NLS-1$
            return null;
        }
        ...
        URL url = null;
        try {
            url = MySQLServiceExtension.toURL(the_username, the_password, the_host, intPort,
                    the_database);
        } catch (MalformedURLException e) {
