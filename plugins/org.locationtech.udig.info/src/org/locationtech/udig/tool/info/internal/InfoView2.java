...
public class InfoView2 extends SearchPart {
    ...
    private class InfoViewLabelProvider extends LabelProvider implements IColorProvider {
        ...
        public Color getBackground(Object element) {
            if (element instanceof AdaptableFeature) {
                ...
                IColorProvider colorProvider = (IColorProvider) layer
                        .getAdapter(IColorProvider.class);
    ...
    protected void fillActionBars() {
        ...
        IAction infoTool = ApplicationGIS.getToolManager().getToolAction(InfoTool.ID,
    ...
    protected void showDetail(Object selection) {
        ...
        if (selection instanceof SimpleFeature) {
            ...
            FeaturePropertySource source = new FeaturePropertySource((SimpleFeature) selection,
                    false, false);
        ...
        if (selection instanceof LayerPointInfo) {
            ...
            if (info.getMimeType() == null) {
                ...
                browserDisplay.setInfo(info);
                    ...
                    && info.getMimeType().startsWith(LayerPointInfo.XML)) {
                getDetails().showPage(browserDisplay.getControl());
                browserDisplay.setInfo(info);
            } else if (info.getRequestURL() != null
                    && info.getMimeType().startsWith(LayerPointInfo.ERROR)) {
                getDetails().showPage(browserDisplay.getControl());
                browserDisplay.setInfo(info);
            } else if (info.getRequestURL() != null
                    && info.getMimeType().startsWith(LayerPointInfo.TEXT)) {
                getDetails().showPage(browserDisplay.getControl());
                browserDisplay.setInfo(info);
            } else {
                getDetails().showPage(information);
    ...
    protected void searchImplementation(Object filter, IProgressMonitor monitor, ResultSet set) {
        ...
        for (int i = request.layers.size() - 1; i > -1; i--) {
            ...
            if (!layer.isVisible())
                continue;
            if (!layer.getInteraction(Interaction.INFO))
                continue;
            ...
            if (layer.hasResource(FeatureSource.class)) {
                try {
                    List<SimpleFeature> more = DataStoreDescribeLayer.info2(layer, request.bbox,
                            monitor);
                    if (!more.isEmpty()) {
                        set.addAll(more);
                    }
            ...
            if (layer.hasResource(GridCoverage.class)) {
                try {
                    CoveragePointInfo hit = CoverageDescribeLayer.info2(layer, request.bbox,
                            monitor);
                    if (hit != null)
                        set.add(hit);
                } catch (Throwable t) {
    ...
    /**
     * With each entry from the tree, creates a FidFilter, retrieves from the store that query. If a
     * feature is returned, then this feature still exist, if not, then remove the associated
     * treeItem.
     */
    private void removeFeatureFromTree() {
        ...
        Display.findDisplay(fatherThread).asyncExec(new Runnable() {
            ...
            private boolean isFeatureOnStore(Filter id) {
                ...
                try {
                    collection = layerList.get(0).getResource(SimpleFeatureSource.class, null)
                            .getFeatures(id);
