...
public class ProjectRegistryImpl extends EObjectImpl implements ProjectRegistry {
	...
	public List<Project> getProjects() {
		if (projects == null) {
			projects = new EObjectResolvingEList(Project.class, this, ProjectPackage.PROJECT_REGISTRY__PROJECTS);
		}
	...
	public Project getDefaultProject() {
		...
		String path = new File(Platform.getLocation().toString() + File.separatorChar + projectName).getAbsolutePath();
			...
			if (currentProject != oldCurrentProject) {
				if (eNotificationRequired())
					eNotify(new ENotificationImpl(this, Notification.RESOLVE,
							ProjectPackage.PROJECT_REGISTRY__CURRENT_PROJECT, oldCurrentProject, currentProject));
			}
		}
		return currentProject;
	}
	...
	public void setCurrentProject(Project newCurrentProject) {
		...
		if (eNotificationRequired())
			eNotify(new ENotificationImpl(this, Notification.SET, ProjectPackage.PROJECT_REGISTRY__CURRENT_PROJECT,
	...
	public Project getProject(String projectPath) {
		...
		final String uriText = url.toExternalForm() + File.separatorChar + ProjectRegistry.PROJECT_FILE;
	 ...
	 * @return
	 */
	private static ProjectRegistry backwardsCompatibility(ProjectRegistry projectRegistry, IPath registrypath,
			URI uri) {
		IPath oldregistrypath = ProjectPlugin.getPlugin().getStateLocation().append("ProjectRegistry"); //$NON-NLS-1$
	...
	public static ProjectRegistry getProjectRegistry() {
		synchronized (ProjectRegistry.class) {
			...
			if (projectRegistry.eResource() == null || projectRegistry.eResource().getResourceSet() != resourceSet
					|| resourceSet == null)
				throw new AssertionError();
		}
		return projectRegistry;
