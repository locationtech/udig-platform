 ...
 * this renderer keeps a list of the children as a list of RenderInfo objects. Each of these render
 * info objects has a render executor, and a render metrics.
 *
 * <p>
 * This is used by the Tile Rendering system so that the TileRendererCreator does not have to keep
 * track of the render metrics in order to create a renderer. The default CompositeRenderer requires
 * the the TileRendererCreator create the renderers. This renderer uses the information in the
 * render metrics to create the children renderers as required.
 * </p>
 *
 * @author Emily Gouge
 * @since 1.1.0
 */
public class TiledCompositeRendererImpl extends CompositeRendererImpl
    ...
    /**
     * This is the collection of children this renderer is responsible for rendering.
     */
    private final Set<RenderInfo> childrenRenderers = new HashSet<>();
    ...
    /**
     *
     * Returns a list that is a copy of all the children renders. This list is a list of RenderInfo
     * which contains the render executor and render metrics.
     *
     */
    public List<RenderInfo> getChildren() {
        List<RenderInfo> children = new ArrayList<>();
    ...
    public List<AbstractRenderMetrics> getChildrenMetrics() {
        List<AbstractRenderMetrics> children = new ArrayList<>();
        for (RenderInfo victim : childrenRenderers) {
    ...
    /**
     * Used to create a render executor for a particular renderer. Also adds a listner to the
     * executor to listen to state events and set the state for the executor.
     *
     * @param renderer
     */
    @Override
     ...
     * maintained. This should only be used to get things such as viewport bounds and map
     * information.
     *
     */
    @Override
    ...
    public IRenderContext getIContext() {
        throw new UnsupportedOperationException(
                "Cannot get the context for a tile composite thing."); //$NON-NLS-1$
    }
    ...
    public Collection<RenderExecutor> getRenderExecutors() {
        ArrayList<RenderExecutor> executors = new ArrayList<>();
    ...
    public void render(IProgressMonitor monitor) throws RenderException {
        if (this.childrenRenderers.isEmpty())
            setState(DONE);
    ...
    public void removeChild(AbstractRenderMetrics childtoremove) {
        ArrayList<RenderInfo> toRemove = new ArrayList<>();
        synchronized (childrenRenderers) {
    ...
    /**
     * Class to track the information need for successful rendering. This includes a render executor
     * (which references a renderer) and a metrics for creating the renderer.
     *
     * <p>
     * The metrics contains the context.
     * </p>
     *
     * @author Emily Gouge
     * @since 1.1.0
