...
public class RenderFactoryImpl extends EFactoryImpl implements RenderFactory {
	...
	private RenderExecutor locateClosestFit(Renderer r) {
		...
		for (Iterator<IConfigurationElement> iter = ExtensionPointList
			...
			try {
				...
				Class<?> rendererClass = bundle.loadClass(elem.getAttribute(RenderExecutor.RENDERER_ATTR));
		...
		if (!list.isEmpty()) {
			Collections.sort(list, new Comparator<IConfigurationElement>() {
				...
				public int compare(IConfigurationElement o1, IConfigurationElement o2) {
					try {
						...
						Class clazz1 = bundle.loadClass(o1.getAttribute(RenderExecutor.RENDERER_ATTR));

						bundle = Platform.getBundle(o2.getNamespaceIdentifier());
						Class clazz2 = bundle.loadClass(o2.getAttribute(RenderExecutor.RENDERER_ATTR));
			...
			try {
				return (RenderExecutor) list.get(0).createExecutableExtension(RenderExecutor.EXECUTOR_ATTR);
	...
	private RenderExecutor locateMatch(Renderer renderer) {
		...
		for (Iterator<IConfigurationElement> iter = ExtensionPointList
				.getExtensionPointList(RenderExecutor.EXTENSION_ID).iterator(); iter.hasNext();) {
			IConfigurationElement elem = iter.next();
			if (elem.getAttribute(RenderExecutor.RENDERER_ATTR).equals(ofInterest.getClass().getName()))
				list.add(elem);
		}
	...
	public CoordinateReferenceSystem createCoordinateReferenceSystemFromString(EDataType eDataType,
		...
		return (CoordinateReferenceSystem) ProjectFactory.eINSTANCE
				.createFromString(ProjectPackage.eINSTANCE.getCoordinateReferenceSystem(), initialValue);
	}
	...
	public String convertCoordinateReferenceSystemToString(EDataType eDataType, Object instanceValue) {
		return ProjectFactory.eINSTANCE.convertToString(ProjectPackage.eINSTANCE.getCoordinateReferenceSystem(),
				instanceValue);
	}
	...
	public Envelope createEnvelopeFromString(EDataType eDataType, String initialValue) {
		return (Envelope) ProjectFactory.eINSTANCE.createFromString(ProjectPackage.eINSTANCE.getEnvelope(),
				initialValue);
	}
	...
	public String convertReferencedEnvelopeToString(EDataType eDataType, Object instanceValue) {
		return ProjectFactory.eINSTANCE.convertToString(ProjectPackage.eINSTANCE.getReferencedEnvelope(),
				instanceValue);
	}
