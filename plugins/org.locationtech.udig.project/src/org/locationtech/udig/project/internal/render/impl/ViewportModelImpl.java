...
public class ViewportModelImpl extends EObjectImpl implements ViewportModel {
	...
	public void setCRS(CoordinateReferenceSystem newCRS) {
		...
		if (getBounds().isNull() || !validState()) {
			...
			try {
				...
				if (oldCRS != DefaultEngineeringCRS.GENERIC_2D && oldCRS != DefaultEngineeringCRS.GENERIC_3D
	...
	public void setCRSGen(CoordinateReferenceSystem newCRS) {
		...
		if (eNotificationRequired())
			eNotify(new ENotificationImpl(this, Notification.SET, RenderPackage.VIEWPORT_MODEL__CRS, oldCRS, cRS,
					!oldCRSESet));
	}
	...
	public void unsetCRS() {
		...
		cRSESet = false;
		if (eNotificationRequired())
			eNotify(new ENotificationImpl(this, Notification.UNSET, RenderPackage.VIEWPORT_MODEL__CRS, oldCRS,
					getDefaultCRS(), oldCRSESet));
	}
	...
	public void setBoundsInternal(Envelope newBounds, boolean forceContainBBoxZoom) {
		...
		if (getDefaultPreferredScaleDenominators() != getPreferredScaleDenominators() && validState()) {
			...
			double scale = ScaleUtils.calculateScaleDenominator(referenced, mapDisplay.getDisplaySize(),
					mapDisplay.getDPI());
			scale = ScaleUtils.calculateClosestScale(getPreferredScaleDenominators(), scale,
					ScaleUtils.zoomClosenessPreference());

			finalBounds = ScaleUtils.calculateBoundsFromScale(scale, mapDisplay.getDisplaySize(), mapDisplay.getDPI(),
					referenced);
			if (forceContainBBoxZoom && !finalBounds.contains(newBounds)) {
				...
				if (nextLargest != null) {
					finalBounds = ScaleUtils.calculateBoundsFromScale(nextLargest, mapDisplay.getDisplaySize(),
		...
		if (!getBounds().isNull() && !Double.isNaN(getAspectRatio()) && !Double.isNaN(finalBounds.getWidth())
			...
			if (validState() && Math.abs(nRatio - dRatio) > ACCURACY) {
				...
				finalBounds.init(finalBounds.getMinX() - (arbitraryChange), (finalBounds.getMaxX() + arbitraryChange),
						finalBounds.getMinY(), finalBounds.getMaxY());
	...
	public Coordinate getCenter() {
		return new Coordinate(bounds.getMinX() + bounds.getWidth() / 2, bounds.getMinY() + bounds.getHeight() / 2);
	}
	...
	public void setCenter(Coordinate newCenter) {
		...
		setBounds(new Envelope(newCenter.x - dw, newCenter.x + dw, newCenter.y - dh, newCenter.y + dh));
	}
	...
	private boolean validState() {
		return getRenderManagerInternal() != null && getRenderManagerInternal().getMapDisplay() != null
	...
	public NotificationChain basicSetMapInternal(Map newMapInternal, NotificationChain msgs) {
		msgs = eBasicSetContainer((InternalEObject) newMapInternal, RenderPackage.VIEWPORT_MODEL__MAP_INTERNAL, msgs);
		return msgs;
	}
	...
	public void setMapInternalGen(Map newMapInternal) {
		if (newMapInternal != eInternalContainer()
			...
			if (newMapInternal != null)
				msgs = ((InternalEObject) newMapInternal).eInverseAdd(this, ProjectPackage.MAP__VIEWPORT_MODEL_INTERNAL,
						Map.class, msgs);
			msgs = basicSetMapInternal(newMapInternal, msgs);
			if (msgs != null)
				msgs.dispatch();
		} else if (eNotificationRequired())
			eNotify(new ENotificationImpl(this, Notification.SET, RenderPackage.VIEWPORT_MODEL__MAP_INTERNAL,
					newMapInternal, newMapInternal));
	}
		...
		if (newRenderManagerInternal != renderManagerInternal) {
			...
			if (renderManagerInternal != null)
				msgs = ((InternalEObject) renderManagerInternal).eInverseRemove(this,
						RenderPackage.RENDER_MANAGER__VIEWPORT_MODEL_INTERNAL, RenderManager.class, msgs);
			...
			eNotify(new ENotificationImpl(this, Notification.SET, RenderPackage.VIEWPORT_MODEL__RENDER_MANAGER_INTERNAL,
	...
	public AffineTransform worldToScreenTransform() {
		...
		return worldToScreenTransform(getBounds(), getRenderManagerInternal().getMapDisplay().getDisplaySize());
	}
	...
	public Point worldToPixel(Coordinate coord) {
		if (!validState())
			return null;
		return ScaleUtils.worldToPixel(coord, getBounds(), getRenderManagerInternal().getMapDisplay().getDisplaySize());
	}
	...
	public Coordinate pixelToWorld(int x, int y) {
		...
		return ScaleUtils.pixelToWorld(x, y, getBounds(), getRenderManagerInternal().getMapDisplay().getDisplaySize());
	}
	...
	public ViewportModel panUsingWorldCoords(double x, double y) {
		...
		setBounds(bounds.getMinX() + x, bounds.getMaxX() + x, bounds.getMinY() + y, bounds.getMaxY() + y);
		return this;
	}
		...
		try {
			...
			if (extent != null && !extent.isNull() && !extent.isEmpty()) {
				...
				for (ILayer layer : layers) {
					ReferencedEnvelope layerBounds = layer.getBounds(ProgressManager.instance().get(), getCRS());
			...
			if (bounds2.getCoordinateReferenceSystem() == null || getCRS() == null
				...
				transform = CRS.findMathTransform(bounds2.getCoordinateReferenceSystem(), getCRS(), true);
	...
	private void fireNotification(Envelope oldBounds) {
		if (eNotificationRequired()) {
			eNotify(new ENotificationImpl(this, Notification.SET, RenderPackage.VIEWPORT_MODEL__BOUNDS, oldBounds,
					bounds));
	...
	public NotificationChain eBasicRemoveFromContainerFeature(NotificationChain msgs) {
		...
		case RenderPackage.VIEWPORT_MODEL__MAP_INTERNAL:
			return eInternalContainer().eInverseRemove(this, ProjectPackage.MAP__VIEWPORT_MODEL_INTERNAL, Map.class,
					msgs);
		}
	...
	public boolean eIsSet(int featureID) {
		...
		case RenderPackage.VIEWPORT_MODEL__CENTER:
			return CENTER_EDEFAULT == null ? getCenter() != null : !CENTER_EDEFAULT.equals(getCenter());
		case RenderPackage.VIEWPORT_MODEL__HEIGHT:
		...
		case RenderPackage.VIEWPORT_MODEL__PIXEL_SIZE:
			return PIXEL_SIZE_EDEFAULT == null ? getPixelSize() != null : !PIXEL_SIZE_EDEFAULT.equals(getPixelSize());
		case RenderPackage.VIEWPORT_MODEL__MAP_INTERNAL:
	...
	public void setPreferredScaleDenominators(SortedSet<Double> newPreferredScaleDenominators) {
		...
		if (eNotificationRequired())
			eNotify(new ENotificationImpl(this, Notification.SET,
					RenderPackage.VIEWPORT_MODEL__PREFERRED_SCALE_DENOMINATORS, oldPreferredScaleDenominators,
					preferredScaleDenominators));
	}
	...
	public void setCurrentTimestep(LocalDateTime newCurrentTimestep) {
		...
		if (eNotificationRequired())
			eNotify(new ENotificationImpl(this, Notification.SET, RenderPackage.VIEWPORT_MODEL__CURRENT_TIMESTEP,
					oldCurrentTimestep, currentTimestep));
	}
	...
	public void setCurrentElevation(Double newCurrentElevation) {
		...
		if (eNotificationRequired())
			eNotify(new ENotificationImpl(this, Notification.SET, RenderPackage.VIEWPORT_MODEL__CURRENT_ELEVATION,
	...
	public void setScale(double scaleDenominator) {
		...
		ReferencedEnvelope newExtents = ScaleUtils.calculateBoundsFromScale(scaleDenominator, display.getDisplaySize(),
				display.getDPI(), getBounds());
