...
public class RenderJob extends Job {
     ...
     */
    private Queue<ReferencedEnvelope> requests = new LinkedBlockingQueue<>();
    ...
    protected void startRendering(Envelope bounds2, IProgressMonitor monitor) throws Throwable {
        ...
        if (bounds == null || bounds.contains(context2.getViewportModel().getBounds())) {
            bounds = null;
        }
    ...
    protected void finalizeLabelPainter(IRenderContext context2) {
        ...
        try {
            context2.getLabelPainter().endLayer(layerId, graphics,
    ...
    private void handleUnknownError(Throwable renderError) {
        ...
        // user
        executor.getContext().setStatusMessage(
    ...
    private synchronized ReferencedEnvelope combineRequests()
        ...
        try {
            ...
            for (ReferencedEnvelope env : requests) {
                ...
                // Vitalus: fix for deadlock in RenderJob because of
     ...
     * <p>
     * Please note that the envelope is referenced with a CoordinateReferenceSystem, the result will
     * be transformed to the viewport CRS (ie the world) in order to determine what part of the
     * screen needs refreshing.
     * <p>
     *
     * @param envelope ReferencedEvelope describing area to draw, or <code>null</code> for the
     *        entire screen.
     */
    public synchronized void addRequest(ReferencedEnvelope envelope) {
        if (envelope == null) {
            ...
            throw new IllegalArgumentException(
                    "You have asked us to draw a region of the screen without a CRS. Did you intend the viewport CRS?"); //$NON-NLS-1$
     ...
     * parallel fashion. <br/>
     * The GeoResource id is used as a discriminatory, a GeoResource is supposed to be local if its
     * id starts with "file:/"
     */
    private class RenderJobRule implements ISchedulingRule {
        ...
        private boolean isUsingLocalResources(Renderer renderer) {
            try {
                return renderer.getContext().getGeoResource().getIdentifier().toString()
