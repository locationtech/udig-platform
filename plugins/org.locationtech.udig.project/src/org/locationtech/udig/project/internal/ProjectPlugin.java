...
public final class ProjectPlugin extends EMFPlugin {
				...
				public void postShutdown(IProgressMonitor monitor, IWorkbench workbench) throws Exception {
					...
					for (Iterator<Resource> iter = resources.iterator(); iter.hasNext();) {
						...
						if (resource.getContents().isEmpty()) {
							ProjectPlugin.log("Not saving " + resource.getURI() + " empty contents"); //$NON-NLS-1$ //$NON-NLS-2$
							continue;
						}
	...
	public static void log(String message, Throwable e) {
		getPlugin().getLog().log(new Status(IStatus.INFO, ID, 0, message == null ? "" : message, e)); //$NON-NLS-1$
	}
