...
public class SelectionStyleContent extends StyleContent {
    ...
    public void save(IMemento momento, Object value) {
        ...
        momento.putString(fTypeName, style.featureTypeStyles().get(0).featureTypeNames().iterator()
                .next().getLocalPart());
    }
    ...
    private static Style createDefaultStyle(SimpleFeatureType schema) {
        ...
        if (isLine(geom)) {
            ...
            try {
                int size = queryPointSize(style);
                ssc.createGeometrySLD(getSelectionColor2(), getSelectionColor(),
                        schema.getGeometryDescriptor().getName().getLocalPart(), style, size);
            } catch (Exception e) {
                ProjectPlugin.log("", e); //$NON-NLS-1$
                ssc.getDefaultRule(style).symbolizers()
                        .add(ssc.createLineSymbolizer(getSelectionColor2(), false));
                ssc.getDefaultRule(style).symbolizers()
                        .add(ssc.createLineSymbolizer(getSelectionColor(), false));
            }
        }
    ...
    private void createGeometrySLD(Color colour, Color colour2, String geomXPath, Style style,
        ...
        for (Symbolizer s : lineSymbolizers)
            rule.symbolizers().add(s);
        getDefaultFeatureTypeStyle(style).rules().add(rule);
        ...
        rule.symbolizers().clear();
        for (Symbolizer s : polygonSymbolizers)
            rule.symbolizers().add(s);
        getDefaultFeatureTypeStyle(style).rules().add(rule);
    ...
    public FeatureTypeStyle getDefaultFeatureTypeStyle(Style style) {
        ...
        if (styles == null || styles.isEmpty() || styles.get(0).rules().isEmpty()) {
            ...
            FeatureTypeStyle ftStyle = styleBuilder.createFeatureTypeStyle("default", //$NON-NLS-1$
                    new Rule[] {});
