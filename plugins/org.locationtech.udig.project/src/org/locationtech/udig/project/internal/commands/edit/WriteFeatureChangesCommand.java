...
public class WriteFeatureChangesCommand extends AbstractEditCommand implements UndoableMapCommand {
    ...
    public WriteFeatureChangesCommand(IBlockingProvider<SimpleFeature> feature,
            IBlockingProvider<FeatureStore<SimpleFeatureType, SimpleFeature>> featureStore) {
        if (feature == null) {
    ...
    public void run(IProgressMonitor monitor) throws Exception {
        ...
        editFeature = featureProvider.get(subMonitor);
        subMonitor.done();
        store = storeProvider.get(subMonitor);
        ...
        Name[] names = featureType.getAttributeDescriptors().stream().map(e -> e.getName())
                .toArray(Name[]::new);
        ...
        try {
            if (reader.hasNext()) {
                ...
                added = true;
                store.addFeatures(new StaticFeatureCollection(Collections.singleton(editFeature),
                        featureType));
            }
        } finally {
            if (reader != null)
    ...
    public void rollback(IProgressMonitor monitor) throws Exception {
        if (noChange)
            return;
        monitor.beginTask(Messages.WriteFeatureChangesCommand_rollbackTask,
