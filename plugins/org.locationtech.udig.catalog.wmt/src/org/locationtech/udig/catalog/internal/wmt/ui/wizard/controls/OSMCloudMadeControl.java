...
public class OSMCloudMadeControl extends WMTWizardControl {
    ...
    private SelectionAdapter listSelectionListener = new SelectionAdapter() {
        ...
        /**
         * Is called when a style in one of the two groups is selected. Ensures that just one style
         * is selected in both groups.
         */
        @Override
        public void widgetSelected(SelectionEvent event) {
    ...
    protected Control buildControl(Composite infoBox) {
        ...
        previewImage.addMouseListener(new MouseListener() {
            ...
            public void mouseDown(MouseEvent arg0) {
                if (!previewStyleId
                        .equals(OSMCloudMadeStylesManager.CloudMadeStyle.EMPTY_STYLE_ID)) {
                    Program.launch(
                            "http://maps.cloudmade.com/?lat=51.508315&lng=-0.124712&zoom=14&styleId=" //$NON-NLS-1$
                                    + previewStyleId);
                }
            }
        });
        ...
        if (!styleGroupCloudMade.getStyles().isEmpty()) {
    ...
    private void updatePreviewMap(final String styleId) {
        ...
        if (image == null) {
            new Thread() {
                @Override
                public void run() {
                    String imageUrl = "http://tile.cloudmade.com/c8d1aeca771d57d6a0584fea7ce386f4/" //$NON-NLS-1$
                            + styleId + "/256/15/16372/10896.png"; //$NON-NLS-1$
                    ...
                    try {
                        ...
                        if (newImage == null || ((!newImage.isDisposed())
                                && (newImage.getBounds().width < 256))) {
                            newImage = imageCache
                                    .get(OSMCloudMadeStylesManager.CloudMadeStyle.EMPTY_STYLE_ID);
                        }
                    }
