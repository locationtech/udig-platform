...
public class BasicGridCoverageRendererTest {
    ...
    /**
     * Test method for looking at layer scales before rendering in {@link org.locationtech.udig.render.internal.gridcoverage.basic.GridCoverageReaderRenderer#.render(java.awt.Graphics2D graphics, org.eclipse.core.runtime.IProgressMonitor monitor)}.
     */
    @Test
    public void testLookAtLayerScalesBeforeRendering() throws Exception {
        ...
        CreateMapCommand command=new CreateMapCommand("BasicGridCoverageRendererTest", list, project); //$NON-NLS-1$
        ...
        layer.getStyleBlackboard().put(SLDPlugin.ID, sb.createStyle("BasicGridCoverageRendererTest", sb.createRasterSymbolizer(), 10, 100)); //$NON-NLS-1$
        
        // setup rendering and viewport
        createContextAndRenderManager(map);
        context.getRenderManagerInternal().setMapDisplay(new BasicGridCoverageTestViewportPane(1000, 1000));
        
        // test a scale within min and max
        map.getViewportModelInternal().setScale(50);
        // need a valid scale denominator
        assertTrue("could not setup the viewport model", map.getViewportModelInternal().getScaleDenominator() != -1); //$NON-NLS-1$
        // test if the reader is read
        triggerRendering(layer);
        assertTrue("did not read the GridCoverage, although inside min and max scales",reader.getReads() == 1); //$NON-NLS-1$
        ...
        triggerRendering(layer);
        assertTrue("read the GridCoverage, although out of min and max scales",reader.getReads() == 1); //$NON-NLS-1$
    ...
    private void triggerRendering(Layer layer) {
        ...
        while(context.getRenderManagerInternal().getRenderExecutor().getRenderer().getState() != IRenderer.DONE
                && context.getRenderManagerInternal().getRenderExecutor().getRenderer().getState() != IRenderer.NEVER)
            while(Display.getDefault().readAndDispatch());
        // refresh layer and wait for rendering to be done
        layer.refresh(null);
        while(context.getRenderManagerInternal().getRenderExecutor().getRenderer().getState() != IRenderer.RENDERING
                && context.getRenderManagerInternal().getRenderExecutor().getRenderer().getState() != IRenderer.STARTING && context.getRenderManagerInternal().getRenderExecutor().getRenderer().getState() != IRenderer.RENDER_REQUEST)
            while(Display.getDefault().readAndDispatch());
        while(context.getRenderManagerInternal().getRenderExecutor().getRenderer().getState() != IRenderer.DONE)
            while(Display.getDefault().readAndDispatch());
    }
