...
final class OffsetOrientationAnalyzer {
    ...
    public OffsetOrientationAnalyzer(List<Coordinate> inputList, GeometryFactory gf,
    ...
    public List<Geometry> discardClosed() {
        ...
        for (int i = 0; (i < n - 1); i++) {
            ...
            if (intersectionCoord == null) {
                ...
                // intersects.
                intersectionLastPosition = (intersectionLastPosition == n - 1) ? 0
                ...
                Coordinate imaginaryCoord = GeometryUtil
                        .intersection(firstOriginalSegment[0], firstOriginalSegment[1],
            ...
            for (int i = 0; i < ringCoords.length; i++) {
                ...
                for (int j = 0; j < indexList.length; j++) {
                    ...
                    if (data != null) {
                        ...
                        if (actualCoord.equals2D(inputList.get(indexKey))
                                && !(usedIndexes.contains(indexKey))) {
                            try {
    ...
    /**
     * Modify the ring, taking into account the direction(forward or not), set the index of the
     * coordinates that will be removed.
     * 
     * Also, calculate the "intersection coordinate", because from the pointList will recover one
     * coordinate, this calculated one and the retrieved one, must be very very similar. If that
     * doesn't happens, it throws an error.
     * 
     * Finally, when it is decided which coordinates will be deleted (secondIndex and thirdIndex),
     * they are deleted and in the middle of those, there is inserted the calculate coordinate or
     * the retrieved one.
     * 
     * @param i
     * @param j
     ...
     */
    private LinearRing improveTheRing(int i, int j, int n, Coordinate[] ringCoords,
            Boolean forward, int until) throws InvalidDistanceException {
        ...
        Coordinate intersection = GeometryUtil.intersection(firstLine.getCoordinateN(0),
                firstLine.getCoordinateN(1), secondLine.getCoordinateN(0),
                secondLine.getCoordinateN(1));
        ...
        result = modifyRing(coordinateToAdd, firstIndex, secondIndex, thirdIndex, fourthIndex,
                ringCoords, forward, untilDifference);

        return result;
    }

    private int calculateUntilDifference(boolean forward, int until, int j, int i, int n,
            int ringLength) {
    ...
    /**
     * First check the orientation of 3 coordinates, if it return false, check again but this time
     * using the quadrant method. If this also return false, store that index and mark as forward.
     * 
     * If the first check has failed, analyze the previous 2 segments seeking for intersections, if
     * there aren't check orientation of the actual segment but in reverse order, using the last 3
     * coordinates. If that return false, do the same but using the quadrant, and if this also
     * return false, store the index but mark as backward.
     * 
     * @param i
     * @param n
     * @param indexList
     ...
     */
    private DataDirectionUntil[] storeIndex(int i, int n, DataDirectionUntil[] indexList,
            LineString segment, Coordinate[] segCoord) {
    ...
    /**
     * From input coordinates, create a lineString. Then, with that lineString form polygons using
     * the polygonizer, and retrieve its shell that will be the linearRings needed.
     * 
    ...
    private List<LinearRing> getRings() {
        ...
        boolean isCCWinputList = CGAlgorithms.isCCW(inputList.toArray(new Coordinate[inputList
                .size()]));
        // create the rings.
        LineString inputLineString = gf.createLineString(inputList.toArray(new Coordinate[inputList
                .size()]));
    ...
    private LinearRing modifyRing(Coordinate coordinateToAdd, Integer firstIndex, int secondIndex,
            int thirdIndex, int fourthIndex, Coordinate[] ringCoords, Boolean forward,
            int untilDifference) {
    ...
    private boolean eliminateCoords(boolean forward, int firstIndex, int untilDifference, int i,
    ...
    private Coordinate getClosestPoint(Coordinate intersection,
            List<DataSegmentIntersection> pointsThatBelongs) throws InvalidDistanceException {
    ...
    /**
     * If the distance of the imaginary intersection with any of those segment is to little,
     * (intersects says false) we can assume they intersect.
     * 
     * Taking into account that, and also knowing how many intersection that segment has, the
     * function will work like that way:
     * 
     * <pre>
    ...
    private boolean belongsToSegment(Coordinate[] firstOriginalSegment,
            Coordinate[] secondOriginalSegment, Coordinate imaginaryCoord) {
     ...
     * 
     * Will check the orientation of 3 points from the input list and compare with the respective
     * orientation of that 3 points of the source list.
     * 
     * If the orientations are the same, will check the quadrant of those segment and return the
     * result.
     * 
     * @param i The position of the first point. Will check i, i+1 and i+2.
     * @param isClosedLine
    ...
    private boolean hasSameOrientation(int i, boolean isClosedLine) {
        ...
        int offsetOrientation = CGAlgorithms.orientationIndex(inputList.get(i),
                inputList.get(second), inputList.get(third));
        int originalOrientation = CGAlgorithms.orientationIndex(sourceList.get(i),
                sourceList.get(second), sourceList.get(third));
        ...
        if (result) {
            ...
            if (!secondCoordInput.equals2D(thirdCoordInput)
                    && !secondCoordSrc.equals2D(thirdCoordSrc)) {
    ...
    /**
     * Check the orientation of a segment (p0-p1) with a point (p2). But those segment will be the
     * end-segment of the list.
     * 
     * Will check the orientation of 3 points from the input list and compare with the respective
     * orientation of that 3 points of the source list.
     * 
     ...
     * result.
     * 
     * @param i Size of the list, so it will check n -1, n-2, and n-3.
     * @return True if the orientation matches.
     */
    ...
    /**
     * Giving a segment and an intersectionCoordinate, will create a second segment with the
     * provided index
     * 
     * @param segment
     ...
     */
    private boolean findIntersectionBackward(LineString segment, Coordinate firstCandidateCoord,
            int i, int n) {
    ...
    /**
     * Will find the intersection if exist. If exist, return the coordinate. If doesn't exist will
     * return null.
     * 
     * @param segment
     * @param segCoord
     ...
     */
    private Coordinate findIntersectionForward(final Geometry segment, final Coordinate[] segCoord,
            int j, int n) {
    ...
    public List<Geometry> discardNonClosed() {
        ...
        for (int i = start; i < n - 1;) {
            ...
            LineString firstSegment = intersectCoord == null ? createLineSegment(i, i + 1)
                    : createLineSegment(intersectCoord, i + 1);
    ...
    /**
     * Run the quadrant analyzer. If everything is OK, it will return true, false otherwise.
     * 
     * @param i
    ...
    private boolean checkQuadrantOpenLines(int i) {
        ...
        QuadrantAnalyzer qAnalyzer = new QuadrantAnalyzer(inputList, sourceList, i,
                intersectionLastPosition);
    ...
    /**
     * Analyze running the quadrantAnalyzer which will return true or false. If return false, some
     * coordinate is wrong; so it will retrieve which one is and set as badCoordinate.
     * 
     * @param i
     * @param size
     ...
     * @return
     */
    private boolean checkQuadrantClosedLines(int i, int size, Coordinate realIntersectionCoord,
            Coordinate imaginaryCoord) {

        boolean isOk = false;

        QuadrantAnalyzer qAnalyzer = new QuadrantAnalyzer(inputList, sourceList, i,
                intersectionLastPosition, size, realIntersectionCoord, imaginaryCoord);
        ...
        if (!isOk) {
            ...
            case SEGMENT_2_POINT_2:
                ...
                // intersectionLastPosition - 1;
                this.badCoordinate = intersectionLastPosition == 0 ? size - 1
                        : intersectionLastPosition - 1;
                break;
            default:
                break;
    ...
    /**
     * Function called when checking the orientation of the beginning. Will check if the first and
     * third segments intersects each other.
     * 
     * @return
     */
    private boolean hasBeginIntersections() {
        ...
        Coordinate interCoord = AlgorithmUtils.getIntersectionCoord(firstSegment, secondSegment,
                firstSegC);
    ...
    /**
     * Function called when checking the orientation of the ending. Will check if the last and third
     * to last segments intersects each other.
     * 
     * @return
     */
     ...
     * 
     * @param firstCoord The provided coordinate.
     * @param second The position respect the list of the second coordinate.
     ...
     * 
     * @param first The position respect the list of the first coordinate.
     * @param second The position respect the list of the second coordinate.
     * @return The desired lineString segment.
     */
    ...
    /**
     * Find the first intersection of the provided segment. Return the intersection coordinate if
     * exist. Also set the intersectionLastPosition.
     * 
     * @param firstSegment The segment which will be intersected.
    ...
    private Coordinate findIntersectionWithProvidedSegment(LineString firstSegment, int start, int n) {
        ...
        for (int h = start; h < n - 1; h++) {
            ...
            // null.
            candidateCoord = AlgorithmUtils.getIntersectionCoord(firstSegment, interSegment,
                    firstSegC);
            if (candidateCoord != null) {
                finalCoordinate = new Coordinate(candidateCoord);
                intersectionLastPosition = h;
