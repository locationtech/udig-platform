...
public class ParallelParametersComposite extends AbstractParametersComposite {
    ...
    protected void applyChanges() {
        ...
        try {
            ...
            InfoMessage message = new InfoMessage(Messages.PrecisionParallel_error_distance_NaN,
                    Type.ERROR);
        ...
        if (d == 0) {
            InfoMessage message = new InfoMessage(Messages.PrecisionParallel_error_distance_zero,
                    Type.ERROR);
            messageImage.setImage(message.getImage());
            messageText.setText(message.getText());
            parallelContext.setMode(PrecisionToolsMode.ERROR);
        ...
        Display.findDisplay(fatherThread).asyncExec(new Runnable() {
            ...
            public void run() {
                ...
                if (!labelReferenceLine.isDisposed()) {
                    labelReferenceLine.setText(Messages.PrecisionTool_reference_line
                            + ": " + referenceLine); //$NON-NLS-1$
    ...
    /**
     * Reflect changes made on the map in the view. Set the parameters, later other event will
     * updates the changes.
     */
    @Override
    public void update(Observable o, Object arg) {
        ...
        // coor)
        if (parallelContext.mode == PrecisionToolsMode.BUSY
                || parallelContext.mode == PrecisionToolsMode.EDITING) {
            return;
        }
