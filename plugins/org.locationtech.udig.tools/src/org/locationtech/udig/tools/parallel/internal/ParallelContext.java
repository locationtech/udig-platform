...
public class ParallelContext extends PrecisionToolsContext {
    ...
    /**
     * Start position of the parallel curve respect the reference line and its direction.
     */
    ...
    /**
     * Initializes its context. Sets the initial data to null. Set the parallel state waiting.
     * 
     */
    @Override
    ...
    /**
     * Set the reference feature. Also stores its coordinates on {@link #inputCoordinates}.ç
     * 
    ...
    private void removeVertexOfTheSameLine(Coordinate[] inputCoordinates) {
        ...
        this.inputCoordinates = initialCoordinates
                .toArray(new Coordinate[initialCoordinates.size()]);
    }

    /**
     * Calculate the tangents of the segment (i and i +1) and segment (i+1 and i+2) and if they are
     * the same return true, otherwise return false. If the number is nearly the same, the
    ...
    /**
     * Change the current position of the parallel preview respect the reference line. Position will
     * be upper or under.
     */
     ...
     * </p>
     * <p>
     * Given the offset position and the start position will calculate the offset curve taking on
     * account only the new position. Distance will be the same as last time.
     * </p>
     * 
    ...
    private void calculateParallelCurve(OffsetPosition offsetPosition2, int startPosition2)
        ...
        outputCoordinates = builder.getLineCurve(inputCoordinates, distance,
                ((Geometry) referenceFeature.getDefaultGeometry()).getFactory());
     ...
     * </p>
     * <p>
     * First calculate the distance between the initial point and the nearest segment. Second set
     * the offset position and start position respect the reference feature. At last, calculates the
     * offset curve and stores the given coordinates on {@link #outputCoordinates}.
     * </p>
     */
    ...
    /**
     * Set the new distance of the parallel curve and calculate the resultant parallel curve.
     * 
     * @param distance The new distance of the curve.
     */
    public void calculateParallelCurve(Double distance) throws IllegalArgumentException {
        ...
        outputCoordinates = builder.getLineCurve(inputCoordinates, this.distance,
                ((Geometry) referenceFeature.getDefaultGeometry()).getFactory());
        // OffsetBuilderPlus builder2 = new OffsetBuilderPlus((Geometry)
        // referenceFeature.getDefaultGeometry());
        // outputCoordinates = builder2.getLineCurve(distance);
    ...
    /**
     * <p>
     * Calculate the distance between the initial point and its closest line segment.
     * </p>
     * <p>
     * Calculate the distance of each segment of the reference line with the provided point. Once it
     * found the nearest segment, it compute the orientation of the provided point.
     * </p>
     * <p>
     ...
     * </p>
     * <p>
     * Calculate the start position respect the reference line segment and its direction.
     * <li>LEFT means that if you go from reference segment point 0 to point 1, the start point is
     * located at the left.</li>
     * <li>RIGHT means that if you go from reference segment point 0 to point 1, the start point is
     * located at the right.</li>
     * </p>
     * 
     * @return the distance
     */
    private double calculateDistanceAndCurrentPostion() {
        ...
        for (int i = 0; i < inputCoordinates.length - 1; i++) {
            ...
            distance = CGAlgorithms.distancePointLine(initialCoordinate, inputCoordinates[i],
        ...
        int segmentOrientation = CGAlgorithms.computeOrientation(closestSeg.p0, closestSeg.p1,
                initialCoordinate);
        // offset position respect the first segment and the initial point.
        // Useful when is only one segment.
        offsetPosition = (segmentOrientation == -1) ? OffsetPosition.POSITION_UNDER
                : OffsetPosition.POSITION_UPPER;
        ...
        if (length > 2) {
            ...
            if (segmentOrientation == 1) {
                refLineOrientation = CGAlgorithms.computeOrientation(inputCoordinates[0],
                        inputCoordinates[1], inputCoordinates[2]);
                ...
                offsetPosition = (outsideTurn) ? OffsetPosition.POSITION_UPPER
                        : OffsetPosition.POSITION_UNDER;
