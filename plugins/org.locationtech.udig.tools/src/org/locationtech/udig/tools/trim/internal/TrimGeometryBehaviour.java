...
public class TrimGeometryBehaviour implements Behaviour {
    ...
    public UndoableMapCommand getCommand(EditToolHandler handler) {
        ...
        try {
            ...
            throw (RuntimeException) new RuntimeException(
                    "Cannot reproject trim line from " + "layer's crs to map's crs").initCause(e); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        UndoableMapCommand trimCommand = new TrimFeaturesCommand(handler, selectedLayer,
                featuresToTrim, trimmingLineInLayerCrs);

        return trimCommand;
    }

    private FeatureCollection<SimpleFeatureType, SimpleFeature> getFeaturesToTrim(
            ILayer selectedLayer, LineString trimmingLine)
    ...
    private LineString getTrimmingLineInLayerCrs(EditToolHandler handler)
        ...
        final LineString line = GeometryCreationUtil.createGeom(LineString.class, currentShape,
                false);
        return line;
    }
    ...
    private Filter createTrimmingLineFilter(ILayer selectedLayer, LineString trimmingLineInLayerCrs)
        ...
        try {
            ...
            intersectsFilter = ff.intersects(ff.property(typeName),
                    ff.literal(trimmingLineInLayerCrs));
