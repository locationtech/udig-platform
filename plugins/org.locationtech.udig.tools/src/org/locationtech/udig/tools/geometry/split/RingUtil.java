...
final class RingUtil {
    ...
    public static List<LineString> ringToSegmentList(LinearRing ring) {
        ...
        for (int i = 0; i < ringCoords.length - 1; i++) {
            ...
            LineString segment = gf.createLineString(new Coordinate[] { ringCoords[i],
                    ringCoords[i + 1] });
    ...
    /**
     * Go through the rings, for each ring, calculate if there is an intersection with the provided
     * segment. If that's true, calculates the distance of this intersection respect the beginning
     * of the segment. Map each ring which it's closest distance.
     * 
     * @param rings
     * @param segment
     * @param lineSegment
     ...
     */
    public static Map<LinearRing, Double> setRingWithClosestIntersectionDistance(
            final List<LinearRing> rings, final Geometry segment, final Coordinate[] lineSegment) {
        ...
        for (LinearRing eachRing : rings) {
            ...
            if (numGeometries == 1) {
                ...
                distance = SplitUtil.calculateDistanceFromFirst(intersectionPoint, lineSegment[0],
                        lineSegment[1]);
                ...
                for (int j = 0; j < intersection.getNumGeometries(); j++) {
                    ...
                    distance = SplitUtil.calculateDistanceFromFirst(curPoint, lineSegment[0],
                            lineSegment[1]);
