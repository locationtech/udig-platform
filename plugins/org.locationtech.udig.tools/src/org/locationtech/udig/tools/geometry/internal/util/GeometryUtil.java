...
public class GeometryUtil {
     ...
     */
    public static Geometry geometryUnion(
     ...
     */
    public static Geometry[] extractGeometries(
            final FeatureCollection<SimpleFeatureType, SimpleFeature> featureCollection) {

        FeatureIterator<SimpleFeature> iter = featureCollection.features();
        try {
            SimpleFeature feature;
            ArrayList<Geometry> geometries = new ArrayList<>(featureCollection.size());
            int finalSize = 0;
            while (iter.hasNext()) {
     ...
     * <li>MultiLineString -> LineString. Succeeds if merging the parts result in a single
     * LineString, fails otherwise.
     * <li>MultiPolygon -> Polygon. Succeeds if merging the parts result in a single Polygon, fails
     * otherwise.
     * <li>* -> GeometryCollection
     * </ul>
     *
     * If the given type of geometry is the same as the desired one, returns that type of geometry.
     * </p>
     ...
     *         <code>adapTo</code>
     */
    public static Geometry adapt(final Geometry inputGeom,
            final Class<? extends Geometry> adaptTo) {
        ...
        if (MultiLineString.class.equals(adaptTo) || LineString.class.equals(adaptTo)) {
            ...
            ArrayList<LineString> lineList = new ArrayList<>(mergedLineStrings);
            ...
            if (MultiLineString.class.equals(adaptTo)) {
                ...
                if (lineStrings.length == 1) {
                    Geometry mergedResult = lineStrings[0];
                    return mergedResult;
        ...
        if (Polygon.class.equals(adaptTo) && (MultiPolygon.class.equals(geomClass))) {
            ...
            assert inputGeom
                    .getNumGeometries() == 1 : "the collection must have 1 element to adapt to Polygon"; //$NON-NLS-1$
    ...
    /**
     * Creates a geometry collection using the simple geometries contained in the gem List.
     *
     * @param simpleGeometries homogeneous list of zero or more geometries of the class: Points,
     *        LineStrings or Polygons
     * @param expectedClass expected result class
     *
    ...
    public static GeometryCollection adaptToGeomCollection(ArrayList<Geometry> simpleGeometries,
        ...
        if (MultiPolygon.class.equals(expectedClass)) {
            geomResult = geomFactory.createMultiPolygon(
                    simpleGeometries.toArray(new Polygon[simpleGeometries.size()]));
        } else if (MultiLineString.class.equals(expectedClass)) {
            geomResult = geomFactory.createMultiLineString(
                    simpleGeometries.toArray(new LineString[simpleGeometries.size()]));
        } else if (MultiPoint.class.equals(expectedClass)) {
            geomResult = geomFactory
                    .createMultiPoint(simpleGeometries.toArray(new Point[simpleGeometries.size()]));
     ...
     *
     * @param sourceGeomArray one or more geometries
     * @param requiredGeom
     *
     * @return new geometry of the required class
     */
    public static Geometry adapt(final ArrayList<Geometry> sourceGeomArray,
            final Class<? extends Geometry> requiredGeom) {

        assert sourceGeomArray
                .size() >= 1 : "one or more geometries are required in the source geometry collection"; //$NON-NLS-1$
     ...
     */
    public static Class<? extends GeometryCollection> getCompatibleCollection(
            final Class<? extends Geometry> simpleGeometry) {
    ...
    public static Geometry convertLineStringIntoPolygonGeometry(SimpleFeature feature) {
        ...
        List<Geometry> ringsList = new ArrayList<>();
    ...
    /**
     * This coordinates belong to a LineString geometry, it's coordinates aren't closed, close the
     * coordinate. The first coordinate must be equal to the last coordinate.
     *
     * @param lineCoor
     * @return
     */
     ...
     */
    public static Coordinate intersection(Coordinate p1, Coordinate p2, Coordinate q1,
            Coordinate q2) {
        inputLines[0][0] = p1;
        inputLines[0][1] = p2;
        inputLines[1][0] = q1;
    ...
    private static Coordinate intersectionWithNormalization(Coordinate p1, Coordinate p2,
     ...
     * @param normPt
     */
    private static void normalizeToEnvCentre(Coordinate n00, Coordinate n01, Coordinate n10,
            Coordinate n11, Coordinate normPt) {
     ...
     */
    private static Coordinate safeHCoordinateIntersection(Coordinate p1, Coordinate p2,
            Coordinate q1, Coordinate q2) {
     ...
     *
     * @param input The lineString to rotate.
    ...
    /**
     * Rotate the given lineString with the provided angle and a center point as a reference point
     * required for rotate.
     *
     * @param input The input lineString to rotate.
     * @param angle The angle in radiant.
     * @param center The center point as reference to rotate.
     * @return The rotated lineString.
     */
    public static LineString rotation(LineString input, double angle, Coordinate center) {
        ...
        List<Coordinate> rotationCoords = new LinkedList<>();
     ...
     *
     * @param input The polygon to rotate.
     ...
     * @param input The polygon to rotate
     * @param angle Angle in radiant.
     * @param pivot x,y coordinate reference needed to rotate.
    ...
    public static Polygon rotation(Polygon input, double angle, Coordinate pivot) {
        ...
        List<Coordinate> rotationShell = new LinkedList<>();
        ...
        List<LinearRing> holes = new LinkedList<>();

        for (int j = 0; j < input.getNumInteriorRing(); j++) {

            List<Coordinate> rotationHole = new LinkedList<>();
     ...
     *
     * @param actualCoord Coordinate to rotate.
     * @param center Reference coordinate needed to rotate.
     * @param angle Angle in radiant.
    ...
    private static Coordinate rotate(Coordinate actualCoord, Coordinate center, double angle) {
        ...
        rotateCoord.x = (Math.cos(angle) * (actualCoord.x - center.x))
                - (Math.sin(angle) * (actualCoord.y - center.y)) + center.x;
        rotateCoord.y = (Math.sin(angle) * (actualCoord.x - center.x))
                + (Math.cos(angle) * (actualCoord.y - center.y)) + center.y;
