...
public class GeometryUtil {
    ...
    public static Geometry adapt(final Geometry inputGeom, final Class<? extends Geometry> adaptTo) {
        ...
        if (MultiPolygon.class.equals(adaptTo)) {
             ...
             * (MultiPolygon.class.equals(adaptTo)) { return gf.createMultiPolygon(polygons); } if
             * (polygons.length == 1) { return polygons[0]; }
             */
        }
            ...
            if (MultiLineString.class.equals(adaptTo)) {
                ...
                if (lineStrings.length == 1) {
                    ...
                    final String msg = MessageFormat.format(
                            Messages.GeometryUtil_DonotKnowHowAdapt, geomClass.getSimpleName(),
    ...
    public static GeometryCollection adaptToGeomCollection(ArrayList<Geometry> simpleGeometries,
        ...
        if (MultiPolygon.class.equals(expectedClass)) {
            geomResult = geomFactory.createMultiPolygon(simpleGeometries
                    .toArray(new Polygon[simpleGeometries.size()]));
        } else if (MultiLineString.class.equals(expectedClass)) {
            geomResult = geomFactory.createMultiLineString(simpleGeometries
                    .toArray(new LineString[simpleGeometries.size()]));
        } else if (MultiPoint.class.equals(expectedClass)) {
            geomResult = geomFactory.createMultiPoint(simpleGeometries
                    .toArray(new Point[simpleGeometries.size()]));
