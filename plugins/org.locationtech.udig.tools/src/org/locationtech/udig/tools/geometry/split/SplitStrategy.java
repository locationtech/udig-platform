...
public class SplitStrategy {
    ...
    public SplitStrategy(final LineString splitLine) {
        ...
        this.strategies = new HashMap<Class<?>, SplitStrategy.SpecificSplitOp>(4);
    ...
    public List<Geometry> split(final Geometry geomToSplit) {
        ...
        UsefulSplitLineBuilder usefulSplitLineBuilder = UsefulSplitLineBuilder
                .newInstance(getSplitLine());
     ...
     * </pre>
     * 
     * @param geomToSplit is a LineString, MultilineString, Polygon or MultiPolygon
     * @param splitInMapCrs a LineString
    ...
    /**
     * Depending on the class of the Geometry to be split, found the correspondent class responsible
     * of doing the split operation.
     * 
     * @param geomToSplitClass Class of the geometry to be split.
    ...
    /**
     * Strategy object for split geometry, subclasses are targeted towards specific kinds of
     * geometry.
     * <p>
     * The GeometryCollectionSplitter will hold onto a LineString provided by the user, and use it
     * to break provided geometry one by one.
     */
    private static interface SpecificSplitOp {
        /**
         * LineString used for splitting; as provided by the user.
         * 
         * @param splitLine LineString used by the split method to break up geometry one by one.
         */
         ...
         * 
         * @param geomToSplit Geometry to split using the user supplied lineString
    ...
    private static class LineStringSplitter extends AbstractSplitter {
         ...
         * 
         * @param geomToSplit the {@link LineString} to be split.
     ...
     * <p>
     * The GeometryCollectionSplitter will hold onto the SpecificcSplitOp in order to hold onto the
     * LineString provided by the user for splitting.
     * 
     */
    private static abstract class AbstractGeometryCollectionSplitter implements SpecificSplitOp {
         ...
         * Constructor.
         * 
         * @param spliterOperation Specific split operation class.
         */
         ...
         * 
         * @param splitLine LineString used by split method to split provided geometry one by one
         */
        ...
        public boolean canSplit(Geometry geomToSplit) {
            ...
            if (!((geomToSplit instanceof MultiPolygon) || (geomToSplit instanceof MultiLineString))) {
                throw new IllegalArgumentException(
                        "MultiPolygon or MultiLineString geometry is expected"); //$NON-NLS-1$
            }
            final GeometryCollection coll = (GeometryCollection) geomToSplit;
            final int numParts = coll.getNumGeometries();
         ...
         * 
         * @param gf Provided geometry factory.
         * @param parts Split parts.
         * @return The geometry collection with the geometries after being split.
         */
     ...
     * Polygon Strategy:
     * <ul>
     * <li>1- Build a graph with all the edges and nodes from the intersection between the polygon
     * and the line
     * <li>2- Go through the graph, building the new polygons.
     * <li>2.1- Get a non-visited edge from the intersection with the line, and go through it.
     * <li>2.2- Get the next edges, if exist an intersection with other edges, take the one with
     * less angle, CW direction. If not, take the next. Add the edge.
     * <li>2.3- Do the same with the non-visited shell edges and holes. The next edges will be
     * calculated with the less angles in CCW direction.
     * <li>2.4- Analysis. Once all the rings are created, check that all intersection edges (those
     * edges belong to 2 features) are visited twice.
     * </ul>
     * 
     */
    private static class PolygonSplitter extends AbstractSplitter {
         ...
         * 
         * @param geomToSplit Polygon geometry to split.
         * 
         ...
         * 
         * @param aPolygon The source polygon to split.
         * @return Return a single polygon or multiPolygon depending on how the split went.
         */
        ...
        /**
         * Get the non split holes. The interior rings are calculated while finding rings, and also
         * while construction the graph.
         * 
         * @param graph The split graph.
        ...
        /**
         * Go through the graph and find the created rings, those rings will be the new features.
         * Start finding a ring from an intersection edge if it isn't visited. Then go through the
         * shell edges. At this point, the edges that form the new polygons are created, go through
         * the hole edges that weren't visited, those holes will be interior rings that aren't
         * modified. At the end, all the shell edges will be covered.
         * 
         * @param directedEdgeList List with all the edges from the graph that are forward.
         * @return A list containing rings, those rings will be the new pre-polygons.
         */
        ...
        private void storeHolesRings(List<DirectedEdge> directedEdgeList) {
            ...
            for (DirectedEdge de : directedEdgeList) {
                ...
                if (!edge.isVisited() && edge.isHoleEdge()) {
                    ...
                    LinkedHashSet<SplitEdge> interiorRings = builtRing(startEdge,
         ...
         */
        private List<LinkedHashSet<SplitEdge>> findShellRings(List<DirectedEdge> directedEdgeList,
            ...
            for (DirectedEdge de : directedEdgeList) {
                ...
                if (!edge.isVisited() && edge.isShellEdge()) {
                    edgeList.add(builtRing(startEdge, CGAlgorithms.COUNTERCLOCKWISE));
                }
            }
            return edgeList;
        }
        ...
        private List<LinkedHashSet<SplitEdge>> findIntersectionRings(
                List<DirectedEdge> directedEdgeList, List<LinkedHashSet<SplitEdge>> edgeList) {
            ...
            for (DirectedEdge de : directedEdgeList) {
                ...
                if (!edge.isVisited() && edge.isIntersectionEdge()) {
                    ...
                    // get the ring.
                    LinkedHashSet<SplitEdge> result = builtRing(startEdge, CGAlgorithms.CLOCKWISE);
        ...
        /**
         * Check if the given edge is unique edge, that means that there isn't an intersection edge
         * with the same coordinates.
         * 
         * @param edge Edge to find.
         * @param directedEdgeList List where to find the edge.
        ...
        private boolean isOnlyHoleEdge(SplitEdge edge, List<DirectedEdge> directedEdgeList) {
            ...
            for (DirectedEdge direct : directedEdgeList) {
                ...
                if (possibleEdge.isIntersectionEdge()) {
                    if (sameCoordinates(edge.getCoordinates(), possibleEdge.getCoordinates())) {
                        return false;
                    }
                }
            }
        ...
        /**
         * Check if the given edge is unique edge, that means that there isn't a hole edge with the
         * same coordinates.
         * 
         * @param edge Edge to find.
         * @param directedEdgeList List where to find the edge.
         * @return True if only exist this intersection edge, any hole edge has the same
         *         coordinates.
         */
        private boolean isOnlyIntersectionEdge(SplitEdge edge, List<DirectedEdge> directedEdgeList) {
            ...
            for (DirectedEdge direct : directedEdgeList) {
                ...
                if (possibleHole.isHoleEdge()) {
                    if (sameCoordinates(edge.getCoordinates(), possibleHole.getCoordinates())) {
                        return false;
                    }
                }
            }
        ...
        /**
         * Assure that all the intersection rings have been taken into account twice. Each edge
         * belong to 2 features, so it must be visited twice. If it was not visited twice, visit it
         * again.
         * 
         * @param directedEdgeList List with all the edges from the graph that are forward.
         * @return A list containing rings, those rings will be the new polygons.
         */
            ...
            for (DirectedEdge de : directedEdgeList) {
                ...
                if (!edge.isTwiceVisited() && edge.isIntersectionEdge()) {
                    ...
                    if (!intersectionAndHoleSame(directedEdgeList)) {
                        ...
                        int direction = testDirection(startEdge, CGAlgorithms.CLOCKWISE);
        ...
        /**
         * Seek out if there exist an intersection edge that is exactly as a hole edge.
         * 
         * @param directedEdgeList The list with the edges.
            ...
            for (DirectedEdge de : directedEdgeList) {
                ...
                if (edge.isIntersectionEdge()) {
                    ...
                    for (DirectedEdge direct : directedEdgeList) {
                        ...
                        if (possibleHoleOrShell.isHoleEdge() || possibleHoleOrShell.isShellEdge()) {
        ...
        /**
         * check the coordinates are the same, that means, if the coordinate 1 equals to the
         * coordinate 2 from the hole, then, the coordinate 2 must equal the coordinate 1 from the
         * hole.
         * 
         * @param intersection Array containing coordinates from the intersection.
         * @param hole Array containing coordinates from the hole.
         * @return True if a coordinate from the intersection or the hole exists on the other side
         *         (hole/intersection).
         */
        private boolean sameCoordinates(Coordinate[] intersection, Coordinate[] hole) {
            ...
            if (intersection[0].equals(hole[0]) && intersection[1].equals(hole[1])) {
                return true;
            }
            if (intersection[1].equals(hole[0]) && intersection[0].equals(hole[1])) {
                return true;
            }
            if (intersection[0].equals(hole[1]) && intersection[1].equals(hole[0])) {
                return true;
            }
            if (intersection[1].equals(hole[1]) && intersection[0].equals(hole[0])) {
                return true;
            }
        ...
        /**
         * Get the next edges, if exists an intersection with other edges, take the one with less
         * angle in the given direction. If not, take the next. Add the edge.
         * 
         * @param startEdge The directedEdge from which start.
         * @param direction CCW or CW direction.
         * @return A list with a ring of edges.
         */
        private LinkedHashSet<SplitEdge> builtRing(final DirectedEdge startEdge, final int direction) {
            ...
            while (!edgesAreEqual((SplitEdge) startEdge.getEdge(), nextDirectedEdge)) {
        ...
        /**
         * This function will test if the given direction will form new rings, or will form repeated
         * rings. It depends on the line direction, so first test with CW direction, if the new ring
         * isn't repeated, thats ok.
         * 
         * @param startEdge The directedEdge from which start.
         * @param direction CCW or CW direction.
         * @return The correct direction to form the new ring.
         */
        private int testDirection(final DirectedEdge startEdge, final int direction) {
         ...
         * 
         * @param startEdge Start directedEdge
         * @param nextDirectedEdge The next one that could be the same as the start.
         ...
         * 
         * @param allRings The ring of the polygon/s.
         * @param nonSplitHoles Non-split holes.
         * @return A list with all the built polygons.
         */
        private List<Geometry> buildSimplePolygons(List<LinkedHashSet<SplitEdge>> allRings,
            ...
            for (LinkedHashSet<SplitEdge> edgeList : allRings) {
                ...
                for (LinearRing holeRing : nonSplitHoles) {
                    ...
                    if (holeRing.within(poly)) {
                        ...
                        Geometry hole = this.geometryFactory.createPolygon(holeRing, null);
         ...
         * 
         * @param edgeList A ring of edges.
         * @param gf The geometry factory.
        ...
        private Polygon buildPolygon(LinkedHashSet<SplitEdge> edgeList) {
            ...
            Polygon poly;
            if (holeList.isEmpty()) {
                poly = this.geometryFactory.createPolygon(ring[0], (LinearRing[]) null);
            } else {
        ...
        /**
         * Check the coordinates seeking for self-intersection, that means, seek for repeated
         * coordinates.
         * 
         * @param shellCoords Shell coordinates off the polygon.
        ...
        /**
         * Go through the shell coordinates seeking for self-intersections and extract them. Then
         * those extracted rings will be inserted as holes of the polygon.
         * 
         * @param shellCoords Shell coordinates off the polygon.
         * @return 2 linearRing, one will be the shell and the other a hole if exists.
         */
        private LinearRing[] extractInteriorRing(Coordinate[] shellCoords) {
            ...
            for (int i = 1; i < shellCoords.length && selfIntersectionStart == -1; i++) {
        ...
        /**
         * From those interior rings (holes), build linearRings. Those rings will be the new split
         * features, or an existent ring that has not been modified.
         * 
         * @param edgeList List of splitEdges.
        ...
        private List<LinearRing> buildLinearRing(LinkedHashSet<SplitEdge> edgeList) {
            ...
            do {
                ...
                } else if (result[1] != null && hasSelfIntersection(result[1].getCoordinates())) {
         ...
         * </p>
         * 
         * @param polygon Polygon that will suffer the split operation.
