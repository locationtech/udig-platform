...
/**
 * Performs the trimming of a LineString or MultiLineString using a provided LineString as cutting
 * edge.
 * 
...
public class TrimGeometryStrategy {
    ...
    public Geometry trim(final Geometry original) {
        ...
        if (original instanceof LineString) {
            ...
            throw new IllegalArgumentException(
                    Messages.TrimGeometryStrategy_defined_for_line_geometries);
        }
        return trimmed;
    }
     ...
     * @param original
     * @param trimmingLine
     * @return if <code>original</code> intersects <code>trimmingLine</code> at a single point, the
     *         result of cutting off the part of <code>original</code> that lies at the right of
     *         <code>trimmingLine</code>. If they do not intersect, returns <code>original</code>.
    ...
    private LineString trimLine(final LineString original, final LineString trimmingLine)
        ...
        // point itself
        final Coordinate lineTo = intersectionPoint.equals2D(lineFrom) ? trimmingLine
                .getCoordinateN(1) : intersectionPoint;
        ...
        if (difference instanceof MultiLineString) {
            ...
            throw new IllegalStateException(Messages.TrimGeometryStrategy_difference_unknown_type
                    + difference);
        }
        ...
        final int firstLineOrientation = CGAlgorithms.computeOrientation(lineFrom, lineTo,
                firstLinePoint);
     ...
     * <code>Coordinate</code> right before the <code>intersectionPoint</code> on the segment that
     * containst it.
     * 
     * @param line
     * @param pointInLine
    ...
    private Coordinate getCoordinateBeforePoint(final LineString line, final Coordinate pointInLine) {
        ...
        final String msg = MessageFormat.format(Messages.TrimGeometryStrategy_point_not_on_line,
                pointInLine, line);
