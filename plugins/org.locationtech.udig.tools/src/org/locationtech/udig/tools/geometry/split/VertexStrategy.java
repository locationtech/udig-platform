...
public final class VertexStrategy {
     ...
     * </p>
     * 
     * @param geomToAddVertex The geometry that will be modified.
     * @param line The split line.
     * @param geomNeighbor The neighbor geometry list, those one had been modified by the split
     *        operation.
     * @return The geometry with the vertex added.
     */
    public static Geometry addIntersectionVertex(Geometry geomToAddVertex, Geometry line,
            List<Geometry> geomNeighbor) {
        ...
        assert geomToAddVertex instanceof Polygon || geomToAddVertex instanceof LineString
                || geomToAddVertex instanceof MultiPolygon
                || geomToAddVertex instanceof MultiLineString : "Input geometry has unexpected class."; //$NON-NLS-1$
        ...
        } else if (geomToAddVertex instanceof LineString
                || geomToAddVertex instanceof MultiLineString) {
     ...
     * </p>
     * 
     * @param polygonToAddVertex The polygon geometry.
     * @param line The split line.
     * @param geomNeighborList The list with neighbor geometries.
    ...
    private static Geometry addIntersectionVertexToPolygon(Geometry polygonToAddVertex,
            Geometry line, List<Geometry> geomNeighborList) {
        ...
        for (Geometry neighbor : geomNeighborList) {
            ...
            if (boundary.touches(neighbor)) {
                ...
                boundaryCoordinates = shellCoordinates.toArray(new Coordinate[shellCoordinates
                        .size()]);
     ...
     * </p>
     * 
     * @param lineToAddVertex The line to add vertex.
     * @param line The split line.
     * @return the geomToAddVertex with the intersection vertex added.
     */
    private static Geometry addIntersectionVertexToLine(Geometry lineToAddVertex, Geometry line) {
        ...
        Coordinate[] shellClosed = shellCoordinates
                .toArray(new Coordinate[shellCoordinates.size()]);
        shellClosed = GeometryUtil.closeGeometry(shellClosed);
    ...
    /**
     * For each hole, get it and if it touches with the neighbor geometry add the intersection
     * vertex if exist to the hole boundary. Then add this hole to the holeRing list.
     * 
     * @param holes Array of holes.
     * @param line The split line.
     * @param fc Geometry factory.
     * @param geomNeighborList The list with neighbor geometries.
     * @return A list containing the modified holes.
     */
    private static List<LinearRing> addIntersectionToHoles(Geometry[] holes, Geometry line,
            GeometryFactory fc, List<Geometry> geomNeighborList) {
        ...
        for (int i = 0; i < holes.length; i++) {
            ...
            for (Geometry neighbor : geomNeighborList) {
                ...
                if (eachHole.touches(neighbor)) {
                    ...
                    boundaryCoordinates = holeCoordinates.toArray(new Coordinate[holeCoordinates
                            .size()]);
    ...
    /**
     * Go through boundaryCoordinates and add a coordinate were the split line intersect with the
     * boundaryCoordinates.
     * 
     * @param boundaryCoordinates The boundary of a geometry, could be the exterior or interior
     *        boundary.
     * @param line The split line.
     * @param fc The geometry factory.
     * @param neighbor The neighbor. The new point will touch this geometry.
     * @return An array list with the coordinates.
     */
    private static List<Coordinate> addIntersection(Coordinate[] boundaryCoordinates,
            Geometry line, GeometryFactory fc, Geometry neighbor) {
    ...
    /**
     * Will return the coordinates sorted, that relation depends on the lineString direction and the
     * coordinates. Calculate which coordinate is the nearest to the beginning of the segment until
     * the array is sorted.
     * 
     * @param point
     * @param segment
     * @return An array of points, all of them sorted respect the segment orientation.
     */
    ...
    /**
     * Add a vertex to the coordinate list if the segment intersect with the neighbor and if the
     * segment with the neighbor intersection has a dimension bigger than 0.
     * 
     * @param segment The analyzed segment.
     * @param neighbor The neighbor boundary.
     * @param point The intersection point between the segment and the boundary.
     * @param resultCoordinates The list with the geomToAddVertex coordinates.
     * @return The list with geomToAddVertex coordinates with/without the added vertex.
     */
    private static List<Coordinate> addPoint(Geometry segment, Geometry neighbor, Geometry point,
    ...
    /**
     * If the geometry has holes, will return and array of geometries with all the holes. If it
     * hasn't holes will return an empty array.
     * 
     * 
     * @param geomToAddVertex The geometry to analyze
