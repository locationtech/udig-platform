 ...
 * </p>
 * <p>
 * For those Feature geometries that were split, the original Feature will be deleted and as many
 * new Features as geometries resulted of the split operation will be created, maintaining the
 * original Feature's attributes other than the default geometry.
 * </p>
 *
...
final class SplitFeaturesCommand extends AbstractCommand implements UndoableMapCommand {
     ...
     * layer with the line present in the handler's {@link EditToolHandler#getCurrentShape() current
     * shape}.
     *
    ...
    public SplitFeaturesCommand(EditToolHandler handler) {
        ...
        Class<?> geometryBinding = selectedLayer.getSchema().getGeometryDescriptor().getType()
                .getBinding();
     ...
     * {@link UndoableComposite} with the list of commands needed to remove the split features and
     * create the new ones, then delegates the execution to that {@link UndoableComposite} which is
    ...
    public void run(IProgressMonitor monitor) throws Exception {
        ...
        FeatureCollection<SimpleFeatureType, SimpleFeature> featuresToSplit = getFeaturesToSplit(
                splitter);
        prepMonitor.worked(2);

        final List<UndoableMapCommand> undoableCommands = new ArrayList<>();
    ...
    private LineString getSplittingLineInMapCRS(final EditToolHandler handler)
        ...
        final LineString lineInLayerCrs = GeometryCreationUtil.createGeom(LineString.class,
                currentShape, false);
     ...
     * @throws SplitFeaturesCommandException
     */
    private LineString reprojectSplitLine(final LineString lineInLayerCrs,
            final CoordinateReferenceSystem targetCrs) throws SplitFeaturesCommandException {

        try {

            CoordinateReferenceSystem splitLineCrs = (CoordinateReferenceSystem) lineInLayerCrs
                    .getUserData();
            ...
            LineString splitlineInMapCRS = (LineString) GeoToolsUtils.reproject(lineInLayerCrs,
                    splitLineCrs, targetCrs);
            ...
            throw new SplitFeaturesCommandException(
                    Messages.SplitFeaturesCommand_cannot_transform_the_splitline_crs);
        }
    }
     ...
     * <p>
     * To get the features, <code>splittingLine</code> is transformed to the layer's CRS and an
     ...
     * @throws SplitFeaturesCommandException
     */
    private FeatureCollection<SimpleFeatureType, SimpleFeature> getFeaturesToSplit(
            final LineString splittingLine) throws SplitFeaturesCommandException {

        try {
            Filter extraFilter = createSplittingLineFilter(splittingLine);
            FeatureCollection<SimpleFeatureType, SimpleFeature> selection = LayerUtil
                    .getSelectedFeatures(selectedLayer, extraFilter);
            return selection;
            ...
            throw new SplitFeaturesCommandException(
                    Messages.SplitFeaturesCommand_cannot_retrieve_features_to_be_split);
        }
    }

    /**
     * Creates the filter based on the layer filter and the split line. It would be an intersect
     * filter between the split line and the layer, taking into account the own features that are
     * selected in the layer.
     *
     * @param splittingLine The split line.
     ...
     * </p>
     *
     * @return A command list will all the operations that the split tool will do.
     * @throws SplitFeaturesCommandException
     * @throws IOException
     */
    private List<UndoableMapCommand> buildCommandList(
            FeatureCollection<SimpleFeatureType, SimpleFeature> featuresToSplit,
            LineString splitterInMapCrs) throws SplitFeaturesCommandException {
        ...
        try {
            ...
            SplitFeatureBuilder builder = SplitFeatureBuilder.newInstance(originalFeatureList,
                    splitterInMapCrs, mapCRS);
            try {
                builder.buildSplit();
            ...
            final List<UndoableMapCommand> undoableCommands = new LinkedList<>();
            ...
            for (SimpleFeature feature : featuresThatSufferedSplit) {
                ...
                ProjectPlugin.log(className + " - Delete original feature: " //$NON-NLS-1$
                        + ((Geometry) feature.getDefaultGeometry()).toText());
            }
            ...
            for (SimpleFeature feature : splitResult) {
                ...
                undoableCommands.add(command);
                ProjectPlugin.log(className + " - Split result: " //$NON-NLS-1$
                        + ((Geometry) feature.getDefaultGeometry()).toText());
            }
            ...
            for (SimpleFeature neighbor : modifiedNeighbour) {
                ...
                for (SimpleFeature original : originalFeatureList) {
                    if (original.getID().equals(neighbor.getID())) {
                        UndoableMapCommand command = new ModifyGeometryFeatureCommand(
                                original.getID(), (Geometry) neighbor.getDefaultGeometry(),
                                (Geometry) original.getDefaultGeometry(), this.selectedLayer);
                        undoableCommands.add(command);

                        ProjectPlugin.log(className + " - Neighbour result: " //$NON-NLS-1$
                                + ((Geometry) neighbor.getDefaultGeometry()).toText());
                    }
                }
            }
            ...
            throw new SplitFeaturesCommandException(
                    Messages.SplitFeaturesCommand_split_transaction_failed);
     ...
     * @throws SplitFeaturesCommandException
     */
    private List<SimpleFeature> asList(final FeatureIterator<SimpleFeature> featureIterator)
            throws SplitFeaturesCommandException {
        ...
        while (featureIterator.hasNext()) {
            ...
            ProjectPlugin.log(className + "- Original feature: " + defaultGeometry.toText()); //$NON-NLS-1$

            if (!defaultGeometry.isValid()) {
                ProjectPlugin.log(className + "- Original feature has invalid geometry: " //$NON-NLS-1$
