    ...
    private List<UndoableMapCommand> buildCommandList(
        ...
        try {
            ...
            for (SimpleFeature feature : featuresThatSufferedSplit) {
                ...
                ProjectPlugin
                        .log(className
                                + " - Delete original feature: " + ((Geometry) feature.getDefaultGeometry()).toText()); //$NON-NLS-1$
            }
            ...
            for (SimpleFeature feature : splitResult) {
                ...
                ProjectPlugin.log(className
                        + " - Split result: " + ((Geometry) feature.getDefaultGeometry()).toText()); //$NON-NLS-1$
            }
            ...
            for (SimpleFeature neighbor : modifiedNeighbour) {
                ...
                for (SimpleFeature original : originalFeatureList) {
                    if (original.getID().equals(neighbor.getID())) {
                        ...
                        ProjectPlugin
                                .log(className
                                        + " - Neighbour result: " + ((Geometry) neighbor.getDefaultGeometry()).toText()); //$NON-NLS-1$
                    }
                }
            }
