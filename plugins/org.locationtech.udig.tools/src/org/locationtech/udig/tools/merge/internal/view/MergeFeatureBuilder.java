...
/**
 * A builder for a {@link Feature} that allows to select which attributes from a set of source
 * features the new Feature is going to be composed of.
 * <p>
...
class MergeFeatureBuilder {
    ...
    private List<SimpleFeature> sourceFeatures = Collections
            .synchronizedList(new LinkedList<SimpleFeature>());

    /** the attributes of the merge feature (in other words the feature to build) */
    private Object[] mergedFeature;

    /**
     * Geometry if it have a null value the build executes the union operation in the source
     * features
     */
    ...
    /**
     * Creates a MergeFeatureBuilder that works over the provided features and their geometries.
     * 
    ...
    public MergeFeatureBuilder(ILayer layer) {
        ...
        if (geometryDescriptor == null) {
            throw new IllegalStateException(
                    "The layer schema does not contain a geometry descriptor"); //$NON-NLS-1$
        }
     ...
     * 
     * @param feature
     * @return return the position of the added feature. -1 will be returned because the feature had
     *         added previously.
     */
    public int addSourceFeature(SimpleFeature feature) {
        ...
        if (this.sourceFeatures.isEmpty()) {
            // The porperty values of the first feature are used as default values for the merge
            // feature
    ...
    /**
     * Uses the feature's values to set the merge feature. The geometry value is not set by this
     * method.
     * 
     ...
     * 
     * @return a new {@link Feature} of the same {@link FeatureType} than the provided source
     *         features, built with the attribute values established through this builders methods
     * @throws IllegalStateException if it is not possible to create the Feature with the current
    ...
    /**
     * Checks if any geometry was set in the merge feature. In other case the union of source
     * geometries will be exectured.
     * 
    ...
    /**
     * @param attIndex the index of an attribute in the builder's {@link FeatureType}
    ...
    /**
     * Copies the value of the attribute at index {@code attritbuteIndex} in the merged feature to
     * be the value of the {@code attritbuteIndex} attribute in the {@code featureIndex} source
     * feature, and fires a change event to be caught by the registered {@link ChangeListener}
     * 
     ...
     */
    public void copyAttributeToMerge(int srcFeatureIndex, int attributeIndex)
            throws IllegalArgumentException {
     ...
     * a change event to be caught by the registered {@link ChangeListener}
     * <p>
     * If {@code attributeIndex == #getDefaultGeometryIndex()}, sets the geometry value of the
     * merged feature to {@link #getPrittyMergeGeometry()}
     * </p>
     * 
    ...
    /**
     * @return the number of attributes defined in the shared {@link FeatureType} for the source
     ...
     * <code>value</code> and fires a change event to be caught by the registered
     * {@link ChangeListener}
     * 
    ...
    /**
     * Used by MergeView to remove all features before adding new collection (while in Operation
     * Mode)
     */
    ...
    /**
     * Checks if the feature's geometry fulfill the conditions to be added in the list of features
     * to merge.
     * 
     * <lu> <li>should be compatible with the layer geometry.</li> <li>Multipolygon,
     * MultiLineString, MultiPoint can be merge always. <li>Polygon, LineString, Point can be merge
     * if and only if they intersect.<li>
     * </lu>
     * 
     * @param newFeature
    ...
    public boolean canMerge(SimpleFeature newFeature) {
        ...
        Class<? extends Object> geomClass = (defaultGeometry != null) ? defaultGeometry.getClass()
                : Object.class;

        Class<? extends Geometry> layerGemetryClass = (Class<? extends Geometry>) layer.getSchema()
        ...
        assert (defaultGeometry instanceof Polygon) || (defaultGeometry instanceof LineString)
                || (defaultGeometry instanceof Point);
    ...
    /**
     * Builds the merge geometry. If any geometry was set in the merge feature, this method will
     * build a new one execturing the union of source geometries.
     * 
    ...
    private Geometry buildMergeGeometry() {
        ...
        if (geom != null) {
            ...
            final Class<?> expectedGeometryType = type.getGeometryDescriptor().getType()
                    .getBinding();
