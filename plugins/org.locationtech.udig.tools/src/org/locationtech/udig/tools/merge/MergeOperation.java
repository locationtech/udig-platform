 ...
 * <p>
 * This class provides a starting point for opening the MergeView in the MERGEMODE_OPERATION status.
 * This field is stored in MergeContext that acts as a blackboard, and is retrieved throughout the
 * whole plug-in whenever a difference in tool behaviour has been introduced to support the
 * operation-mode workflow (against the 'classic' MergeTool one)
 * </p>
 * 
        ...
        Thread t = new Thread() {
            ...
            public void run() {
                try {
                    ...
                    ILayer preSelectedLayer = ApplicationGIS.getActiveMap().getEditManager()
                            .getSelectedLayer();
                    Filter preSelectedFilter = preSelectedLayer.getFilter();
                    if (preSelectedFilter != Filter.EXCLUDE) {
                        List<SimpleFeature> preSelectedFeatures = Util.retrieveFeatures(
                                preSelectedFilter, preSelectedLayer);
                        mergeContextSingleton.addPreselectedFeatures(preSelectedFeatures,
                                preSelectedLayer);
                    }
