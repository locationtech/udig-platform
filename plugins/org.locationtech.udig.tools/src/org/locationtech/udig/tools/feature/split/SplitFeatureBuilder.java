...
/**
 * <p>
 * This class is responsible of doing the split operation and adding the vertex to the neighbour.
 * </p>
 * <p>
 * <b>USAGE:</b>
 * <p>
 * 
 * <pre>
 * SplitFeatureBuilder builder = SplitFeatureBuilder.newInstance(featureList, splitLine, mapCRS);
 ...
 * </p>
 * <p>
 * Additionally, you could get the list of original features which have suffered split, calling
 * {@link #getFeaturesThatSufferedSplit()}.
 * </p>
 * <p>
 * <b>Note:</b> to get a correct result you should call first the method {@link #buildSplit()} and
 * then {@link #buildNeighbours()}.
 * </p>
 * </p>
 * 
...
public final class SplitFeatureBuilder {
    ...
    /**
     * Create a new instance of the builder giving the split line, the feature list and the CRS
     * which the operation will be based on.
     * 
     ...
     */
    public static SplitFeatureBuilder newInstance(final List<SimpleFeature> featureList,
            final LineString splitLine, final CoordinateReferenceSystem desiredCRS)
        ...
        if (desiredCRS == null)
            throw new IllegalArgumentException("The desired crs is required, i.e: map crs"); //$NON-NLS-1$
        ...
        try {
            LineString splitReprojected = (LineString) GeoToolsUtils.reproject(splitLine,
                    (CoordinateReferenceSystem) splitLine.getUserData(), desiredCRS);
     ...
     * @param splitLine line used as reference to split the feature
     * @param desiredCRS the crs in which the split operation will be executed
     */
     ...
     */
    public SplitFeatureBuilder buildSplit() throws SplitFeatureBuilderFailException,
            ...
            for (SimpleFeature feature : this.featureList) {
                ...
                if (canSplit(geomToSplit)) {
                    ...
                    this.splitResultList
                            .addAll(createSplitFeatures(splitGeometriesResult, feature));
                }
            }
            if (!existSplit) {
     ...
     */
    private List<SimpleFeature> createSplitFeatures(final List<Geometry> splitGeometries,
            final SimpleFeature feature) throws OperationNotFoundException, TransformException {
    ...
    /**
     * Retrieve the result of the split operation. Use this function after calling
     * {@link #buildSplit()}.
     * 
     ...
     * 
     * @param geomToSplit Geometry that will suffer split operation.
        ...
        try {
            for (SimpleFeature feature : featureList) {
                ...
                if (!canSplit(geom) && (requireVertex(geom))) {
                    ...
                    geomWithAddedVertex = GeoToolsUtils.reproject(geomWithAddedVertex, desiredCRS,
                            featureCRS);
    ...
    /**
     * Retrieve the neighbor features. Use this after calling {@link #buildNeighbours()}.
     * 
     * @return a SimpleFeature list or the empty list
     */
    ...
    /**
     * <p>
     * Responsible of creating a new geometry that will be the given geometry with an extra vertex.
     * That vertex will be the point where the given geometry and the line intersect.
     * </p>
     * 
     * @param geomToAddVertex neighbor of a geometry that has suffered split operation
     * @return geomToAddVertex with one or more added vertex.
     */
    private Geometry addVertexToNeighbour(final Geometry geomToAddVertex) {

        return VertexStrategy.addIntersectionVertex(geomToAddVertex, this.splitLine,
                originalGeometryList);
    }
