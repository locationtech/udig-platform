...
public final class FeatureUtil {
    ...
    /**
     * Copies the property values of source in the target if that is possible. The copy is possible
     * only if the names and the class are equals.
     *
     * @param source
     * @param target
     * @return
     ...
     *
     */
    public static SimpleFeature copyAttributes(final SimpleFeature source,
            final SimpleFeature target) throws IllegalAttributeException {
     ...
     * @param source
     * @param target
     * @param attributeNameToOmit attribute's name to omit. if it is null does not omit any
     *        attribute
     * @return the target feature populated with source data
     *
     * @throws IllegalAttributeException
     */
    public static SimpleFeature copyAttributesOmitting(final SimpleFeature source,
            SimpleFeature target, final List<String> attributeNameToOmit)
            throws IllegalAttributeException {

        assert source != null : "source cannot be null"; //$NON-NLS-1$
        assert target != null : "target cannot be null"; //$NON-NLS-1$

        for (AttributeDescriptor targetAttrDescriptor : target.getFeatureType()
     ...
     * @throws IllegalAttributeException
     */
    public static SimpleFeature copyAttributesInPropertyList(final SimpleFeature source,
            SimpleFeature target, final List<String> propertiesList)
            throws IllegalAttributeException {
        ...
        for (String property : propertiesList) {
            ...
            assert source
                    .getAttribute(property) != null : "propery should be in the source feature"; //$NON-NLS-1$
    ...
    /**
     * Copies the attribute value if it is compatible with the target value. They are compatible if
     * they are equals name and type
     *
     * @param attrName
     * @param source
     * @param target
     ...
     */
    private static SimpleFeature copyAttributeValue(final String attrName,
            final SimpleFeature source, SimpleFeature target) {
    ...
    /**
     * Returns the a feature builder for a type with the default geometry attribute. The default
    ...
    /**
     * Returns the a feature builder for a type with the attributes present in the prototype. The
     ...
     */
    public static SimpleFeatureTypeBuilder createDefaultFeatureType(
            final SimpleFeatureType prototype) {
     ...
     *
     * @param prototype
     * @param typeName name of new FeatureType
     * @return FeatureTypeBuilder
     */
    public static SimpleFeatureTypeBuilder createDefaultFeatureType(
            final SimpleFeatureType prototype, final String typeName) {
        ...
        for (int i = 0; i < attributes.size(); i++) {
            ...
            if (att == defaultGeometry) {
                if (att.getType().getBinding() != MultiPolygon.class
                        && att.getType().getBinding() != Polygon.class) {
     ...
     */
    public static SimpleFeatureTypeBuilder createDefaultFeatureType(final String typeName,
            final CoordinateReferenceSystem crs, final Class<? extends Geometry> targetClass) {
     ...
     *
     * @param prototype feature type used as model
     * @param typeName name of the new feature type
     * @param crs CRS of geometry attribute
     * @return FeatureType a new feature type TODO refactor in Prototype Factory class
     */
    public static SimpleFeatureType createFeatureType(final SimpleFeatureType prototype,
            final String typeName, final CoordinateReferenceSystem crs) throws SchemaException {

        assert prototype != null : "prototype can be null"; //$NON-NLS-1$
        GeometryDescriptor geomAttrType = prototype.getGeometryDescriptor();

        assert geomAttrType != null : "the Geometry Class can be null"; //$NON-NLS-1$
        Class<? extends Geometry> geomClass = (Class<? extends Geometry>) geomAttrType.getType()
                .getBinding();
    ...
    /**
     * Returns the a feature type with the attributes type present in the prototype. The new feature
     * type will be the name CRS and geometry class specified
     *
     * @param prototype feature type used as model
     * @param typeName name of the new feature type
     * @param crs CRS of geometry attribute
     ...
     */
    public static SimpleFeatureType createFeatureType(final SimpleFeatureType prototype,
            final String typeName, final CoordinateReferenceSystem crs,
            final Class<? extends Geometry> geometryClass) throws SchemaException {
        ...
        builder.setDefaultGeometry(
                build.buildDescriptor(defaultGeomAttr.getLocalName(), type).getLocalName());
    ...
    /**
     * Creates a new feature type using the geometry (named "geometry" by default) class and the
     * collections of attributes types.
     *
     * @param name name of the new feature type.
     * @param crs
     * @param geomClass
    ...
    public static SimpleFeatureType createFeatureType(final String name,
            final CoordinateReferenceSystem crs, final Class<? extends Geometry> geomClass,
            final List<AttributeDescriptor> attrTypeCollection) throws SchemaException {
    ...
    /**
     * Creates a new Feature for <code>targetType</code> that holds the common attributes from
     * <code>sourceFeature</code> and the new geometry.
     *
     * @param sourceFeature the original Feature from which to extract matching attributes for the
     *        new Feature
     * @param targetType
     * @param newGeometry
     * @return a new Feature of type <code>targetType</code> holding the common attributes with
    ...
    public static SimpleFeature createFeatureUsing(final SimpleFeature sourceFeature,
            final SimpleFeatureType targetType, final Geometry newGeometry)
            throws IllegalAttributeException {
        ...
        try {
            ...
            List<String> omitAttr = new ArrayList<>(1);
            omitAttr.add(geoAttName);

            newFeature = FeatureUtil.copyAttributesOmitting(sourceFeature, newFeature, omitAttr);

            final Class<? extends Geometry> geomClass = (Class<? extends Geometry>) targetGeometryType
                    .getType().getBinding();
    ...
    public static SimpleFeature copyAttributesCheater(final SimpleFeature source,
            final SimpleFeature target, final List<String> attributeNameToOmit)
            throws IllegalAttributeException {

        assert source != null : "illegal argument: source cannot be null"; //$NON-NLS-1$
        assert target != null : "illegal argument: target cannot be null"; //$NON-NLS-1$

        Map<String, Class<?>> sourceTypes = new HashMap<>();
    ...
    /**
     * Create a new feature of Feature Type with the provided geometry. The geometry will be adapted
     * to geometry class of feature type.
     *
     * @param type
     * @param geometry
     * @return a new feature
     */
    public static SimpleFeature createFeatureWithGeometry(final SimpleFeatureType type,
            Geometry geometry) {
     ...
     * @return the count of features in the collection or Integer.MAX_VALUE if the feature
     *         collection has more than Integer.MAX_VALUE features
     */
    public static int computeCollectionSize(
     ...
     * @param dissolveProperty property name required
     * @return true if the the feature type has a property with the specified name
     */
    public static boolean hasProperty(final SimpleFeatureType featureType,
     ...
     * @return a value equal or greater than 0, it will be -1 if the feature type have not got the
     *         property
     *
     */
    public static int getPositionProperty(final SimpleFeatureType featureType,
            final String propertyName) {

        assert featureType != null : "featureType cannot be null"; //$NON-NLS-1$
        assert propertyName != null : "propertyName != cannot be null"; //$NON-NLS-1$
     ...
     */
    public static List<Integer> getPositionProperty(final SimpleFeatureType featureType,
            final List<String> propertyNames) {

        assert featureType != null : "featureType cannot be null"; //$NON-NLS-1$
        assert propertyNames != null : "propertyName != cannot be null"; //$NON-NLS-1$
    ...
    public static boolean isNewFeature(SimpleFeature feature) {
        ...
        if (feature.getID().startsWith("new")) { //$NON-NLS-1$
            isNew = true;
        }
        return isNew;
    }
    ...
    public static SimpleFeatureType addAttributeToFeatureType(SimpleFeatureType prototype,
            String attributeName, Class<?> clazz) throws SchemaException {
     ...
     * @return
     */
    public static List<SimpleFeature> getFeatures(
            final FeatureCollection<SimpleFeatureType, SimpleFeature> featureCollection) {

        FeatureIterator<SimpleFeature> iter = null;
        List<SimpleFeature> featureList = new ArrayList<>(featureCollection.size());
        try {
