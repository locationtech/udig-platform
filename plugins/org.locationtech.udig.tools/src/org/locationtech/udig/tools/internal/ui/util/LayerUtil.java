...
public final class LayerUtil {
    ...
    /**
     * Creates a Query for the selected features of <code>layer</code> or all the features if it has
     * no selection set and <code>fallbackToWholeLayer</code> is <code>true</code>.
     * 
     * @param layer
     ...
     * 
     * @param layer an <code>ILayer</code> that can resolve to <code>FeatureSource</code> for which
     *        to compute the number of selected features.
     ...
     * @throws IOException
     */
    public static int getCountOfSelectedFeatures(final ILayer layer, final Filter filter)
            throws IOException {
     ...
     */
    public static FeatureCollection<SimpleFeatureType, SimpleFeature> getSelectedFeatures(
     ...
     * @throws IOException
     */
    public static FeatureCollection<SimpleFeatureType, SimpleFeature> findAllFeatures(
            final ILayer layer) throws IOException {
     ...
     * 
     * @param layer the layer to obtain the selected features from
     * @param fallbackToWholeLayer Whether to return the whole layer's features if no selection is
     *        set for the layer
     ...
     */
    public static FeatureCollection<SimpleFeatureType, SimpleFeature> getSelectedFeatures(
            final ILayer layer, final Filter extraFilter) throws IOException {
    ...
    /**
     * Traverses the <code>map</code>'s layers ensuring it does not already contains a layer named
     * <code>name</code> and returns a non existent layer name, either by returning
     ...
     * @return <code>name</code> if it don't already exists in the layer's map nor in its DataStore.
     *         A non existent type name in the data store with <code>name</code> as prefix
     *         otherwise.
     * @throws IOException
     */
    public static String findNewLayerName(final ILayer existingLayer, final String name)
            throws IOException {
        ...
        if (resource != null) {
            FeatureSource<SimpleFeatureType, SimpleFeature> source = resource.resolve(
     ...
     * <p>
     * If <code>sourceLayer.getCRS()</code> is {@link ILayer#UNKNOWN_CRS}, it means the underlying
     * FeatureSource has no CRS defined, thus, for the sake of performing spatial operations, the
     * Map's CRS is returned instead.
     * </p>
     * 
     * @param sourceLayer
    ...
    public static Class<? extends Geometry> getGeometryClass(final ILayer layer) {
        ...
        Class<? extends Geometry> geomClass = (Class<? extends Geometry>) type.getType()
                .getBinding();
    ...
    public static void presentSelection(ILayer sourceLayer, Filter filter) {
        ...
        MapCommand createSelectCommand = SelectionCommandFactory.getInstance().createSelectCommand(
                sourceLayer, filter);
        sourceLayer.getMap().sendCommandSync(createSelectCommand);
    }

    /**
     * Check if the given layer is compatible with the any of the geometries from the geometry list.
     * 
