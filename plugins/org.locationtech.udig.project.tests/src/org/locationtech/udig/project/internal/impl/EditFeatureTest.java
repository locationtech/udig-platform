...
public class EditFeatureTest extends AbstractProjectTestCase {
    ...
    public void testSetFeatureAttribute() throws Exception {
        FeatureStore<SimpleFeatureType, SimpleFeature> store = map.getLayersInternal().get(0)
                .getResource(FeatureStore.class, null);
        FilterFactory fac = CommonFactoryFinder.getFilterFactory(GeoTools.getDefaultHints());
        FeatureCollection<SimpleFeatureType, SimpleFeature> collection = store
                .getFeatures(fac.id(FeatureUtils.stringToId(fac, features[1].getID())));
        FeatureIterator<SimpleFeature> iter = collection.features();
        assertEquals(ORIGINAL_VALUE, iter.next().getAttribute(1));
        iter.close();
        store.modifyFeatures(store.getSchema().getDescriptor(1).getName(), MODIFIED_VALUE,
    ...
    public void testRollback() throws Exception {
        ...
        FeatureCollection<SimpleFeatureType, SimpleFeature> collection = store
                .getFeatures(fac.id(FeatureUtils.stringToId(fac, id)));
        FeatureIterator<SimpleFeature> iter = collection.features();
        assertEquals(ORIGINAL_VALUE, iter.next().getAttribute(1));
        iter.close();
        store.modifyFeatures(store.getSchema().getDescriptor(1).getName(), MODIFIED_VALUE,
                fac.id(FeatureUtils.stringToId(fac, id)));
