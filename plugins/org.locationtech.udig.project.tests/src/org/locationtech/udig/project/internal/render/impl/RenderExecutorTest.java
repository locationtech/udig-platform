...
public class RenderExecutorTest extends AbstractProjectTestCase {
    ...
    public void setUp() throws Exception {
        ...
        LinearRing ring1 = factory
                .createLinearRing(new Coordinate[] { new Coordinate(0, 0), new Coordinate(512, 0),
                        new Coordinate(512, 256), new Coordinate(0, 256), new Coordinate(0, 0),

                });

        Polygon poly1 = factory.createPolygon(ring1, new LinearRing[] {});

        LinearRing ring2 = factory.createLinearRing(
                new Coordinate[] { new Coordinate(0, 256), new Coordinate(512, 256),
                        new Coordinate(512, 512), new Coordinate(0, 512), new Coordinate(0, 256),
     ...
     * @param notSame -1 if all pixels should be non-transparent. Of the minimum y-value of the
     *        non-transparent pixels.
     */
    private void testImage(BufferedImage image, boolean clear, int minx, int miny, int maxx,
            int maxy) {
        for (int x = 0; x < 512; x++) {
            for (int y = 0; y < 512; y++) {
    ...
    public void testRenderEnvelope() throws Exception {
        ...
        assertEquals(image, image2);
        RenderExecutor executor = ((CompositeRendererImpl) ex.getRenderer()).getRenderExecutors()
                .iterator().next();
