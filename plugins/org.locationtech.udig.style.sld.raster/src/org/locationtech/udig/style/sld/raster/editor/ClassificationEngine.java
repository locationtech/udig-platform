...
public class ClassificationEngine {
    ...
    /**
     * Prompts the user to ensure they want to continue when using a large sample size;
     *
        ...
        Display.getDefault().syncExec(new Runnable() {
            ...
            public void run() {
                if (!MessageDialog.openConfirm(Display.getDefault().getActiveShell(),
                        ...
                        MessageFormat.format(Messages.ClassifyDialog_RasterCellWaring,
                                new Object[] { WARN_VALUE }))) {
                    ret[0] = false;
                }
            }
        });
    ...
    /**
     * Computes the minimum and maximum for the only band in a single band raster
     *
     * @param valuesToIgnore values to ignore
     * @param layer raster layer
     * @param sampleSize maximum sample size
     ...
     * @throws Exception
     */
    public double[] computeMinMax(double[] valuesToIgnore, GridCoverageReader layer,
            Long sampleSize) throws Exception {
        final Statistic[] stats = new Statistic[] { Statistic.MIN, Statistic.MAX };
        this.errorMessage = null;
        List<Range<Double>> ignore = new ArrayList<>();
        if (valuesToIgnore != null) {
            for (Double no : valuesToIgnore) {
                ignore.add(new Range<>(no));
            }
        }
        ...
        if (sampleSize != null) {
            ...
            GridGeometry2D world = new GridGeometry2D(gridRange,
                    new ReferencedEnvelope(gcRaw.getEnvelope()));
            DefaultParameterDescriptor<GridGeometry> gridGeometryDescriptor = new DefaultParameterDescriptor<>(
                    AbstractGridFormat.READ_GRIDGEOMETRY2D.getName().toString(), GridGeometry.class,
                    null, world);

            ParameterGroup readParams = new ParameterGroup(
                    new DefaultParameterDescriptorGroup("Test", //$NON-NLS-1$
     ...
     * @throws Exception
     */
    public List<Double> computeQuantile(int numBins, double[] valuesToIgnore,
            GridCoverageReader layer, Long sampleSize, IProgressMonitor monitor) throws Exception {
        this.errorMessage = null;
        GridCoverage gcRaw = layer.read(null);
        if (sampleSize != null) {
            int rSize = (int) Math.ceil(Math.sqrt(sampleSize.doubleValue()));
        ...
        int recSize = 1000;
        HashSet<Double> ignoreset = new HashSet<>();
        if (valuesToIgnore != null) {
        ...
        List<Double> data = new ArrayList<>(width * height);
        ...
        int binPop = Double.valueOf(Math.ceil((double) data.size() / numBins)).intValue();
        ...
        ArrayList<Double> results = new ArrayList<>(numBins + 1);
        for (int i = 0; i < numBins; i++) {
            results.add(min[i]);
        }
     ...
     * @param monitor
     * @return list of unique values found in the raster or null if cancelled
     * @throws IOException
     * @throws IllegalArgumentException
     */
    public Set<Double> computeUniqueValues(GridCoverageReader layer, Long sampleSize,
            IProgressMonitor monitor) throws IllegalArgumentException, IOException {
        this.errorMessage = null;
        GridCoverage gcRaw = layer.read(null);
        if (sampleSize != null) {
            int rSize = (int) Math.ceil(Math.sqrt(sampleSize.doubleValue()));
                ...
                for (int i = 0; i < df.getSize(); i++) {
                    ...
                    if (results.size() >= SingleBandEditorPage.MAX_ENTRIES) {
                        errorMessage = MessageFormat.format(
                                Messages.UniqueValuesDialog_MaxValueError,
                                SingleBandEditorPage.MAX_ENTRIES);
                        maxReached = true;
                        break;
                    }
                }
