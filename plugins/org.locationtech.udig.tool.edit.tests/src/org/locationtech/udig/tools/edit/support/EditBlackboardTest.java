...
public class EditBlackboardTest {
    ...
    public void testSetup() throws Exception {
        ...
        geom = factory.createMultiPoint(
                new Coordinate[] { new Coordinate(10, 5), new Coordinate(20, 10) });
        map = new EditBlackboard(SCREEN.x, SCREEN.y, transform, layerToWorld);
        map.getListeners().add(l);
        ...
        geom = factory.createMultiPolygon(
    ...
    private Polygon createPolygon(GeometryFactory factory, int i) {
        return factory.createPolygon(createShellRing(factory, i),
                new LinearRing[] { createHoleRing(factory, i) });
    }

    private LinearRing createHoleRing(GeometryFactory factory, int offset) {
        return factory.createLinearRing(new Coordinate[] { new Coordinate(offset + 5, 7),
                new Coordinate(offset + 8, 7), new Coordinate(offset + 8, 8),
                new Coordinate(offset + 5, 8), new Coordinate(offset + 5, 7) });
    }

    private LinearRing createShellRing(GeometryFactory factory, int offset) {
        return factory.createLinearRing(new Coordinate[] { new Coordinate(offset + 0, 5),
                new Coordinate(offset + 10, 5), new Coordinate(offset + 10, 10),
                new Coordinate(offset + 0, 10), new Coordinate(offset + 0, 5) });
    }
     ...
     */
    private void assertPixMapState(EditBlackboard map, int numGeoms, int numShellPoints,
            int numHoles, int numHolesPoints) {
    ...
    public void testGetCandidate() throws Exception {
        ...
        assertTrue(candidates.get(0).getGeom() == mapping.get(ring3)
                || candidates.get(1).getGeom() == mapping.get(ring3));
        assertTrue(candidates.get(0).getGeom() == mapping.get(ring2)
                || candidates.get(1).getGeom() == mapping.get(ring2));
    ...
    /*
     * Test method for 'org.locationtech.udig.tools.edit.support.PixelCoordMap.addCoord(int, int,
     * Coordinate)'
     */
    @Test
    public void testAddCoord() {
        ...
        assertTrue(added.get(0).getGeom() == mapping.get(ring3)
                || added.get(1).getGeom() == mapping.get(ring3));
        assertTrue(added.get(0).getGeom() == mapping.get(ring2)
    ...
    /*
     * Test method for 'org.locationtech.udig.tools.edit.support.PixelCoordMap.moveCoords(int, int,
     * int, int)'
     */
    @Test
    public void testMoveCoords() throws Exception, Exception {
        ...
        MathTransform albersToWGS84 = CRS.findMathTransform(CRS.decode("EPSG:3005"), //$NON-NLS-1$
                DefaultGeographicCRS.WGS84, true);
        map = new EditBlackboard(SCREEN.x, SCREEN.y, AffineTransform.getTranslateInstance(0, 0),
                albersToWGS84);
    ...
    /*
     * Test method for 'org.locationtech.udig.tools.edit.support.PixelCoordMap.deleteCoords(int,
     * int)'
     */
    @Test
    ...
    public void testTransform() throws Exception {
        MathTransform t = CRS.findMathTransform(CRS.decode("EPSG:3005"), //$NON-NLS-1$
    ...
    public void testLakeDrawingCase() throws Exception {
        ...
        EditBlackboard map = new EditBlackboard(SCREEN.x, SCREEN.y,
                ScaleUtils.worldToScreenTransform(bounds, new Dimension(100, 100)), layerToWorld);
        ...
        for (int j = 0; j < holes.size(); j++) {
            ...
            for (int i = 0; i < hole.getNumCoords(); i++) {
                assertEquals("hole=" + j + "i=" + i, poly.getInteriorRingN(j).getCoordinateN(i), //$NON-NLS-1$ //$NON-NLS-2$
                        hole.getCoord(i));
            }
    ...
    public void testSetTransform() throws Exception {
        ...
        map = new EditBlackboard(SCREEN.x, SCREEN.y, AffineTransform.getScaleInstance(.5, .5),
                layerToWorld);
        map.getListeners().add(l);
        mapping = map.setGeometries(poly, null);
    ...
    private void assertTranslation(EditBlackboard map, EditGeom geom, int diffX, int diffY) {
        ...
        // check hole
        assertEquals(map.getCoords(25 + diffX, 12 + diffY).get(0),
                geom.getHoles().get(0).getCoord(0));
        assertEquals(map.getCoords(28 + diffX, 12 + diffY).get(0),
                geom.getHoles().get(0).getCoord(1));
        assertEquals(map.getCoords(28 + diffX, 13 + diffY).get(0),
                geom.getHoles().get(0).getCoord(2));
        assertEquals(map.getCoords(25 + diffX, 13 + diffY).get(0),
                geom.getHoles().get(0).getCoord(3));
        assertEquals(map.getCoords(25 + diffX, 12 + diffY).get(0),
                geom.getHoles().get(0).getCoord(4));
    ...
    public void testRemoveGeoms() throws Exception {
        EventListener l = new EventListener();
        EditBlackboard map = new EditBlackboard(SCREEN.x, SCREEN.y, new AffineTransform(),
                layerToWorld);
        map.getListeners().add(l);
        ...
        List<EditGeom> toRemove = new ArrayList<>(map.getGeoms());
        toRemove.remove(0);
        toRemove.remove(0);
        ...
        // this is specific to this case. I know that all geometries in toRemove were in the
        // blackboard
    ...
    /*
     * Test method for 'org.locationtech.udig.tools.edit.support.EditUtils.overVertex(Point,
     * PixelCoordMap, int)'
     */
    @Test
    public void testOverVertex() {
        EditBlackboard map = new EditBlackboard(SCREEN.x, SCREEN.y,
                AffineTransform.getTranslateInstance(0, 0), layerToWorld);
        ...
        for (int x = -snapSize; x < snapSize; x++)
            for (int y = -snapSize; y < snapSize; y++)
                assertEquals("x=" + (10 - x) + ", y=" + (10 - y), valueOf, //$NON-NLS-1$ //$NON-NLS-2$
                        map.overVertex(Point.valueOf(10 - x, 10 - y), snapSize));
    ...
    class EventListener implements EditBlackboardListener {
        ...
        List<EditBlackboardEvent> allEvents = new ArrayList<>();
