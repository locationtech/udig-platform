...
public class DifferenceFeatureCommandTest {
    ...
    public void setUp() throws Exception {
        ...
        LineString line = fac.createLineString(new Coordinate[] { new Coordinate(0, 10),
                new Coordinate(10, 10), new Coordinate(20, 10) });
        Polygon poly = fac
                .createPolygon(
                        fac.createLinearRing(new Coordinate[] { new Coordinate(20, 20),
                                new Coordinate(40, 20), new Coordinate(40, 40),
                                new Coordinate(20, 40), new Coordinate(20, 20) }),
                        new LinearRing[0]);

        handler = new TestHandler();
        features = UDIGTestUtil.createTestFeatures("DifferenceFeatureTests", //$NON-NLS-1$
                new Geometry[] { line, poly }, new String[] { "line", "poly" }); //$NON-NLS-1$//$NON-NLS-2$
        map = MapTests.createNonDynamicMapAndRenderer(
                CatalogTests.createGeoResource(features, true), new Dimension(500, 500));

        Envelope env = map.getBounds(null);
        map.getRenderManagerInternal().setMapDisplay(
                new TestViewportPane(new Dimension((int) env.getWidth(), (int) env.getHeight())));
    ...
    /**
     * Test method for
     * 'org.locationtech.udig.tools.edit.commands.SplitFeatureCommand.run(IProgressMonitor)'
     */
    @Test
    public void testDifferencePolygonOnce() throws Exception {
        ...
        FeatureSource<SimpleFeatureType, SimpleFeature> resource = map.getEditManager()
                .getSelectedLayer().getResource(FeatureSource.class, nullProgressMonitor);
        ...
        while (iter.hasNext()) {
            ...
            if (feature.getID().equals("new0")) { //$NON-NLS-1$
                List<Coordinate> coords = Arrays
                        .asList(((Geometry) feature.getDefaultGeometry()).getCoordinates());
    ...
    public void testDiffOnMultipleGeoms() throws Exception {
        ...
        GeometryFactory fac = new GeometryFactory();
        LinearRing ring = fac
                .createLinearRing(new Coordinate[] { new Coordinate(0, 50), new Coordinate(50, 50),
                        new Coordinate(50, 55), new Coordinate(0, 55), new Coordinate(0, 50), });
        Polygon polygon = fac.createPolygon(ring, new LinearRing[0]);
        FeatureStore<SimpleFeatureType, SimpleFeature> store = map.getMapLayers().get(0)
                .getResource(FeatureStore.class, new NullProgressMonitor());
        FilterFactory filterFactory = CommonFactoryFinder
                .getFilterFactory(GeoTools.getDefaultHints());
        store.modifyFeatures(features[0].getFeatureType().getGeometryDescriptor().getName(),
                polygon,
    ...
    public void testMultiGeometry() throws Exception {
        handler.resetEditBlackboard();
        GeometryFactory fac = new GeometryFactory();
        LinearRing ring1 = fac
                .createLinearRing(new Coordinate[] { new Coordinate(0, 0), new Coordinate(50, 0),
                        new Coordinate(50, 10), new Coordinate(0, 10), new Coordinate(0, 0), });
        Polygon polygon1 = fac.createPolygon(ring1, new LinearRing[0]);

        LinearRing ring2 = fac
                .createLinearRing(new Coordinate[] { new Coordinate(0, 50), new Coordinate(50, 50),
                        new Coordinate(50, 60), new Coordinate(0, 60), new Coordinate(0, 50), });
        ...
        store.removeFeatures(
                filterFactory.id(FeatureUtils.stringToId(filterFactory, features[1].getID())));
