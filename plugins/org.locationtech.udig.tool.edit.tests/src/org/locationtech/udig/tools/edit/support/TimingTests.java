...
public class TimingTests {
    ...
    static {
        try {
            layerToWorld = CRS.findMathTransform(DefaultGeographicCRS.WGS84,
    ...
    public void testPrimitiveShapeIterator() throws Exception {
        ...
        EditBlackboard map = new EditBlackboard(500, 500,
                ScaleUtils.worldToScreenTransform(bounds, new Dimension(1000, 1000)), layerToWorld);

        PrimitiveShape shell = map.setGeometries((Geometry) feature.getDefaultGeometry(), null)
                .values().iterator().next().getShell();
        ...
        //
        // System.out.println("Time to iterate through 10000 points 1000 times is: " + (end-start)
        // +" msec"); //$NON-NLS-1$ //$NON-NLS-2$
        //
    }
}
