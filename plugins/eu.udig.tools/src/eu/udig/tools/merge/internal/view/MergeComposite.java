...
class MergeComposite extends Composite {
	...
	private void createCompositeMergeFeature() {
		...
		TableColumn tableColumnName = new TableColumn(tableMergeFeature,
				SWT.NONE);
	...
	private void deleteSourceFeatures() {
		...
		for (int i = 0; i < items.length; i++) {
			...
			List<SimpleFeature> sourceFeatures = this.mergeBuilder
					.getSourceFeatures();
	...
	private void createImageRegistry() {
		...
		registry.put(
				"trash", ImageDescriptor.createFromFile(MergeComposite.class, imgFile)); //$NON-NLS-1$
	}
...
//	}
		...
		if (sourceFeatures.size() == 0) {
			...
			for (int attIndex = 0; attIndex < attributeCount; attIndex++) {
				...
				if(attIndex == this.mergeBuilder.getDefaultGeometryIndex() ){
					...
					strValue = (mergeFeatureProperty == null) 
											? NULL_LABEL
	...
	private void canDelete() {
		...
		this.trashButton
				.setEnabled(!this.mergeBuilder.getSourceFeatures().isEmpty());
	}
	...
	private void setSelectedFeature(final int featureIndex,
	 ...
	 */
	private void selectAttribute(final int featureIndex,
			final int attributeIndex, final boolean checked) {
	 ...
	 */
	private void selectAttributePropagate(final int featureIndex,
			final int attributeIndex, final boolean checked) {
	...
	private void populateSourceFeaturesView() {
		...
		for (int featureIndex = 0; featureIndex < featureCount; featureIndex++) {
			...
			for (int attIndex = 0; attIndex < mergeBuilder.getAttributeCount(); attIndex++) {
				...
				String strValue = attrValue == null ? NULL_LABEL : String
						.valueOf(attrValue);
				attrItem.setText(VALUE_COLUMN, strValue);
