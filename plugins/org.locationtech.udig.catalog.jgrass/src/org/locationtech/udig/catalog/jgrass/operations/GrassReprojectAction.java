 ...
 */
public class GrassReprojectAction implements IObjectActionDelegate, IWorkbenchWindowActionDelegate,
    ...
    private List<JGrassMapGeoResource> maps = new ArrayList<>();
    ...
    public void run(IAction action) {
        ...
        Display.getDefault().syncExec(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    Dialog dialog = new Dialog(shell) {
                        ...
                        protected Control createDialogArea(Composite maxparent) {
                            ...
                            crsGroup.setLayoutData(new GridData(GridData.FILL_BOTH
                                    | GridData.GRAB_HORIZONTAL | GridData.GRAB_VERTICAL));
                            crsGroup.setText(
                                    "choose the coordinate reference system for the new location");
                    ...
                    ExceptionDetailsDialog.openError("ERROR", message, IStatus.ERROR,
        ...
        IRunnableWithProgress operation = new IRunnableWithProgress() {
            ...
            public void run(IProgressMonitor monitor)
                ...
                if (locationName == null || mapsetName == null || crs == null || maps.isEmpty()) {
                ...
                for (int i = 0; i < mapNum; i++) {
                    ...
                    try {
                        /*
                         * TODO get envelope from original region and reproject it. then adapt it to
                         * the output resolution and finally reproject the map on the new grid.
                         */
                        JGrassRegion sourceRegion = jgMEnv.getActiveRegion();
                        CoordinateReferenceSystem sourceCrs = jgMEnv.getCoordinateReferenceSystem();
                        org.locationtech.jts.geom.Envelope sourceEnvelope = sourceRegion
                                .getEnvelope();
                        MathTransform tr = CRS.findMathTransform(sourceCrs, crs);
                        org.locationtech.jts.geom.Envelope outputEnvelope = JTS
                                .transform(sourceEnvelope, tr);
                        ...
                        GridCoverage2D coverage2D = JGrassCatalogUtilities
                                .getGridcoverageFromGrassraster(jgMEnv, sourceRegion);
                        GridCoverage2D reprojected = (GridCoverage2D) Operations.DEFAULT.resample(
                                coverage2D, crs, outputGridGeometry,
                        ...
                        if (i == 0) {
                            ...
                            try {
                                JGrassCatalogUtilities.createLocation(
                                        newLocationFile.getAbsolutePath(), crs, jgRegion);
                                JGrassCatalogUtilities.createMapset(
                                        newLocationFile.getAbsolutePath(), mapsetName, null,
                                        jgRegion);
                            } catch (Exception e) {
                                // ignore this for now
                            }
                        ...
                        JGrassCatalogUtilities.writeGridCoverageFromGrassraster(newMapFile,
                                jgRegion, reprojected);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    monitor.worked(1);
                ...
                JGrassCatalogUtilities.addServiceToCatalog(
                        mapsetFile.getParent() + File.separator
