 ...
 * </p>
 * <h2>Implementing an IService</h2> Implement the abstract methods and you are good to go.
 * <h2>Extending an IService</h2> You may want to implement your own IService in order to provide a
...
public abstract class IService implements IResolve {
    /**
     * Placeholder used to mark info field as unavailable; often the case where we were unable to
     * connect to a service using createInfo.
     */
    ...
    /**
     * Used to save persisted properties for child resources; please see
     * {@link ServiceParameterPersister} for details.
     * <p>
     * IGeoResource makes use of {@link #getPersistentProperties(ID)} for direct access to these
     * maps.
     */
    Map<ID, Map<String, Serializable>> resourceProperties = Collections
    ...
    /**
     * Will attempt to morph into the adaptee, and return that object. Hard coded to capture the
     * IService contract.
     * <p>
    ...
    /**
     * Hard coded to capture the IService contract.
     * <p>
     * That is we *must* resolve the following:
     * <ul>
     * <li>IService: this
    ...
    public List<IResolve> members(IProgressMonitor monitor) throws IOException {
        List<IResolve> resolves = new ArrayList<>(resources(monitor));
        return resolves;
    }
     ...
     * <p>
     * Subclasses are encouraged to override this to type narrow to the specific ServiceInfo class
     * returned.
     * </p>
     * Example:
     *
    ...
    /**
     * Map of indicated resource's persistent properties. Returns an empty map if this resource has
    ...
    public synchronized Map<String, Serializable> getPersistentProperties(ID child) {
        if (resourceProperties.containsKey(child)) {
            ...
            Map<String, Serializable> properties = Collections
                    .synchronizedMap(new HashMap<String, Serializable>());
    ...
    protected void finalize() throws Throwable {
        // clean up connection
        if (!isDisposed) {
            CatalogPlugin.trace(getClass().getName()
                    + " being cleaned up by fianlize, without prior call to dispose", null); //$NON-NLS-1$
    ...
    public void dispose(IProgressMonitor monitor) {
        if (isDisposed) {
            throw new IllegalStateException("IService.dispose() called, for the second time"); //$NON-NLS-1$
        }
        ...
        if (getStatus() == Status.CONNECTED) {
            try {
                ...
                List<? extends IResolve> members = members(SubMonitor.convert(monitor, 1));
                ...
                for (IResolve resolve : members) {
                    try {
                        ...
                        ErrorManager.get().displayException(e,
                                "Error during dispose: " + resolve.getIdentifier(), //$NON-NLS-1$
                                CatalogPlugin.ID);
                    }
                }
            } catch (Throwable e) {
