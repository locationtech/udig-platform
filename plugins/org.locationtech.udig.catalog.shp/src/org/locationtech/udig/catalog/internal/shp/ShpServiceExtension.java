 ...
 * @since 0.6
 */
public class ShpServiceExtension extends AbstractDataStoreServiceExtension
    ...
    public IService createService(URL id, Map<String, Serializable> params) {
        final Map<String, Serializable> internalParameterMap = copyParams(params);

        if (internalParameterMap.containsKey(OLD_URLP_KEY)) {
            internalParameterMap.put(ShapefileDataStoreFactory.URLP.key,
                    internalParameterMap.get(OLD_URLP_KEY));
            internalParameterMap.remove(OLD_URLP_KEY);
        }
        if (internalParameterMap.containsKey(ShapefileDataStoreFactory.URLP.key)) {
            // shapefile ...

            URL url = null;
            if (internalParameterMap.get(ShapefileDataStoreFactory.URLP.key) == null) {
                return null;
            } else if (internalParameterMap
                    .get(ShapefileDataStoreFactory.URLP.key) instanceof URL) {

                url = (URL) internalParameterMap.get(ShapefileDataStoreFactory.URLP.key);
            } else {
                try {
                    url = (URL) ShapefileDataStoreFactory.URLP.parse(internalParameterMap
                            .get(ShapefileDataStoreFactory.URLP.key).toString());
                    internalParameterMap.put(ShapefileDataStoreFactory.URLP.key, url);
                } catch (Throwable e1) {
                    // log this?
                    e1.printStackTrace();
                    return null;
            ...
            if (!(file.endsWith(".shp") || file.endsWith(".shx") || file.endsWith(".qix")
                    || file.endsWith(".dbf"))) {
                return null;
            }

            if (getSHPDSFactory().canProcess(internalParameterMap)) {
                if (id == null) {
                    return new ShpServiceImpl(url, internalParameterMap);
                } else {
                    return new ShpServiceImpl(id, internalParameterMap);
                }
            }
        }
        return null;
     ...
     * @return
     */
    private Map<String, Serializable> copyParams(Map<String, Serializable> params) {
        Map<String, Serializable> internalParamMap = new HashMap<>();
        if (params != null) {
            internalParamMap.putAll(params);
        }
        return internalParamMap;
    }
    ...
    private URL toShpURL(URL url) {
        ...
        // previous approach tries to duplicate the functionality of URLUtils above; using it as a
        // fallback
    ...
    protected String doOtherChecks(Map<String, Serializable> params) {
        ...
        if (!factory.canProcess(params)) {
            // this is tough we don't have a good error message out of the geotools factory
            // canProcess method
