 ...
 * @since 1.1.0
 */
public class ExportResourceSelectionPage extends WorkflowWizardPage
        implements IPageChangedListener, ICheckStateListener {
    ...
    public void createControl(Composite parent) {
        ...
        viewer.setCellEditors(new CellEditor[] { new TextCellEditor(tree),
                new CRSDialogCellEditor(tree), new CharsetDialogCellEditor(tree) });
            ...
            public Object getValue(Object element, String property) {
                ...
                if (crs.equals(property)) {
                    ...
                    if (data.getName() == null) {
                        return ((ExportResourceLabelProvider) viewer.getLabelProvider())
                                .getColumnText(data, 0);
                    } else {
                        return data.getName();
                    }
        ...
        ExportResourceSelectionState state = getState();
        setInput(state);
    ...
    public void syncWithUI() {
        ExportResourceSelectionState state = getState();
        List<Data> selectedLayers = new ArrayList<>();
    ...
    /**
     * Adapts the new ResolveTitlesDecorator(new ResolveLabelProviderSimple()) so that it will can
     * appear in the table with the title returned by the decorator as the name column and the
     * projection as the title column
     *
     * @author Jesse
     * @since 1.1.0
     */
    public class ExportResourceLabelProvider implements ITableLabelProvider {
        ...
        public ExportResourceLabelProvider() {
            ResolveTitlesDecorator titleDecorator = new ResolveTitlesDecorator(
                    new ResolveLabelProviderSimple());
            wrapped = new DecoratingLabelProvider(titleDecorator.getSource(), titleDecorator);
        }
            ...
            case 2:
                if (data.getCharset() == null) {
                    ...
                    data.setCharset((System.getProperty("shp.encoding") != null) //$NON-NLS-1$
                            ? Charset.forName(System.getProperty("shp.encoding")) //$NON-NLS-1$
