...
public class SearchView extends SearchPart {
        ...
        private void refresh(IResolve res) {
            ...
            if (resolveURL != null && resolveURL.equals(cachedURL)) {
                if (res.canResolve(ICatalogInfo.class)) {
                    try {
                        showInfo(new Info(cache.getId(),
                                res.resolve(ICatalogInfo.class, searchMonitor)));
                    } catch (IOException e) {
                ...
                if (res.canResolve(IServiceInfo.class)) {
                    try {
                        showInfo(new Info(cache.getId(),
                                res.resolve(IServiceInfo.class, searchMonitor)));
                    } catch (IOException e) {
                        CatalogUIPlugin.log(null, e);
                    }
                }
                ...
                if (res.canResolve(IGeoResourceInfo.class)) {
                    try {
                        showInfo(new Info(cache.getId(),
                                res.resolve(IGeoResourceInfo.class, searchMonitor)));
                    } catch (IOException e) {
                        CatalogUIPlugin.log(null, e);
                    }
                }
        ...
        contextMenu.addMenuListener(new IMenuListener() {
            ...
            public void menuAboutToShow(IMenuManager mgr) {
                ...
                refreshAction.setImageDescriptor(
                        CatalogUIPlugin.getDefault().getImageDescriptor(ImageConstants.REFRESH_CO));
                ...
                contextMenu.add(UiPlugin.getDefault().getOperationMenuFactory()
                        .getContextMenu(viewer.getSelection()));
            }
    ...
    protected StructuredViewer createViewer(Composite parent) {
        ...
        catalogTreeViewer.setMessageBoard(new StatusLineMessageBoardAdapter(
    ...
    protected void showInfo(Info info) {
        ...
        if (cache.title != null && !(cache.title.equals(""))) { //$NON-NLS-1$
            summary.append(
                    MessageFormat.format(Messages.SearchView_title, new Object[] { cache.title })
                            + "\n\n"); //$NON-NLS-1$
        }

        if (cache.name != null && !(cache.name.equals(""))) { //$NON-NLS-1$
            summary.append(MessageFormat.format(Messages.SearchView_name,
