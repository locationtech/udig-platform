...
public class BasicWMSMetrics2 extends AbstractRenderMetrics {
    ...
    public boolean canAddLayer( ILayer layer ) {
        ...
        try {
            if( !layer.findGeoResource(Layer.class).service(ProgressManager.instance().get()).
                    equals(getRenderContext().getGeoResource().service(ProgressManager.instance().get())) )
                    return false;
    ...
    public Set<Range> getValidScaleRanges() {
        ...
        try {
            ...
            // lets assume that some implementations are idiotic and min could be greater than max:
            if( min < max ){
