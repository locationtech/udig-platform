...
public class SelectionToolTest extends AbstractToolTest{
    ...
    protected void setUp() throws Exception {
        ...
        FeatureStore source = handler.getContext().getMapLayers().get(0).getResource(FeatureStore.class, null);
        ...
        for( FeatureIterator iter = source.getFeatures().features(); iter.hasNext(); ){
            ...
            source.modifyFeatures(feature.getFeatureType().getAttributeType("name"), "feature"+i, filterFac.createFidFilter(feature.getID()));  //$NON-NLS-1$//$NON-NLS-2$
            Geometry geom;
            if( i==0 ){
            ...
            source.modifyFeatures(feature.getFeatureType().getDefaultGeometry(), geom, filterFac.createFidFilter(feature.getID()));
            i++;
        }
