...
public class ViewportPainter {
    ...
    /**
     * Switches on/off graphics being rendered by custom <code>IDrawCommand</code>s. <br>
     * <code>IMapTransformCommand</code>s are processed always regardless enablement.
     *
     * @param enable
     */
     ...
     * @param minWidth
     */
    public void paint(ViewportGraphics g, org.eclipse.swt.graphics.Image image,
            AffineTransform imageTrsf, int minWidth, int minHeight) {
        processCommands(g, true);
        clearPane(g, minWidth, minHeight);
        if (image != null) {
    ...
    /**
     * Paints the image of all the given tiles, and all the currently valid commands on the
     * viewport.
     *
     * @param g The graphics to draw on.
     ...
     */
    public void paint(ViewportGraphics g, Map<ReferencedEnvelope, Tile> tiles, int minWidth,
            int minHeight) {
        processCommands(g, true);
        clearPane(g, minWidth, minHeight);
        ...
        if (tiles != null) {
            ...
            if (gc != null) {
                ...
                for (ReferencedEnvelope env : keySet) {
                    ...
                    ViewportModel viewportModelInternal = tile.getRenderExecutor().getRenderer()
                            .getContext().getViewportModelInternal();
                    java.awt.Point a = viewportModelInternal
                            .worldToPixel(new Coordinate(env.getMinX(), env.getMinY()));
                    java.awt.Point b = viewportModelInternal
                    ...
                    synchronized (tile) {
                        ...
                        try {
                            ...
                            gc.drawImage(im, 0, 0, im.getBounds().width, im.getBounds().height, a.x,
                                    b.y, width, height);
                        } catch (Exception ex) {
                        }
                    }
    ...
    private void runEditFeatureCommand(ViewportGraphics g) {
        if (editfeatureCommand == null
                && pane.getMapPart().getMap().getViewportModelInternal() != null) {
            editfeatureCommand = new DrawEditFeatureCommand(
                    pane.getMapPart().getMap().getViewportModelInternal());
        }
        if (editfeatureCommand != null) {
        ...
        try {
            ...
            for (int i = 0; i < varray.length; i++) {
                ...
                if (command.isValid()) {
                    ...
                    try {
                        command.setMap(this.pane.getMapPart().getMap());
    ...
    /**
     * Draws the glass pane if the ViewportGraphics is a SWTGraphics or NonAdvancedSWTGraphics.
     *
     * Will not draw the glasspane if it is null or the Viewport Graphics is a AWT Graphics.
     *
     * @param g
     */
