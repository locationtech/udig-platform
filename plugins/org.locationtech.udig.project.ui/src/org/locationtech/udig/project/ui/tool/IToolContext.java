...
public interface IToolContext extends IAbstractContext {
     ...
     * ViewportPane and the ViewportPane will be refreshed.
     *
    ...
    /**
     * Calculates the best zoom level based on the Preferred Zoom levels in ViewportModel. As
     * recommended in the API the preferred zoom levels are used only if they are not defaults.
     *
     * That behavior can be overridden by setting alwayUsePreferredZoomLevels to be true
     *
     * @param previousZoom the value of the previousZoom level. 1 is no zoom. This is required for
     *        incremental zooming
     * @param zoomChange the difference between the previousZoom level and the new desiredZoom
     * @param fixedPoint the zoom center
     * @param alwayUsePreferredZoomLevels true to always used preferred zoom levels even if they are
     *        the defaults
