...
/**
 * A facade into uDig to simplify operations such as getting the active map and opening a map
 * editor.
 *
 * @author jeichar
 * @since 0.9.0
...
public class ApplicationGIS {
    ...
    /**
     * @return MapPart if there is one, otherwise null.
     */
    public static MapPart getActiveMapPart() {
        if (activeMapTracker == null) {
            return null;
        }
        return activeMapTracker.getActiveMapPart();
    }
    ...
    public static List<UDIGEditorInputDescriptor> getEditorInputs(
        ...
        final List<UDIGEditorInputDescriptor> newInputs = new ArrayList<>();

        List<IConfigurationElement> extensions = ExtensionPointList
                .getExtensionPointList("org.locationtech.udig.project.ui.editorInputs"); //$NON-NLS-1$
        Class toMatch;
        ...
        for (IConfigurationElement element : extensions) {
            String projectElementClassName = element.getAttribute("projectElement"); //$NON-NLS-1$
    ...
    private static Class match(Class toMatch, String projectElementClassName) {
        if (toMatch.getName().equals(projectElementClassName)
                || toMatch.getCanonicalName().equals(projectElementClassName)) {
            return toMatch;
        }
        ...
        synchronized (ToolManager.class) {
            if (toolManager == null) {
                ...
                IToolManager result = (IToolManager) UiPlugin.lookupConfigurationObject(
                        ...
                        ProjectUIPlugin.ID, prefConstant, xpid, idField, classField);
                if (result != null) {
                    toolManager = result;
                } else {
    ...
    public static IToolContext createContext(IMap map) {
        if (map instanceof org.locationtech.udig.project.internal.Map) {
            ...
            context.setMapInternal((org.locationtech.udig.project.internal.Map) map);
            return context;
        }
        return null;
    }
     ...
     * @param startPosition z-position of the layers to add. if -1 it will be added to the top of
     *        the map (0 is the bottom of the map and map.getMapLayer.size() is the top of the map).
     ...
     * @param project project that map should be added to... Only used if there is no current map.
     *        If project is then the default project is used.
     * @param wait if true then method will block until map has been opened otherwise will return
     *        without blocking.
     */
    public static List<? extends ILayer> addLayersToMap(IMap map2, List<IGeoResource> resourceList,
        ...
        if (ProjectPlugin.getPlugin().getPluginPreferences()
                .getBoolean(PreferenceConstants.P_CHECK_DUPLICATE_LAYERS)) {
    ...
    public static IProjectElement loadProjectElement(URL url, IProject project)
        ...
        try {
            ...
            throw new IllegalArgumentException(Messages.ApplicationGIS_illegalArgumentPart1 + uri
    ...
    public static class DrawMapParameter {
         ...
         * @param boundsStrategy An object for determining how to set the bounds on the map. Feel
         *        free to extend the current implementation
         * @param dpi the DPI of the map. The standard PDF is 72, the standard according to the OGC
         *        is 90
         * @param selectionStyle how to handle the selection (getFilter()) on a layer
         * @param monitor a progress monitor
         */
        public DrawMapParameter(Graphics2D graphics, Dimension destinationSize, IMap toDraw,
                BoundsStrategy boundsStrategy, int dpi, SelectionStyle selectionStyle,
            ...
            if (boundsStrategy == null) {
                this.boundsStrategy = new BoundsStrategy(
                        toDraw.getViewportModel().getScaleDenominator());
            } else {
        ...
        public DrawMapParameter(Graphics2D graphics, Dimension destinationSize, IMap toDraw,
            ...
            this(graphics, destinationSize, toDraw, boundsStrategy, dpi, selectionStyle, monitor,
                    false, false);
        }
        ...
        public DrawMapParameter(Graphics2D graphics, Dimension destinationSize, IMap toDraw,
                IProgressMonitor monitor) {
            this(graphics, destinationSize, toDraw,
                    new BoundsStrategy(toDraw.getViewportModel().getScaleDenominator()), 90,
                    SelectionStyle.OVERLAY, monitor);
        }
         ...
         */
        public DrawMapParameter(Graphics2D graphics, Dimension destinationSize, IMap toDraw,
                int dpi, IProgressMonitor monitor) {
            this(graphics, destinationSize, toDraw,
                    new BoundsStrategy(toDraw.getViewportModel().getScaleDenominator()), dpi,
                    SelectionStyle.OVERLAY, monitor);
        }
        ...
        public DrawMapParameter(Graphics2D graphics, Dimension destinationSize, IMap toDraw,
            ...
            this(graphics, destinationSize, toDraw,
                    new BoundsStrategy(toDraw.getViewportModel().getScaleDenominator()), 90,
                    SelectionStyle.OVERLAY, monitor, transparent, false);
        }

        /**
         * Copies the parameters as well as the graphics and destinationSize objects. DOES NOT COPY
         * map ToDraw
         *
         * @param params2 the parameters to copy
         */
        public DrawMapParameter(DrawMapParameter params2) {
            this((Graphics2D) params2.graphics.create(), new Dimension(params2.destinationSize),
                    params2.toDraw, params2.boundsStrategy, params2.dpi, params2.selectionStyle,
     ...
     *
     * @throws RenderException Thrown if an error occurs such as a unreachable server.
     *
    ...
    public static IMap drawMap(final DrawMapParameter drawMapParams) throws RenderException {
        ...
        IRunnableWithProgress runnable = new IRunnableWithProgress() {
            ...
            public void run(IProgressMonitor monitor)
                ...
                // Load IGeoResources using original map. The new map can't do this because it
                // doesn't have a
                ...
                Color background = (Color) map.getBlackboard()
                        .get(ProjectBlackboardConstants.MAP__BACKGROUND_COLOR);
                params.graphics.setBackground(background);
                if (!drawMapParams.transparent) {
                    params.graphics.clearRect(0, 0, params.destinationSize.width,
                            params.destinationSize.height);
                }
                List<Layer> layersToRender = params.selectionStyle
                        .handleSelection(map.getLayersInternal());

                ProjectUIPlugin.trace(ApplicationGIS.class,
                        "ApplicationGIS.drawMap() beginning rendering of map '" + map.getName() //$NON-NLS-1$
                                + "'", //$NON-NLS-1$
                        null);

                ReferencedEnvelope bounds = params.toDraw.getViewportModel()
                        .getBounds();
                ReferencedEnvelope boundsCopy = new ReferencedEnvelope(bounds);
                RenderContext tools = configureMapForRendering(map, params.destinationSize,
                        params.dpi, params.boundsStrategy, boundsCopy);
                ...
                SortedSet<RenderContext> sortedContexts = new TreeSet<>(
            ...
            private void render(final DrawMapParameter params, IProgressMonitor monitor,
                    RendererCreator decisive, SortedSet<RenderContext> sortedContexts)
                    throws InvocationTargetException {

                monitor.beginTask("Rendering map", sortedContexts.size()); //$NON-NLS-1$
                ...
                while (iter.hasNext()) {
                    ...
                    if (!(layer instanceof SelectionLayer) || ((layer instanceof SelectionLayer)
                            && params.selectionStyle.getShowLabels())) {
                    ...
                    try {
                        ...
                        labelPainter.endLayer(layerId, params.graphics, new Rectangle(displaySize));
                    }
                }
                labelPainter.end(params.graphics, new Rectangle(displaySize));
                labelPainter.clear();
            }
    ...
    /**
     * Creates a ViewportModel and RenderManager; configures them correctly and sets them on the
     * RenderContext. The layer and the GeoResource still must be set on the context. Returns the
     * Context.
     *
     * @param map the map
     ...
     * @param baseMapBounds the bounds of the reference map, depending on the boundsStrategy it may
     *        be null.
     * @return a render context
     */
    public static RenderContext configureMapForRendering(Map map, final Dimension destinationSize,
            final int dpi, BoundsStrategy boundsStrategy, ReferencedEnvelope baseMapBounds) {
        ...
        ProjectUIPlugin.trace(ApplicationGIS.class, "Firing size changed event. Changing to size: " //$NON-NLS-1$
                + destinationSize.width + " by " + destinationSize.height, null); //$NON-NLS-1$
    ...
    public static void setActiveMapTracker(ActiveMapTracker activeMapTrackerToSet) {
        if (activeMapTracker != null && activeMapTrackerToSet != null) {
            throw new Error(
                    "This method has already been called! It is an error for non-uDig code to call this method"); //$NON-NLS-1$
        }
    ...
    public static IMap copyMap(IMap mapToCopy) {
        // Load IGeoResources using original map. The new map can't do this because it doesn't have
        // a
        // Resource(file) and therefore can't resolve relative URIs
     ...
     *
     * If the typeToCreate is NOT the same or a superclass of the object created or if an object
     * cannot be created a {@link IllegalArgumentException} will be thrown
     *
     * @param project
     *
     ...
     *
     * @return A {@link ProjectElementAdapter} that wraps/adapts the object created using the
     *         extension
     */
    public static ProjectElementAdapter createGeneralProjectElement(IProject project,
        ...
        return ElementFactory.eINSTANCE.createProjectElementAdapter(project, typeToCreate,
                extensionId);
    }
     ...
     */
    public static ProjectElementAdapter createGeneralProjectElement(IProject project,
            String elementName, Class<? extends IGenericProjectElement> typeToCreate,
            String extensionId) throws IllegalArgumentException {
        return ElementFactory.eINSTANCE.createProjectElementAdapter(project, elementName,
                typeToCreate, extensionId);
    }
