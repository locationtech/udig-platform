...
public class RenderManagerAdapters {
        ...
        return new AdapterImpl() {
            ...
            public void notifyChanged(Notification msg) {
                ...
                case RenderPackage.VIEWPORT_MODEL__BOUNDS: {
                    if (ApplicationGIS.getActiveMap() != null && ApplicationGIS.getVisibleMaps()
                            .contains(ApplicationGIS.getActiveMap())) {
     ...
     *
     * <p>
     * This viewport listeners calls a soft refresh function with doesn't clear existing contexts or
     * rerender tiles before drawing so it should reuse tiles if they have already been loaded. New
     * tiles will be rendered.
     * </p>
     * <p>
     * A CRS change causes a full refresh; flushing all the tiles as currently tiles do not have a
     * CRS associated with them so they have to be cleared so the wrong projection tiles are not
     * used.
     * </p>
     *
     * <p>
     * This listeners listens to:
     * <ul>
    ...
    /**
     * Who are you and what do you do? It does not actually get added anywhere so we do not know
     * where and how.
     *
     * @param manager
     * @returnContext
    ...
    static ContextModelListenerAdapter createContextModelListener(
        ...
        return new ContextModelListenerAdapter() {
            ...
            /**
             * Will sychronizeAndRefresh( the manager based on the notification; and updateImage().
             *
    ...
    /**
     * This private method is called by listeners (such as the context model listener) in order to
     * kick layers into life.
     * <p>
     * This implementation hunts down a layer can calls layer.refresh( null ) in order to ask it to
     * redraw pretty much everything it has... the synchronizeRenderers will hunt down the list of
     * layers effected by this notification....scared.
     * <p>
     *
     * @param msg
     * @param manager
    ...
    private static void synchronizeAndRefresh(Notification msg, RenderManagerDynamic manager) {
        List<Layer> toRender = synchronizeRenderers(msg, manager.configuration,
                manager.getRendererCreator());
    ...
    /**
     * This method is responsible for spitting out a list of layers that need to be refreshed in
     * response to the provided notification.
     * <p>
     *
     *
     * @param msg notification message (such as a zorder change) causing this change
     * @param configuration RemderContexts being drawn into...
     * @param rendererCreator RemderCreator responsible for setting up renderers associated with
     *        these layers
    ...
    public static List<Layer> synchronizeRenderers(final Notification msg,
            final Collection<RenderContext> configuration, final RendererCreator rendererCreator) {
        /**
         * This is a back up of the render context (ie with buffered image and stuff) so we can
         * recycle them...
         */
        ...
        for (RenderContext newcontext : configuration2) {
            if (!oldToCopy.containsKey(newcontext)
                ...
                if (newcontext instanceof CompositeRenderContext) {
                    ...
                    List<Layer> oldLayers = new ArrayList<>(
                            ((CompositeRenderContext) oldToCopy.get(newcontext))
                                    .getLayersInternal());
                    // these are our old layers; we want to check if they are in a different order
                    // or something ...
                    ...
                    if (!oldLayers.isEmpty()) {
                        ...
                        toRender.add(newcontext.getLayerInternal()); // perhaps the old stuff will
    ...
    /**
     * <!-- begin-user-doc --> Listens to the setMap or setViewportModel methods getting called;
     * will wire up our viewportListener so we can tell what is going on at runtime. <!--
     * end-user-doc -->
     *
     * @generated NOT
     */
    static Adapter createViewportModelChangeListener(final RenderManagerImpl manager,
            final Adapter viewportListener,
            final ContextModelListenerAdapter contextModelListener) {
        return new AdapterImpl() {
            /**
             * @see org.eclipse.emf.common.notify.impl.AdapterImpl#notifyChanged(org.eclipse.emf.common.notify.Notification)
             */
            ...
            public void notifyChanged(Notification msg) {
                ...
                case RenderPackage.RENDER_MANAGER__MAP_INTERNAL: {
                    if (msg.getOldValue() != null)
                        ((Map) msg.getOldValue()).getContextModel().eAdapters()
                                .remove(contextModelListener);
                    if (msg.getNewValue() != null)
                        ((Map) msg.getNewValue()).getContextModel().eAdapters()
                                .add(contextModelListener);
                    break;
                } // case
    ...
    static Adapter createLayerListener(final RenderManagerDynamic manager) {
        return new AdapterImpl() {
            ...
            public void notifyChanged(Notification event) {
                ...
                if (event.getNotifier() instanceof Map) {
                    ...
                    case ProjectPackage.LAYER__STYLE_BLACKBOARD:
                        ...
                        synchronizeRenderers(event, manager.configuration,
                                manager.getRendererCreator());
            ...
            private void featureEvent(final RenderManagerDynamic manager, Notification event,
                    FeatureEvent featureEvent) {
                ReferencedEnvelope refreshBounds = null;
                ILayer refreshLayer = null;
                if (ProjectPlugin.getPlugin().getPluginPreferences()
                        .getBoolean(PreferenceConstants.P_FEATURE_EVENT_REFRESH_ALL)) {
                    ILayer notifier = (ILayer) event.getNotifier();
                    ReferencedEnvelope viewportBounds = notifier.getMap().getViewportModel()
                            .getBounds();
                    ...
                    if (delta != null) {
                        ...
                        if (delta.isNull()) {
                            ...
                            try {
                                ...
                                CoordinateReferenceSystem mapCRS = notifier.getMap()
                                        .getViewportModel().getCRS();
                                ReferencedEnvelope bounds = new ReferencedEnvelope(mapDelta,
                                        mapCRS);
    ...
    /**
     * This one actually kicks the viewport pane to redraw as rendering events occur.
     */
     ...
     *
     * <p>
     * This listener listens to the following events:
     * <ul>
    ...
    static Adapter createLayerListener(final TiledRenderManagerDynamic manager) {
        return new AdapterImpl() {
            ...
            private void featureEvent(final TiledRenderManagerDynamic manager, Notification event,
                    FeatureEvent featureEvent) {
                ReferencedEnvelope refreshBounds = null;
                ILayer refreshLayer = null;
                if (ProjectPlugin.getPlugin().getPluginPreferences()
            ...
            private void filterChanged(final TiledRenderManagerDynamic manager,
                    Notification event) {
                if (!event.getOldValue().equals(event.getNewValue())) {
                    Filter newFilter = (Filter) event.getNewValue();
    ...
    /**
     * Creates an adapter that listens for visibility changes in layers. When a layer visibility
     * change occurs the tiles in the tiles render manager need to be invalidated appropriately.
     *
     * @param manager
     * @return
     */
        ...
        return new AdapterImpl() {
            ...
            public void notifyChanged(final Notification msg) {
                if (msg.getNotifier() instanceof Layer
                        && msg.getFeatureID(Layer.class) == ProjectPackage.LAYER__VISIBLE) {
