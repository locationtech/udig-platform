...
/**
 *
 * The Grid Handler of the Legend View. This class is designed to handle the maintenance of the grid
...
public class LegendViewGridHandler implements ILayerListener {
    ...
    /**
     * Cleans the handler of listeners attached to the grid layers, current map and the current map
     * itself.
     */
    ...
    /**
     * Initializes the current map and adds listeners to it.
     *
     * @param map
     */
    ...
    /**
     * Initializes the grid layers from the passed map. The method traverses the map and looks for
    ...
    private void initGridLayers(Map map) {
        ...
        if (map != null) {
            ...
            if (gridLayers.isEmpty()) {
                ...
                this.map.sendCommandASync(new AddLayersCommand(
                        Collections.singletonList(LegendViewUtils.getGridMapGraphic())));
            }
    ...
    /**
     * Initializes and returns the gridAction action.
     *
     * @return
     */
    public Action getGridAction() {
        ...
        gridAction.setImageDescriptor(
                ProjectUIPlugin.getDefault().getImageDescriptor(ISharedImages.TOG_GRID_CO));
    ...
    /**
     * Refreshes the grid toggle button display and sets/unsets the grid layers handled by the
     * handler
     */
