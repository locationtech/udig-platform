...
public class ProjectExplorer extends ViewPart
        implements IMenuListener, ISetSelectionTarget, IDropTargetProvider {
     ...
     * for extension. <!--
     *
     * @param viewer The viewer to create a context menu for.
     */
    ...
    public void createPartControl(Composite parent) {
        ...
        treeViewer.setLabelProvider(
                new AdapterFactoryLabelProviderDecorator(getAdapterFactory(), treeViewer));
            ...
            public void selectionChanged(SelectionChangedEvent event) {
                ...
                if (obj instanceof ILayer)
                    getViewSite().getActionBars().getStatusLineManager()
                            .setMessage(((ILayer) obj).getStatusMessage());
                if (preferenceStore.getBoolean(PROJECT_EXPLORER_LINKED)) {
                    if (obj instanceof IProjectElement) {
                        IWorkbenchPage page = getSite().getPage();
                        IEditorPart part = page
                                .findEditor(ApplicationGIS.getInput((IProjectElement) obj));
     ...
     * custom delete action that is willing to delete a project.
     */
    ...
    private IAction createLinkAction() {
        ...
        linkAction.setImageDescriptor(
                ProjectUIPlugin.getDefault().getImageDescriptor(ISharedImages.LINKED_ENABLED_CO));
        linkAction.setDisabledImageDescriptor(
                ProjectUIPlugin.getDefault().getImageDescriptor(ISharedImages.LINKED_DISABLED_CO));
    ...
    public void menuAboutToShow(IMenuManager manager) {
        ...
        manager.add(ApplicationGIS.getToolManager()
                .createOperationsContextMenu(treeViewer.getSelection()));
        manager.add(new Separator());
        manager.add(ActionFactory.EXPORT.create(getSite().getWorkbenchWindow()));
        if (!treeViewer.getSelection().isEmpty()
                && ((IStructuredSelection) treeViewer.getSelection())
                        .getFirstElement() instanceof Map) {
            manager.add(new Separator());
    ...
    /**
     * Selects the element in the tree. This is a non-blocking method and selection may take a while
     * to take effect.
     *
    ...
    private void inDisplaySelect(Collection<? extends IProjectElement> element, boolean reveal) {
        ...
        if (visibleElements.containsAll(element)) {
            ...
            synchronized (contentProvider) {
                SetSelectionListener old = inputChangedListener
                        .getAndSet(new SetSelectionListener(element, reveal));
