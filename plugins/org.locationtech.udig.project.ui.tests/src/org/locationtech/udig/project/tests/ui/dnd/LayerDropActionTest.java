...
public class LayerDropActionTest extends AbstractProjectUITestCase {
    ...
    public void setUp() throws Exception {
        ...
        IGeoResource[] resources = new IGeoResource[] {
                CatalogTests.createGeoResource("DropActionTestFeatures2", 3, true), //$NON-NLS-1$
                CatalogTests.createGeoResource("DropActionTestFeatures2", 4, true), //$NON-NLS-1$
                CatalogTests.createGeoResource("DropActionTestFeatures2", 5, true), //$NON-NLS-1$
                CatalogTests.createGeoResource("DropActionTestFeatures2", 6, true) //$NON-NLS-1$
        };
    ...
    public void testAccept() throws Exception {
        ...
        action.init(null, null, ViewerDropLocation.ON, map.getLayersInternal().get(0),
                map.getLayersInternal().get(1));
        assertTrue(action.accept());
        // layer dropped before another layer in same map.
        action.init(null, null, ViewerDropLocation.BEFORE, map.getLayersInternal().get(0),
                map.getLayersInternal().get(1));
        assertTrue(action.accept());
        // layer dropped after another layer in same map.
        action.init(null, null, ViewerDropLocation.AFTER, map.getLayersInternal().get(0),
                map.getLayersInternal().get(1));
        assertTrue(action.accept());
        ...
        action.init(null, null, ViewerDropLocation.NONE, map.getLayersInternal().get(0),
                map.getLayersInternal().get(0));
        assertTrue(action.accept());

        // layer dropped on a layer in another map.
        action.init(null, null, ViewerDropLocation.AFTER, ProjectFactory.eINSTANCE.createLayer(),
                map.getLayersInternal().get(0));
        assertFalse(action.accept());
