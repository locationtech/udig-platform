...
class PostgisResourceInfo extends IGeoResourceInfo {
                ...
                try {
                    ...
                    CatalogPlugin.getDefault().getLog()
                            .log(new org.eclipse.core.runtime.Status(IStatus.WARNING,
                                    ...
                                    Messages.PostGISGeoResource_error_layer_bounds, e));
                    bounds = new ReferencedEnvelope(new Envelope(), null);
                }
            }
        }
    ...
    private SimpleFeatureType getFeatureType() {
        try {
            ...
            PostgisPlugin.log("Unable to retrieve SimpleFeatureType schema for type '" //$NON-NLS-1$
                    + owner.typename + "'.", e); //$NON-NLS-1$
            keywords = new String[] { "teradata", //$NON-NLS-1$
                    owner.typename };
            return null;
        }
