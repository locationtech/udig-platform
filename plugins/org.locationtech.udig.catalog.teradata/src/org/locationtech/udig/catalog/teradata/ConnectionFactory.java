...
public class ConnectionFactory extends UDIGConnectionFactory {
    ...
    protected URL toCapabilitiesURL(Object data) {
        if (data instanceof IResolve) {
            ...
            Activator.log(
                    "Unable to convert object: " + data + " to a capabilities url for teradata", //$NON-NLS-1$ //$NON-NLS-2$
                    null);
    ...
    protected URL toCapabilitiesURL(String string) {
        ...
        if (!string.contains(DIALECT.urlPrefix) && !string.contains("jdbc.teradata") //$NON-NLS-1$
                && !string.contains("teradata")) { //$NON-NLS-1$
            return null;
        }
