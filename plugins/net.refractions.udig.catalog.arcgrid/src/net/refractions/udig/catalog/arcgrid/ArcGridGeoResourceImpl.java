 ...
 * @since 0.6.0
 */
public class ArcGridGeoResourceImpl extends AbstractRasterGeoResource {
    private ArcGridGeoResourceInfo info;

    /**
     * Construct <code>ArcGridGeoResourceImpl</code>.
     *
     * @param service
     * @param name
     */
    public ArcGridGeoResourceImpl(AbstractRasterService service, String name) {
        super(service, name);
    }

    public IGeoResourceInfo getInfo(IProgressMonitor monitor) {
        if(monitor != null)  {
            monitor.beginTask(Messages.ArcGridGeoResource_connect, 2);
            monitor.worked(1);
        }
        if(this.info == null) {
            this.info = new ArcGridGeoResourceInfo();
            if(monitor != null)
                monitor.worked(1);
        }
        if(monitor != null)
    ...
    public class ArcGridGeoResourceInfo extends IGeoResourceInfo {
        ArcGridGeoResourceInfo() {
            this.keywords = new String[] {
                    "ArcGrid", ".arc"};   //$NON-NLS-1$//$NON-NLS-2$ //$NON-NLS-3$
            ...
            if(this.bounds == null) {
                ...
                try {
                    ...
                    CatalogPlugin.getDefault().getLog().log(new org.eclipse.core.runtime.Status(IStatus.WARNING,
                            "net.refractions.udig.catalog.arcgrid", 0, "Error while getting the bounds of a layer", e ));   //$NON-NLS-1$ //$NON-NLS-2$
                }
            }
            return this.bounds;
        }
