...
public class UDIGDropHandler extends DropTargetAdapter {
    ...
    private Set<IDropHandlerListener> listeners = new CopyOnWriteArraySet<>();
    ...
    private Object getJavaObject(Transfer transfer, TransferData data) {
        try {
            ...
            Method m = transfer.getClass().getMethod("nativeToJava", //$NON-NLS-1$
                    new Class[] { TransferData.class });
    ...
    public void dragEnter(DropTargetEvent event) {
        if (UiPlugin.isDebugging(Trace.DND)) {
            ...
            System.out.println("UDIGDropHandler.dragEnter " + control.toString() //$NON-NLS-1$
                    + ": Setting event.detail to COPY"); //$NON-NLS-1$
        }
        event.detail = DND.DROP_COPY;
    }
    ...
    public void drop(DropTargetEvent event) {
        ...
        if (actions != null && !actions.isEmpty()) {
            ...
            UiPlugin.trace(Trace.DND, getClass(),
                    "Target " + getTarget() + " found " + filteredActions.size() + " drop actions", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                    null);
            for (IDropAction action : filteredActions) {
                UiPlugin.trace(Trace.DND, getClass(), " * Action " + action.getName() //$NON-NLS-1$
                        + " implementation " + action.getClass().getSimpleName(), null); //$NON-NLS-1$
            }

            actionJob.addActions(this, filteredActions);
        } else {
            UiPlugin.trace(Trace.DND, getClass(), event.data + " dropped on " + getTarget() //$NON-NLS-1$
                    + " found no actions for processing it", null); //$NON-NLS-1$
            notifyNoDropAction(event.data);
        }
    }

    /**
     * A hook for subclasses to modify the list of actions that will be executed for this drop
     * event.
     *
    ...
    private List<IDropAction> findDropActions(Object data, DropTargetEvent event) {
        if (UiPlugin.isDebugging(Trace.DND)) {
            ...
            String value = data.getClass().isArray() ? Array.getLength(data) + " items" //$NON-NLS-1$
                    : data.toString();
    ...
    public void performDrop(Object data, DropTargetEvent event) {
        ...
        CompositeDropActionJob actionJob = getActionJob();
        if (!actions.isEmpty()) {
            UiPlugin.trace(Trace.DND, getClass(), data + " dropped on " + getTarget() + " found " //$NON-NLS-1$ //$NON-NLS-2$
                    + actions.size() + " drop actions", null); //$NON-NLS-1$
            ...
            UiPlugin.trace(Trace.DND, getClass(),
                    data + " dropped on " + getTarget() + " found no actions for processing it", //$NON-NLS-1$//$NON-NLS-2$
                    null);
    ...
    public static class CompositeDropActionJob extends Job {
        ...
        /**
         * Returns the drag and drop job queue. It contains all the actions that have not been
         * executed. It is not modifiable
         *
         * @return
         */
        public synchronized List<Collection<DropActionRunnable>> getJobQueue() {
            return Collections
                    .unmodifiableList(new ArrayList<>(queue));
        }
        ...
        synchronized void addActions(UDIGDropHandler handler, Collection<IDropAction> actions) {
            List<DropActionRunnable> runnables = new ArrayList<>();
            for (IDropAction action : actions) {
        ...
        protected IStatus run(IProgressMonitor monitor2) {
            ...
            while (!Thread.currentThread().isInterrupted()) {
                ...
                for (Iterator<DropActionRunnable> iterator = next.iterator(); iterator.hasNext()
                    ...
                    SubMonitor monitor = SubMonitor.convert(monitor2, 10);

                    monitor2.setTaskName(Messages.UDIGDropHandler_performing_task + ": " //$NON-NLS-1$
