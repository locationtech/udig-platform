 ...
 * </p>
 * <p>
 * A GeometryTextTransfer can encode the geometry as a GML string or a WKT string (as created by JTS
 * WKTWriter). The GeometryTextTransfer class subclasses AbstractStrategizedTransfer and only needs
 * to provide the current strategy and the set of possible strategies. The
 * AbstractStrategizedTransfer will first try to use the current strategy for encoding the geometry
 * then all the rest if the current strategy fails. Similarly for decoding an input(nativeToJava)
 * the current strategy will be tried first then the rest of the known strategies will be used until
 * an object is obtained.
 * </p>
 *
 * @see org.locationtech.udig.internal.ui.TransferStrategy
 * @author jeichar
 */
public abstract class AbstractStrategizedTransfer extends ByteArrayTransfer
        implements UDIGTransfer {

    final Set<TransferStrategy> knownStrategies = Collections
            .synchronizedSet(new HashSet<TransferStrategy>());
    ...
    public void javaToNative(Object object, TransferData transferData) {
        try {
            ...
            LoggingSupport.log(UiPlugin.getDefault(), "Error encoding " + object, e); //$NON-NLS-1$
        }
    ...
    public Object nativeToJava(TransferData transferData) {
        ...
        for (TransferStrategy strategy : knownStrategies) {
            ...
            try {
                ...
                LoggingSupport.log(UiPlugin.getDefault(), "Error decoding transferData", e); //$NON-NLS-1$
                continue;
            }
        }
    ...
    /**
     * Returns the current strategy as indicated by the preferences
     *
     * @return the current strategy as indicated by the preferences
     */
    public TransferStrategy getCurrentStrategy() {
        String indicator = UiPlugin.getDefault().getPreferenceStore()
                .getString(getClass().getName());
