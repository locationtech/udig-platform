...
public class FeatureTypeEditor {
    ...
    private static final int MAX_ATTRIBUTE_LENGTH = 10485759; // Maximum allows by postgis and is
                                                              // "big enough"

    private static final List ALLOWED_BINDINGS_FOR_LENGHT_SET = Arrays.asList(String.class,
            Double.class, Long.class, Integer.class, Short.class);
    ...
    static {
        List<LegalAttributeTypes> types = new ArrayList<>();
        ...
        types.add(new LegalAttributeTypes(Messages.FeatureTypeEditor_lineStringType,
                LineString.class));
        ...
        types.add(new LegalAttributeTypes(Messages.FeatureTypeEditor_multiPointType,
                MultiPoint.class));
        types.add(new LegalAttributeTypes(Messages.FeatureTypeEditor_multiLineStringType,
                MultiLineString.class));
        types.add(new LegalAttributeTypes(Messages.FeatureTypeEditor_multiPolygonType,
                MultiPolygon.class));
    ...
    public void createTable(Composite parent, Object layoutData, SimpleFeatureType featureType,
        ...
        viewer.setColumnProperties(new String[] { String.valueOf(NAME_COLUMN),
                String.valueOf(TYPE_COLUMN), String.valueOf(OTHER_COLUMN),
                String.valueOf(LENGTH_COLUMN), String.valueOf(IS_NULL_COLUMN) });
    ...
    public void setEditable(boolean editable) {
        if (editable) {
            ...
            viewer.setCellEditors(new CellEditor[] { attributeNameEditor, attributeTypeEditor,
                    crsEditor, attributeLengthEditor, attributeIsNullEditor });
    ...
    /**
     * Creates a ContextMenu (the menu is created using the Table's composite as a parent) and
     ...
     *
     * @param builder
     * @deprecated with the new {@link SimpleFeatureTypeBuilder} this is no more possible, therefore
     *             deprecating.
     */
    @Deprecated
    ...
    private CoordinateReferenceSystem getDefaultCRS() {
        String crsInfo = UiPlugin.getDefault().getPreferenceStore()
                .getString(PreferenceConstants.P_DEFAULT_GEOMEMTRY_CRS);
    ...
    public void setDefaultCRS(CoordinateReferenceSystem crs) {
        ...
        UiPlugin.getDefault().getPreferenceStore()
                .setValue(PreferenceConstants.P_DEFAULT_GEOMEMTRY_CRS, crsInfo);
    ...
    public synchronized IAction getCreateAttributeAction() {
        if (createAttributeAction == null) {
            createAttributeAction = new Action() {
                ...
                public void runWithEvent(Event event) {
                    ...
                    for (AttributeDescriptor at : ft.getAttributeDescriptors()) {
                        if (at.getLocalName().startsWith(
                                Messages.FeatureTypeEditor_newAttributeTypeDefaultName)) {
                            try {
                                usedIndices.add(Integer.valueOf(at.getLocalName().replace(
                                        Messages.FeatureTypeEditor_newAttributeTypeDefaultName,
                                        ""))); //$NON-NLS-1$
                            } catch (Exception e) {
                    ...
                    ftB.add(Messages.FeatureTypeEditor_newAttributeTypeDefaultName + index,
                            String.class);
            ...
            createAttributeAction
                    .setId("org.locationtech.udig.ui.FeatureTypeEditor.createAttributeAction"); //$NON-NLS-1$
            ...
            createAttributeAction.setImageDescriptor(
                    UiPlugin.getDefault().getImageDescriptor("elcl16/new_attribute.gif")); //$NON-NLS-1$
        }
        ...
        if (deleteAttributeAction == null) {
            deleteAttributeAction = new Action() {
                ...
                public void runWithEvent(Event event) {
                    ...
                    for (Iterator<AttributeDescriptor> iter = selection.iterator(); iter
                            .hasNext();) {
            ...
            deleteAttributeAction.setImageDescriptor(
                    UiPlugin.getDefault().getImageDescriptor("elcl16/delete.gif")); //$NON-NLS-1$
            deleteAttributeAction.setDescription(Messages.deleteAttributeAction_description);
            deleteAttributeAction
                    .setId("org.locationtech.udig.ui.FeatureTypeEditor.deleteAttributeAction"); //$NON-NLS-1$
        }
    ...
    public class AttributeCellModifier implements ICellModifier {
        ...
        public boolean canModify(Object element, String property) {
            if (String.valueOf(OTHER_COLUMN).equals(property)
                    && !(element instanceof GeometryDescriptor)) {
                return false;
            }
            if (String.valueOf(LENGTH_COLUMN).equals(property) && !(ALLOWED_BINDINGS_FOR_LENGHT_SET
                    .contains(((AttributeDescriptor) element).getType().getBinding()))) {
                return false;
            }
            return true;
        }
        ...
        private AttributeDescriptor createNewAttributeType(AttributeDescriptor editElement,
     ...
     * @return
     */
    protected static <T> T getAttributeRestriction(AttributeDescriptor editElement,
            String restrictionName, Class<T> type) {
        ...
        for (Filter r : editElement.getType().getRestrictions()) {
            if (r instanceof PropertyIsLessThanOrEqualTo) {
                ...
                if (c.getExpression1() instanceof Function && ((Function) c.getExpression1())
                        .getName().toLowerCase().endsWith(restrictionName.toLowerCase())) {
