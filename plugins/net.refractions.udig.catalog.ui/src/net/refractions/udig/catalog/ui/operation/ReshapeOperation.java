...
public class ReshapeOperation implements IOp {
    ...
    public void op(final Display display, Object target, final IProgressMonitor monitor)
        ...
        final FeatureSource featureSource = handle.resolve( FeatureSource.class, null );
        Feature feature;
     ...
     * @return */
    public IGeoResource process(FeatureSource source, ReshapeDialog dialog, IProgressMonitor monitor ) throws IOException {
        if( monitor == null ) monitor = new NullProgressMonitor();

        monitor.beginTask(Messages.ReshapeOperation_task, 100 );

        final FeatureType featureType = dialog.getFeatureType();
        ...
        final FeatureStore store = scratch.resolve(FeatureStore.class, SubMonitor.convert(monitor,Messages.ReshapeOperation_createTempSpaceTask, 10));
            ...
            collection.accepts( new FeatureVisitor(){
                ...
                public void visit(Feature feature) {
                    ...
                    for( int i=0; i<expressionList.size(); i++){
                        values[i] = expressionList.get(i).getValue( feature );
                    }
                    try {
                        Feature created = featureType.create( values, feature.getID() );
                        store.addFeatures( DataUtilities.collection( new Feature[]{created} ));
                    } catch (Throwable t) {
                        if( warning ){
                            t.printStackTrace();
                            warning = false;
    ...
    class ReshapeDialog extends Dialog {
        ...
        public ReshapeDialog(Shell parent, Feature sample ) {
            super( parent );
            this.origional = sample.getFeatureType();
            this.sample = sample;
        ...
        protected Control createDialogArea(Composite parent) {
            ...
            ImageDescriptor desc = Images.getDescriptor(Images.ERROR_OVR);
            ImageRegistry imageRegistry = CatalogUIPlugin.getDefault().getImageRegistry();
            Image image = imageRegistry.get(Images.ERROR_OVR);
            if( image==null ){
                imageRegistry.put(Images.ERROR_OVR, desc.createImage());
            }
        ...
        String getDefaultText(){
            ...
            for( AttributeType descriptor : origional.getAttributeTypes() ){
         ...
         * @return a FeatureType created based on the contents of Text
         */
        private FeatureType createFeatureType() throws SchemaException {

            CoordinateReferenceSystem crs = sample.getFeatureType().getDefaultGeometry().getCoordinateSystem();

            FeatureTypeBuilder build = FeatureTypeBuilder.newInstance( origional.getTypeName() );
            ...
            for( int i=0; i<names.size(); i++){
                ...
                Object value = expression.getValue( sample );

                //hack because sometimes expression returns null.  I think the real bug is with AttributeExpression
                Class<?> binding = null;
                if( value == null){
                    if(  expression instanceof AttributeExpression){
                        String path = ((AttributeExpression)expression).getAttributePath();
                        AttributeType attributeType = sample.getFeatureType().getAttributeType(path);
                ...
                if( Geometry.class.isAssignableFrom( binding )){
                    type = AttributeTypeFactory.newAttributeType( name, binding, true, Integer.MAX_VALUE, null, crs );
                }
                else {
                    type = AttributeTypeFactory.newAttributeType( name, binding );
                }
                build.addType( type );
            }
            build.setName( getNewTypeName( origional.getTypeName() ) );
