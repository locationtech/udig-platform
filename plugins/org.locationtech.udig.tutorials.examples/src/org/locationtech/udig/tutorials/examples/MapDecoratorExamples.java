...
public class MapDecoratorExamples {
    ...
    public void lookupExample() {
        ...
        final ID GRID_ID = new ID("mapgraphic:///localhost/mapgraphic#grid", null); //$NON-NLS-1$
        IGeoResource gridResource = local.getById(IGeoResource.class, GRID_ID,
    ...
    public void draw(MapGraphicContext context) {
        ...
        List<Coordinate> coordinates = (List<Coordinate>) blackboard.get("locations"); //$NON-NLS-1$

        if (coordinates == null) {
            return; // no coordinates to draw
        }
        try {
            MathTransform dataToWorldTransform = CRS.findMathTransform(DefaultGeographicCRS.WGS84,
                    context.getCRS());
