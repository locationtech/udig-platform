...
public class WMSCTileUtils {
    ...
    public static TileSet toTileSet(IGeoResource resource, AbstractOpenWebService<?, ?> server,
        ...
        SubMonitor subMonitor = SubMonitor.convert(monitor, "TileSet generation", 100); //$NON-NLS-1$
        ...
        try {
            ...
            IGeoResourceInfo info = resource.getInfo(subMonitor.split(50));
            ...
            String configuredTilesetWidth = (String) properties.get(PreferenceConstants.P_TILESET_WIDTH);
            String configuredTilesetHeight = (String) properties.get(PreferenceConstants.P_TILESET_HEIGHT);

            Integer width;
            if (configuredTilesetWidth == null) {
                width = PreferenceConstants.DEFAULT_TILE_SIZE;
            } else {
                width = Integer.parseInt(configuredTilesetWidth);
            }

            Integer height;
            if (configuredTilesetHeight == null) {
                height = PreferenceConstants.DEFAULT_TILE_SIZE;
            } else {
                height = Integer.parseInt(configuredTilesetHeight);
            }
            ...
            if (resource.canResolve(Layer.class)) {
                Layer layer = resource.resolve(Layer.class, subMonitor.split(50));
