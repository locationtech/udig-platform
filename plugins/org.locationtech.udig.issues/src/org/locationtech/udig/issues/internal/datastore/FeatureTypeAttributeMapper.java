...
/**
 * Returns the names of attributes given a SimpleFeatureType that map to the different issue
 * properties.
 *
 * @author Jesse
 * @since 1.1.0
...
public class FeatureTypeAttributeMapper {
    ...
    /**
     * Creates a new instance with the "preferred" feature type
     */
    ...
    /**
     * Creates a new instance. Maps the required fields as good as possible to the fields present in
     * the schema.
     *
    ...
    public FeatureTypeAttributeMapper(SimpleFeatureType schema) {
        this.schema = schema;
        ArrayList<AttributeDescriptor> notMapped = new ArrayList<>();
    ...
    private static SimpleFeatureType createOptimalSchema(String featureTypeName)
            throws SchemaException {
        SimpleFeatureTypeBuilder builder = new SimpleFeatureTypeBuilder();
        builder.setName(featureTypeName);
        builder.crs(DefaultGeographicCRS.WGS84).add("bounds", MultiPolygon.class); //$NON-NLS-1$
        builder.length(128).nillable(true).add("description", String.class); //$NON-NLS-1$
        builder.length(80).nillable(false).defaultValue("").add("extensionId", String.class); //$NON-NLS-1$ //$NON-NLS-2$
        builder.length(80).nillable(true).add("groupId", String.class); //$NON-NLS-1$
        builder.length(80).nillable(false).add("id", String.class); //$NON-NLS-1$
        builder.length(1024).nillable(false).defaultValue("").add("memento", String.class); //$NON-NLS-1$ //$NON-NLS-2$
        builder.length(20).nillable(true).add("priority", String.class); //$NON-NLS-1$
        builder.length(20).nillable(true).add("resolution", String.class); //$NON-NLS-1$
        builder.length(1024).nillable(true).add("viewMemento", String.class); //$NON-NLS-1$
    ...
    private boolean isDescription(AttributeDescriptor att) {
        return compare(att.getName(), Messages.FeatureTypeAttributeMapper_description)
                && isStringType(att);
    }

    private boolean isPriority(AttributeDescriptor att) {
        return compare(att.getName(), Messages.FeatureTypeAttributeMapper_priority)
                && isStringType(att);
    }

    private boolean isResolution(AttributeDescriptor att) {
        return compare(att.getName(), Messages.FeatureTypeAttributeMapper_resolution)
                && isStringType(att);
    }

    private boolean isId(AttributeDescriptor att) {
        return ((compare(att.getName(), Messages.FeatureTypeAttributeMapper_id))
                || compare(att.getName(), Messages.FeatureTypeAttributeMapper_issue))
                && isStringType(att);
    }

    private boolean isGroupId(AttributeDescriptor att) {
        return compare(att.getName(), Messages.FeatureTypeAttributeMapper_group)
                && isStringType(att);
    }

    private boolean isMemento(AttributeDescriptor att) {
        return compare(att.getName(), Messages.FeatureTypeAttributeMapper_memento)
                && isStringType(att);
    }

    private boolean isViewMemento(AttributeDescriptor att) {
        return compare(att.getName(), Messages.FeatureTypeAttributeMapper_viewmemento)
                && isStringType(att);
    }

    private boolean isExtensionID(AttributeDescriptor att) {
        return compare(att.getName(), Messages.FeatureTypeAttributeMapper_extensionPoint)
                && isStringType(att);
    }
